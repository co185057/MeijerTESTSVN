...
bool DMCashCount::CurrencyDispensed(bool bBill, CString csDen,long lQuantity)
{
	//Read in Cash Counts
	int iDenomPosition = 0;
	int iCashCount = 0;
	//GetCashCounts(&caCashAcceptorCounts, &caCoinAcceptorCounts, &caCashDispenserCounts, &caCoinDispenserCounts);

	// Search through list for the Currency Denomination and update the Currency Count.
	if (bBill == true)
	{
		iDenomPosition = FindDenom(csDen,&caCashDispenserDenomination);

		if ((iDenomPosition != -1) && (iDenomPosition <= caCashDispenserCounts.GetUpperBound()))
		{
			iCashCount = caCashDispenserCounts.GetAt(iDenomPosition);
			iCashCount -= lQuantity;
			caCashDispenserCounts.SetAt(iDenomPosition,iCashCount);
			// Save the Cash Counts.
			return PutCashCountToRegistry(&caCashAcceptorCounts, &caCoinAcceptorCounts, &caCashDispenserCounts, &caCoinDispenserCounts);
		}
	}
	else
	{
		iDenomPosition = FindDenom(csDen,&caCoinDispenserDenomination);

		if ((iDenomPosition != -1) && (iDenomPosition <= caCoinDispenserCounts.GetUpperBound()))
		{
			iCashCount = caCoinDispenserCounts.GetAt(iDenomPosition);
			iCashCount -= lQuantity;
			caCoinDispenserCounts.SetAt(iDenomPosition,iCashCount);
			// Save the Cash Counts.			
			return PutCashCountToRegistry(&caCashAcceptorCounts, &caCoinAcceptorCounts, &caCashDispenserCounts, &caCoinDispenserCounts);
		}
	}

	trace(L6,_T("DMCashCount::CurrencyDispensed() Currency not updated"));
	//If we have arrived here no counts have been updated.
	return false;
...
bool DMCashCount::CurrencyAccepted(bool bBill, CString csDen, int nQuantity)
{
	...
	if (bBill == true)
	{
		iDenomPosition = FindDenom(csDen,&caCashAcceptorDenomination);

		if ((iDenomPosition != -1) && (iDenomPosition <= caCashAcceptorCounts.GetUpperBound()))
		{
			iCashCount = caCashAcceptorCounts.GetAt(iDenomPosition);
			iCashCount += nQuantity;
			caCashAcceptorCounts.SetAt(iDenomPosition,iCashCount);
			// Save the Cash Counts.
			return PutCashCountToRegistry(&caCashAcceptorCounts, &caCoinAcceptorCounts, &caCashDispenserCounts, &caCoinDispenserCounts);
		}
	}
	...
	{
		iDenomPosition = FindDenom(csDen,&caCoinAcceptorDenomination);

		if ((iDenomPosition != -1) && (iDenomPosition <= caCoinAcceptorCounts.GetUpperBound()))
		{
			iCashCount = caCoinAcceptorCounts.GetAt(iDenomPosition);
			iCashCount += nQuantity;
			caCoinAcceptorCounts.SetAt(iDenomPosition,iCashCount);
			// Save the Cash Counts.
			return PutCashCountToRegistry(&caCashAcceptorCounts, &caCoinAcceptorCounts, &caCashDispenserCounts, &caCoinDispenserCounts);
		}
	}

	trace(L6,_T("DMCashCount::CurrencyAccepted() Currency not updated"));
	//If we have arrived here no counts have been updated.
	return false;
}
...
bool DMCashCount::UpdateCashArrays()
{
	...
	//Obtain the latest from the registry
	bRetVal = GetCashCountFromRegistry(&pCashAcceptorCashCountString, &pCoinAcceptorCashCountString, &pCashDispenserCashCountString, &pCoinDispenserCashCountString);
	if (bRetVal == true)
	{
		trace(L6,_T("DMCashCount::UpdateCashArrays() Got Counts from the Registry"));
        iSuccess++;
		bRetVal = false;
	}
	...
	if (bRetVal == true)
	{
		trace(L6,_T("DMCashCount::UpdateCashArrays() Get CashAcceptor successful"));
        iSuccess++;
		bRetVal = false;
	}
	...
	if (bRetVal == true)
	{
		trace(L6,_T("DMCashCount::UpdateCashArrays() Get CoinAcceptor successful"));
		iSuccess++;
		bRetVal = false;
	}
	...
	if (bRetVal == true)
	{
		trace(L6,_T("DMCashCount::UpdateCashArrays() Get CashDispenser successful"));
        iSuccess++;
		bRetVal = false;
	}
	...
	if (bRetVal == true)
	{
		trace(L6,_T("DMCashCount::UpdateCashArrays() Get CoinDispenser successful"));
        iSuccess++;
		bRetVal = false;
	}
