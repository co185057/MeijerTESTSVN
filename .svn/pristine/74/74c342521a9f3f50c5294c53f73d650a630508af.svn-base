...
SMStateBase  *SMSmmKeyInWtTolBase::Initialize(void)
{
  ...
  //+SSCOP-2097: Use the localized decimal separator
  trace(L7,_T("Retrieving Locale for Decimal."));
    int cchData 
  ...
  trace(L7,_T("Done Retrieving Locale for Decimal. [%c]"), cLocalizedDecimal);

  CString csLocDec(cLocalizedDecimal);
  trace(L7,_T("Setting Locale for Decimal.[%s]"), csLocDec);
  m_pAppPSX->SetConfigProperty(_T("KeyBoardP3"), ps.GetCurrentContext(), UI::PROPERTYTEXT, csLocDec);
  trace(L7,_T("Done Setting Locale for Decimal."));
  delete [] pccbBuffer;
  //Perform Redraw to refresh screen
  CPSXRedrawLock  csRedrawLock(_T("Display"), _T("SmmKeyInWtTolBase::Initialize"));
  //-SSCOP-2097
