...
bool CSigReceiptDataFile::ConvertSig5991toBitmap(BSTR pRawData, LPCTSTR csBitmapFileName, int nWindowWidth, int nWindowHeight) 
{
	BYTE *PA;  
	long tpoints;
	bool bSuccess = true; 
	...
	if (bSuccess)
	{
	    bSuccess = ConvertSigPointsToBitmap(PA, tpoints, csBitmapFileName, nWindowWidth, nWindowHeight);
	}
	if(PA)
		delete [] PA;
	return bSuccess;
...
void CSigReceiptDataFile::PrintSignatureData(LPCTSTR lpReceiptType)
{
  ...
  BYTE *btString;
	...
	{
        SetSigWidth(0); // Using the default Width
        SetSigHeight(0); // Using the default Height
		// TAR 189420 RJF011902 need to check results to avoid deleting non-existent bitmap file
		if(ConvertSig5991toBitmap(sigData, csBitmapName, co.GetSignatureWindowWidth(), co.GetSignatureWindowHeight()))
		{
		  SysFreeString(sigData);
...
bool CSigReceiptDataFile::ConvertSigPointsToBitmap(BYTE* Points, long lPointCount, LPCTSTR BitmapFileName, int nWindowWidth, int nWindowHeight)
{
    ...
    if(GetSigWidth() == 0) // Using the default logic
    {
	    ...
	    if (sigHeight * nWindowWidth / sigWidth <= nWindowHeight) 
	    {
		    bmWidth = nWindowWidth;
		    bmHeight = sigHeight * nWindowWidth / sigWidth;
	    }
	    else
	    {
		    bmHeight = nWindowHeight;
		    bmWidth = sigWidth * nWindowHeight / sigHeight;
	    }

    }
    else // Using the fixed input
    {
        sigWidth = GetSigWidth();
        sigHeight = GetSigHeight();
        bmWidth = nWindowWidth;
        bmHeight = nWindowHeight;
    }
...
void CSigReceiptDataFile::PlotPointInBitmapData(long x, long y)
{
    ...
    int TopazSignatureBoldness = co.GetTopazSignatureBoldness();   
    if(co.fOperationsEnableTopazSignatureBoldness)
    {
        //trace(L6,_T("+PlotPointInBitmapData...TopazSignatureBoldness = [%ld]."), TopazSignatureBoldness);
        switch( TopazSignatureBoldness )
        {
            case 1: 
                c = 1 << (7 - xofs);
                break;
            ...
            case 255: 
                ...
                break;
            default: 
                // c = g_BoldnessOverride << (7 - xofs);
                trace(L6,_T("+PlotPointInBitmapData(): Invalid Value for Topaz(Bold) setting [Boldness] = [%ld].")
                            , TopazSignatureBoldness);
                TopazSignatureBoldness = 7;
                break;                  
        }
    }
...
BOOL CSigReceiptDataFile::PrintSlipData()
{
	...
	if (ReadString(strBuffer))  // slip data string ;
	{
		...
		dm.ReceiptLine(strBuffer + _T("\n"));
		strBuffer.Empty();
		SMStateBase::m_bSignaturePrintingStatus = true; //TAR 428027
        return TRUE;
	}
    else return FALSE;
}
...
void CSigReceiptDataFile::SetSigWidth(const long lValue)
{
    m_lSigWidth = lValue;
}

void CSigReceiptDataFile::SetSigHeight(const long lValue)
{
    m_lSigHeight = lValue;
}
