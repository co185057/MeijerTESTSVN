       ...
       /// <summary>
        ...
        private void UpdateWeightDetailsPanel()
        {
            if (IsInSystemMessageState())
            {
                ...
                /// VSTS 462319 - adjusted weight details panel placement on screen to prevent it from overlapping buttons.
                MainGrid.Margin = new Thickness(-125, weightDetailsMargin.Top, weightDetailsMargin.Right, weightDetailsMargin.Bottom);
            }
            else
            {
            ...
            {
                var terminalType = _viewModel.GetPropertyValue("TerminalType") as string;
                this.WeightDetailsPanel.Visibility = !string.IsNullOrEmpty(terminalType) && terminalType.Equals("SS90", StringComparison.OrdinalIgnoreCase) ? Visibility.Collapsed : Visibility.Visible;
            }
        }
        ...
        private void OnSCOMode()
        {
            ...
            if (ACSVersionTextBlock.Text.Length < 24 && VersionTextBlock.Text.Length < 24) //Default height can accommodate only 24 chars. Adjust if needed.
            {
                VersionBlock.Margin = new Thickness(0, 50, 0, 0);
            }
            else if (ACSVersionTextBlock.Text.Length > 24 && VersionTextBlock.Text.Length > 24)
            {
                VersionBlock.Margin = new Thickness(0, 30, 0, 0);
            }
            else
                VersionBlock.Margin = new Thickness(0, 40, 0, 0);

            VersionBlock.RowDefinitions[1].Height = ACSVersionTextBlock.Text.Length < 24 ? new GridLength(15) : new GridLength(25);
            VersionBlock.RowDefinitions[2].Height = VersionTextBlock.Text.Length < 24 ? new GridLength(15) : new GridLength(25);
        }
    }
}