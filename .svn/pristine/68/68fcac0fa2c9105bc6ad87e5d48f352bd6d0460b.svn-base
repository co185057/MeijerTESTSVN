...
void PSProceduresBase::RefreshCMTotal()
{
    ...
    m_pPSX->GetContext(csContext);

    m_pPSX->GetConfigProperty(_T("Amount1Label"), csContext, UI::PROPERTYTEXTFORMAT, v);

    CString csCurrentLabel(v.bstrVal);

    CString csChangeDue = GetPSText(TXT_CHANGEDUE);

    trace(L6, _T("csChangeDue = %s, csCurrentLabel = %s"), csChangeDue, csCurrentLabel);

    if(csChangeDue != csCurrentLabel)
    {
