...
{
  csOrigCode.Empty();								  // item code scanned, picked or keyed
  csItemCode.Empty();								  // item code without the scan type if any
  csOrgItemDescription.Empty();								  // item description if item is a linked item         
  
  // BeginCodeWLDBSA11
  csItemCodeWOPrice.Empty();					  // SA need a special UPC withoout Price 
...
{
  csDescription.Empty();							  // last item desc
  csItemCode.Empty();
...
void TenderDetails::Reset(void)
{
  ...
  csTender.Empty();					  // last tender desc
}
