// $Header: /Fastlane/4.4 Patch G/Development/Core/Source/SCOTAPP/PSXInterface.cpp 40    8/07/09 12:57p Bd123405 $
/*
 * @(#)PSXInterface.h    $Revision: 40 $ $Date: 8/07/09 12:57p $
 *
 ...
 */
/*
* $Log: /Fastlane/4.4 Patch G/Development/Core/Source/SCOTAPP/PSXInterface.cpp $
 * 
 * 40    8/07/09 12:57p Bd123405
 * FL 2605 - THD Migration - Dataneeded Enhancements
 * 
 * 39    7/10/09 5:55a pl250020
 * Fix for the P3 TAR 409017 - G2_374: Can log on to Store Mode
 * successfully even with the invalid ID/PW.
 * 
 * 38    5/11/09 4:51a Mm185096
 * RFC 2054 TAR 381428 RAP Sign On Changes
 * 
 * 37    12/19/08 5:35p Jj100011
 * Just changing trace to be less confusing.
 * 
 * 36    11/28/08 4:47p Jj100011
 * default USSF should be ON
 * 
 * 35    11/03/08 5:16p Tp151000
 * Check in for a try to see the performance is any better.
 * 
 * 34    10/17/08 1:55p Cs180011
 * 
 * 33    5/29/08 1:35p Hh185033
 * Added the search capability on the picklist
 * 
 * 33    5/15/08 1:51p hh185033
 * Implementing the search capacity for the picklist
 *
 * 32    3/17/08 1:51p Ah150000
 * Resolving code review comments for Hierarchical Picklist
 * 
 * 31    3/12/08 8:20p Cs185088
 * TAR 371639
 * 
 * 30    2/29/08 8:58a Ah150000
 * Hierarchical picklist
 * 
 * 29    1/17/08 12:04p Ml185042
 * RFQ681
 * 
 * 28    11/27/07 5:34p Lp185019
 * SR629 (TAR 334800) - Resolve CISP Issue: RCM is tracing password by
 * default
 * 
 * 27    9/20/07 5:30p Db185023
 ...
 * 
 * 26    8/14/07 6:40p Sk185127
 * RFC 356672
 * 
 * 25    8/01/07 4:33p Dk185053
 * TAR 356672
 * Change Text for skipbagging, item removed, & bag/box
 * Expanded button as well.
 * 
 * 24    6/12/07 1:30p Tp151000
 * remove some code.
 * 
 * 23    6/07/07 3:18p Dk185053
 * Prevent the unnessary events from Launchpad and RCM to scotapp.
 * 
...
CPSXInterface::SMappingEntry CPSXInterface::m_mappingEntries[] = {
	...
	MAP_ENTRY_N_0_0(_T("CMButton1LookUpIn3ButtonRow"),      PS_BUTTON1)
	...
	MAP_ENTRY_N_0_0(_T("CMButton2KeyInCodeIn3ButtonRow"),   PS_BUTTON2)
    ...
    MAP_ENTRY_N_0_0(_T("CMButton6OwnBag"),      PS_BUTTON6)
	...
	MAP_ENTRY_N_0_L(_T("SmmKeyBoardP1"),        PS_KEYSTROKE, 0) //SR821
    ...
    MAP_ENTRY_N_0_0_METHOD(_T("SmartWICTenderImage"),   PS_LISTLOOKUP, TenderImage) //POS138637 // RFQ 7477
    ...
    MAP_ENTRY_N_0_L(_T("SmAlphaNumP2"),           PS_KEYSTROKE, 1)
	MAP_ENTRY_N_0_L(_T("SmAlphaNumP3"),           PS_KEYSTROKE, 1)
    MAP_ENTRY_N_0_L(_T("SmAlphaNumP4"),           PS_KEYSTROKE, 1)
    MAP_ENTRY_N_0_0_METHOD(_T("HotKeys"),         PS_HOTKEY, HotKey)
    MAP_ENTRY_N_0_0_METHOD(_T("PFKBHotKeys"),     PS_HOTKEY, HotKey)
    ...
    MAP_ENTRY_N_0_0_METHOD(_T("PFKBLine1AlphaNumericKeys"),            PS_CHARKEY, CharKey)
    MAP_ENTRY_N_0_0_METHOD(_T("PFKBLine2AlphaNumericKeys"),            PS_CHARKEY, CharKey)
    MAP_ENTRY_N_0_0_METHOD(_T("PFKBLine3AlphaNumericKeys"),            PS_CHARKEY, CharKey)
    MAP_ENTRY_N_0_0_METHOD(_T("PFKBLine4AlphaNumericKeys"),            PS_CHARKEY, CharKey)
    MAP_ENTRY_N_0_0(_T("UIActivity"),        PS_ACTIVITY)
    MAP_ENTRY_N_0_0(_T("Display"),        PS_ACTIVITY) //Jira 1880
    MAP_ENTRY_N_0_0(_T("SymbolsKey"),     PS_SYMBOLKEY) 
...
//USSF END
    ...
    long lHandlingFlags = 0;
    CLaneStateData laneStateData(pPSX);
    if (evt.strRemoteConnectionName.IsEmpty() && UI::EVENTCLICK == evt.strEventName)
    {
        laneStateData.SendNextGenUIData(_T("RemoteProxyUserInputCompleted"), false);
    }

    PPSXEVENTHANDLER pHandler = NULL;  
	if (!((evt.nEvent == PS_ASSISTDENIED) || (evt.nEvent == PS_ASSISTACCEPT) || (evt.nEvent == PS_RAPSCANNERENGAGED) || (evt.nEvent == PS_RAPSCANNERAVAILABLE)
		|| (evt.nEvent == PS_RAPSCANNERUNAVAILABLE) || (evt.nEvent == PS_RAPSCANNERDEACTIVATED) || (evt.nEvent == PS_RAPPRINTERBITMAPNOTLOADED))) 	// POS181195 // RFQ 7477 //CRD149415
	{
    try
    {
...
bool IsReceiptUpDownEvent(CString contextName, CString controlName, CString eventName)
{	
	if( (contextName == _T("SmAssistMode") || contextName == _T("VoidItem") || contextName == _T("AM_VoidItem") ) &&
		(controlName == _T("SMReceiptScrollDown") || controlName == _T("CMReceiptScrollDown")) &&
		eventName == _T("Click")
		)
	{
		//Scroll event is coming from either AssistMode or VoidItem context
		return true;
	}	
                                 ...
                                 , PPSXEVENT pEvent )
{
	...
	//SSCOI-47558, SSCOI-47594 - Receipt item identifier received as string, not as integer. The below code handles 
	//the receipt click events for AssistMode and VoidItem. There are few more, which are out of these issues scope but
	//still need to be addressed by base SCO.
	if(IsReceiptUpDownEvent(evt.strContextName, evt.strControlName, evt.strEventName))
	{		
		//Scroll event is coming from either AssistMode or VoidItem context
		CString *receiptItemId = new CString;				
		*receiptItemId = OLE2T(pEvent->vParam.bstrVal);	
		if(!receiptItemId->IsEmpty())
		{
			evt.wParam = 1111; //Some unique number to indicate that lParam contains the receipt item id as string
			evt.lParam = (LPARAM) receiptItemId;                
		}
		else
			evt.wParam = evt.lParam = 0;
	}
	else
	{		
    ...
    else if( evt.strEventName == UI::EVENTCONNECTREMOTE )
    {
        ...
        trace( L6,  _T("%s temp:  %s"), UI::EVENTCONNECTREMOTE, temp);
        if (-1 != temp.Find(KEY_CT_OPERATION_SIGNON_AUTO))
        {
            trace( L6, _T("PSXInterface::AddOperator"));
            evt.nEvent = PS_ADDOPERATOR_AND_CONNECT;
        }
        else if( evt.strRemoteConnectionName.Find(REMOTE_CONNECTION_PREFIX) == 0 )
        {
    ...
    else if( evt.strEventName == UI::EVENTREMOTEDATA ) 
    {
        ...
        else if (-1 != temp.Find(KEY_CT_OPERATION_CHECKIN))  //POS81634
        {
            evt.nEvent = PS_RAPCHECKIN;
        }
        else if (-1 != temp.Find(KEY_CT_OPERATION_CHECKOUT))  //POS81634
        {
            evt.nEvent = PS_RAPCHECKOUT;
        }
// (+) POS181195
        else if(-1 != temp.Find(KEY_CT_SCANNER_ENGAGED))
        {
            evt.nEvent = PS_RAPSCANNERENGAGED;
        }
        else if(-1 != temp.Find(KEY_CT_SCANNER_AVAILABLE))
        {
            evt.nEvent = PS_RAPSCANNERAVAILABLE;
        }
        else if(-1 != temp.Find(KEY_CT_SCANNER_UNAVAILABLE))
        {
            evt.nEvent = PS_RAPSCANNERUNAVAILABLE;
        }
		else if(-1 != temp.Find(KEY_CT_SCANNER_DEACTIVATED)) //CRD149415
        {
            evt.nEvent = PS_RAPSCANNERDEACTIVATED;
        }
		else if (-1 != temp.Find(KEY_CT_RAPPRINTER_BITMAP_NOT_LOADED))
		{
			evt.nEvent = PS_RAPPRINTERBITMAPNOTLOADED;
		}
...
// (-) POS181195
    ...
    {
 		if(!co.fLocaleAltF4ExitAllowed) // SSCOP-1600
		{
			trace(L2, _T("ALT-F4 pressed, Ignore it because AltF4ExitAllowed=N") );
		}
		else
		{
    ...
    else if(evt.strEventName == _T("Signature"))
    {
        CString csParam = evt.vParam.bstrVal;
        if(-1 != csParam.Find(_T("start")))
        {
            evt.nEvent = PS_SIGNATURE_START;
        }
        else
        {
            evt.nEvent = PS_SIGNATURE_END;
            gpSCOTSigCap->SetcsSigData(csParam);
        }
    }
    else if(evt.strEventName == _T("PS_AUTHENTICATION_REQUEST_OPTIONAL"))
    {
        evt.nEvent = PS_AUTHENTICATION_REQUEST_OPTIONAL;
    }
    else if(evt.strEventName == _T("PS_AUTHENTICATION_REQUEST_REQUIRED"))
    {
        evt.nEvent = PS_AUTHENTICATION_REQUEST_REQUIRED;
    }    
    else if(evt.strEventName == _T("PS_AUTHENTICATION_SUCCESS"))
    {
        evt.nEvent = PS_AUTHENTICATION_SUCCESS;
    }    
    else if(evt.strEventName == _T("PS_AUTHENTICATION_INVALID_ID"))
    {
        evt.nEvent = PS_AUTHENTICATION_INVALID_ID;
    }
    else if(evt.strEventName == _T("PS_AUTHENTICATION_INVALID_PW"))
    {
        evt.nEvent = PS_AUTHENTICATION_INVALID_PW; // for future use
    }
                                     ...
                                     , PPSXEVENT pEvent )
{
    ...
    switch (pEvent->nEvent)
    {
        ...
        default :
            if (pEvent->strRemoteConnectionName.IsEmpty() &&
                UI::EVENTCLICK == pEvent->strEventName &&
                pEvent->strControlName != UI::CONTROLDISPLAY)
            {
                lHandlingFlags = lHandlingFlags | NOTIFY_USERINPUTCOMPLETED;  //set flag to notify NextGenUI after processing done
            }

            trace( L6, _T("PostPS %d,%d,%d,%s,%d")
                 , pEvent->nEvent
                 , pEvent->lParam
                 , pEvent->wParam
                 , pEvent->strContextName
                 , lHandlingFlags); // SSCOI-45254
#if defined(_SCOT) || defined(_SCOTSSF_)
...
// The TenderImage individual buttons will have button data set to this format:
// "index;tenderType"
...
bool CPSXInterface::TenderImage(SMappingEntry_ *pMappingEntry, PPSXEVENT pEvent)
{
    if (pEvent->strContextName != _T("Finish"))
    {
        return false;
    }    
    COleVariant v(pEvent->vParam);
    v.ChangeType(VT_BSTR);
    CString csParam(v.bstrVal);
    int iIndex = csParam.Find(_T(";"));
    if (iIndex > -1)
    {
        csParam = csParam.Left(iIndex);
    }
    SetupEntry(pMappingEntry, csParam);
    return true;
}
// -- NXTUI-73
// RFQ 7477
//+POS138637
bool CPSXInterface::SmartWICTenderImage( SMappingEntry_ *pMappingEntry
                               , PPSXEVENT pEvent )
{
    if ( pEvent->strContextName != _T("Finish") )
        return false;
...
// ++ Replaced the following function for NXTUI-73 SystemFunctions buttons
...
bool CPSXInterface::LanguageImage(SMappingEntry_ *pMappingEntry, PPSXEVENT pEvent)
{
    if (pEvent->strContextName != _T("SelectLanguage"))
    {
        return false;
                                ...
                                , PPSXEVENT pEvent )
{
   if ((pEvent->strContextName.Find(_T("Keyboard"))!=0) 
       && (pEvent->strContextName.Find(_T("OperatorKeyboard"))!=0)
       && (pEvent->strContextName.Find(_T("SmDataEntryAlphanumeric"))!=0)
       && (pEvent->strContextName.Find(_T("ProduceFavorites"))!=0)
       && (pEvent->strContextName.Find(_T("MultiSelectProduceFavorites"))!=0)
       && (pEvent->strContextName.Find(_T("XMCashier"))!=0)
       && (pEvent->strContextName.Find(_T("LookUpItem"))!=0)
       && (pEvent->strContextName.Find(_T("MultiSelectLookUpItem"))!=0))
        return false;
