...
namespace TerminalInfoCE
{
	...
	public class TerminalInfo : System.Windows.Forms.Form
	{
		...
		private ArrayList  CollectInfo( Hashtable dataPrompts, Hashtable dataFunctions )
		{
			...
			//add the HH version info.  version info is stored in a file
			string path = @"\scot\bin\lanesetup.txt";
			StringBuilder sVerString = new StringBuilder();
			string sStr = "";
			...
			if (File.Exists(path))
			{
				using (StreamReader sr = File.OpenText(path)) 
				{
					...
					while ((sStr = sr.ReadLine()) != null) 
					{
					    sVerString  = new StringBuilder( String.Format( "Version : {0}", sStr  ));
					}
				}
			...
			{	
				sVerString  = new StringBuilder( String.Format( "Version : UNKNOWN" ) );
			}	
			list.Add(sVerString.ToString() );
			return list;
		}
