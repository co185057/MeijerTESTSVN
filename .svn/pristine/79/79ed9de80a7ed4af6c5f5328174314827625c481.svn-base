...
#include "SafePayC.h"
#include "RecyclerDioParser.h"  // Add Glory CM Support
   ...
   if ( lResult == OPOS_SUCCESS )
	...
	// it means dispensing is successfully
      ...
      if ( sDispensed.IsEmpty() == FALSE ) 
		 ...
		 mo.PostDM(DM_STATUS, DMCLASS_CASHCHANGER, m_nDeviceId, CASH_STATUS_BILLS_TAKEN, NULL); //SR672
      }
	return lResult;
}
...
long CCashRecyclerWrapper::ReadCashCounts(BSTR* pCashCounts, BOOL* pDiscrepancy)
{
    ...
    long lResult = ReadCashCountsInternal( sNotePart, bDiscrepancy );   
    *pDiscrepancy = bDiscrepancy ; 
    // Add Glory CM Support
	...
	if ( lResult == OPOS_SUCCESS )
	{
        ...
        CString csFinalCashCounts = sCoinResult;
        if(m_bSubtractCassetteCounts)
        {
            CCashCounts cashCounts = (LPCTSTR)sCoinResult;
            cashCounts = cashCounts.SubtractLikeDenoms(cashCounts, ReadCassetteCounts());
            csFinalCashCounts = cashCounts.ToString().c_str();
            trace(L6, _T("ReadCashCounts result after subtracted by the cassette counts [%s]"), (LPCTSTR)csFinalCashCounts);
        }

        ::SysFreeString(*pCashCounts);
        *pCashCounts = csFinalCashCounts.AllocSysString();
	}
	return lResult;
}

CCashCounts CCashRecyclerWrapper::ReadCassetteCounts(void)
{   
    BSTR pCashCounts = ::SysAllocStringByteLen(NULL,200);
    CCashCounts cashCounts;

    if(ReadCassetteCounts(&pCashCounts) == OPOS_SUCCESS)
    {
        _bstr_t bstrCashCounts(pCashCounts, false);        
        pCashCounts = NULL;
        cashCounts = (LPCTSTR)bstrCashCounts;
    }

    trace(L7, _T("ReadCassetteCounts [%s]"), cashCounts.ToString().c_str());
    return cashCounts;
}
...
CString CCashRecyclerWrapper::GetDispensedCashList( void )
{
...
long CCashRecyclerWrapper::ReadCashCountsInternal( 
   CString& sResult, BOOL &bDiscrepancy  )
   {
   BSTR bstrCashCounts = NULL;
   sResult.Empty();
    long lResult = TOPOSCashChanger<CGenericOposIF>::ReadCashCounts( &bstrCashCounts , &bDiscrepancy );

    // Currently, we had experience problem on the bDiscrepancy flag 
    // during the ReadCashCounts() call above. So, we added another 
    // check here to using DIRECTIO()call to query the device cash unbalance status
    if(!bDiscrepancy)
    {
        bDiscrepancy = CheckCashBalanceStatus();
        trace(L6, _T("CheckCashBalanceStatus() [%d] "), bDiscrepancy);
    }
...
CString CCashRecyclerWrapper::GetLowIndicatorList( 
   ...
   CString sCoinResult;
   CString sNoteResult;
   BOOL bDispcrepancy = FALSE;  // Add Glory CM Support
   long lResult = ReadCashCountsInternal( sNotePart, bDispcrepancy );
...
// Add Glory CM Support
long CCashRecyclerWrapper::ReadCassetteCounts(BSTR* pCashCounts)
{
    trace(L7, _T("+CCashRecyclerWrapper::ReadCassetteCounts()"));

    CString csCoinResult, csNoteResult, csCashCount;
    BSTR bstrStatus = NULL;
    long lRetCode = OPOS_E_ILLEGAL;
    LONG lReadNo; 
    CRecyclerDioParser parserObj;
    RCYPSRSEISA datSeisa;

    lRetCode = DirectIO(CHAN_DI_SEISA, &lReadNo, &bstrStatus);
    if (lRetCode != OPOS_SUCCESS)
    {
        trace(L6, _T("DirectIO(CHAN_DI_SEISA..) call FAILED [%d]"), lRetCode );
        ::SysFreeString( *pCashCounts );
        *pCashCounts = NULL;
        return lRetCode;
    }

	CString csStatus(bstrStatus);
	::SysFreeString(bstrStatus); // cs185088 mem
    lRetCode = parserObj.Parse(RCYPSR_CMD_SEISA, &lReadNo, 
        csStatus, &datSeisa, sizeof(datSeisa));

    if (lRetCode != OPOS_SUCCESS)
    {
        trace(L6, _T("Parse(RCYPSR_CMD_SEISA..) call FAILED [%d]."), lRetCode);
        ::SysFreeString( *pCashCounts );
        *pCashCounts = NULL;
        return lRetCode;
    }
    ...
    BOOL bDiscrepancy = FALSE;  
    long lResult = ReadCashCountsInternal( csCashCount, bDiscrepancy ); 
    trace(L7, _T("ReadCashCountsInternal csCashCount [%s]"), csCashCount);

    if(lResult == OPOS_SUCCESS)
    {
        CString csCoin = StrToken( csCashCount, _T(";") );
        for(int nIndex=0; nIndex < m_aDispenseCoins.GetSize(); nIndex++)
        {
            TClassPtr<CCashDispenseUnit> pUnit = m_aDispenseCoins.GetAt( nIndex );
            
            // No coin acceptor bin for Glory. Forcing to set the counts to zero.
            BuildCashCountString(csCoinResult, pUnit->m_dwUnit, 0); 
        }

        for(nIndex=0; nIndex < m_aNoteDenom.GetSize(); nIndex++)
        {
            long lCount = 0;
            switch (m_aNoteDenom[nIndex])
            { 
                case 1000:
                    lCount = datSeisa.nCountOf1000YenInCst;
                    break;
                case 2000:
                    lCount = datSeisa.nCountOf2000YenInCst;
                    break;
                case 5000:
                    lCount = datSeisa.nCountOf5000YenInCst;
                    break;
                case 10000:
                    lCount = datSeisa.nCountOf10000YenInCst;
                    break;
                default:
                    trace(L7, _T("ERROR: Denomination [%d] not supported."), m_aNoteDenom[nIndex]);
                    break;
            }
            BuildCashCountString( csNoteResult, m_aNoteDenom[nIndex], lCount );
        }

        CString csUpdatedCassetteCounts = csCoinResult;
        csUpdatedCassetteCounts += _T(";");
        csUpdatedCassetteCounts += csNoteResult;
        ::SysFreeString( *pCashCounts );
        trace(L6, _T("Updated bill cassette counts [%s]"), csUpdatedCassetteCounts);
        *pCashCounts = csUpdatedCassetteCounts.AllocSysString();
    }
    trace(L7, _T("-CCashRecyclerWrapper::ReadCassetteCounts()"));
    return 0;
}
...
// Add Glory CM Support
bool CCashRecyclerWrapper::CheckCashBalanceStatus(void)
{
    trace(L7, _T("+CCashRecyclerWrapper::CheckCashBalanceStatus()"));

    bool bAbnormal = false;
    LONG lReadNo01 = 0x01;      // for Coin Status
    LONG lReadNo80 = 0x80;      // for Bill Status
    BSTR bstrStatus01 = NULL;
    BSTR bstrStatus80 = NULL;
    LONG lResult = OPOS_E_ILLEGAL;
    
    CRecyclerDioParser parserObj;
    LONG                lData01 = RCYPSR_DATA_STATREAD01;
    LONG                lData80 = RCYPSR_DATA_STATREAD80;
    RCYPSRSTATUS01      datRead01;
    RCYPSRSTATUS80      datRead80;

    // Get details of the units
    LONG lResultCoin = DirectIO(CHAN_DI_STATUSREAD, &lReadNo01, &bstrStatus01);
    LONG lResultBill = DirectIO(CHAN_DI_STATUSREAD, &lReadNo80, &bstrStatus80);

    // Parse coin recycler data.
	CString csStatus01(bstrStatus01);
	::SysFreeString(bstrStatus01); //cs185088 mem
    lResult = parserObj.Parse(RCYPSR_CMD_STATUSREAD, &lData01, 
        csStatus01, &datRead01, sizeof(datRead01));

    if(lResult != OPOS_SUCCESS)
    {
        ::SysFreeString(bstrStatus80);
        trace(L6, _T("Cash recycler status call for coins FAILED  [%d]."), lResult);
        return false;
    }

    // Parse note recycler data
	CString csStatus80(bstrStatus80);
	::SysFreeString(bstrStatus80); //cs185088 mem
    lResult = parserObj.Parse(RCYPSR_CMD_STATUSREAD, &lData80, 
        csStatus80, &datRead80, sizeof(datRead80));

    if(lResult != OPOS_SUCCESS)
    {
        trace(L6, _T("Cash recycler status call for notes FAILED [%d]."), lResult);
        return false;
    }

    bAbnormal = (datRead80.fAbnormalBalance) ? true : false;
    bAbnormal = (datRead01.fAbnormalBalance) ? true : bAbnormal;

    trace(L7, _T("-CCashRecyclerWrapper::CheckCashBalanceStatus() [%d]"), bAbnormal);
    return bAbnormal;
}

void CCashRecyclerWrapper::ReadRegistry( HKEY hKey )
{ 
    trace(L7, _T("+CCashRecyclerWrapper::ReadRegistry()"));

    CString sData = ReadRegString( hKey, _T("SubtractCassetteCounts") );
    sData.MakeUpper();
    m_bSubtractCassetteCounts = true;
    if(sData.GetLength() > 0)
    {
        if(sData.Left(1) == _T("T"))
        {
            m_bSubtractCassetteCounts = true;
        }
        else if (sData.Left(1) == _T("F"))
        {
            m_bSubtractCassetteCounts = false;
        }
    }

    trace(L7, _T("-CCashRecyclerWrapper::ReadRegistry() m_bSubtractCassetteCounts [%d]"), m_bSubtractCassetteCounts);
}
