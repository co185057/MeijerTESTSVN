...
FP_TENDERSELECTED				CTransBroker::sm_lpfnTenderSelected; // (*) SR797 ====================
...
FUNCTIONPOINTERMAP s_FnPtrMap[] = 
   {
      ...
      {"TBTenderSelected",				 (FARPROC*)&CTransBroker::sm_lpfnTenderSelected,}, // (*) SR797 ====================
      //Old TBAPIs list: ++begin	
...
TBRC CTransBroker::LogReceipt(LPTSTR* lpFileName,BSTR* bstrSignatureString, LPTSTR* lpReceiptType)
{
  ...
  Trace(L0,_T("-CTransBroker::LogReceipt retCode:%s"), csRetCode);
  return nRetCode;   
}
...
TBRC CTransBroker::PreprocessSignatureData(LPCTSTR lpFileName,BSTR bstrSignatureData, LPCTSTR lpReceiptType, BOOL* bProcessData)
{
  ...
  Trace(L0,_T("-CTransBroker::PreprocessSignatureData retCode:%s"), csRetCode);
  return nRetCode;   
}
...
void GetTBReturnStateName(TBRC nTBState, CString & csStateName)
{
    ...
    _T("OUTOFSYNCNEEDASSISTMODE"), //dp185016 Req SR058
        ...
        _T("PRINTLINE"),
        _T("NEEDSIGNATURE"),
        _T("NEEDSIGANDCONFIRM"),
        _T("CLEARSLIPDATA"),
        _T("NEEDSIGNSLIP"),
        _T("CASHMANAGEMENTCOMPLETED"),
        _T("TENDERSTARTED"), // (*) SR797 ====================
        _T("READY_TO_TENDER"),
        _T("READY_FOR_ITEM"),
        _T("GENERIC_DELAYED"),
        _T("GENERIC_DELAYED_COMPLETE"),
		_T("UNKNOWNTBSTATE")
	};

    int nArraySize = sizeof(pTBReturnStates)/sizeof(pTBReturnStates[0]);
	...
	{
	   TRACE(_T("warning: !!!!! Couldnt find the state name for TB state id: %d, number of defined names = %d"), nTBState, nArraySize-2);
...
TBRC CTransBroker::SetTrainingModeState(const bool bTMState)
{
	...
	Trace(L0,_T("-CTransBroker::SetTrainingModeState() retCode:%s"), csRetCode);
	return nRetCode; 
}
...
TBRC CTransBroker::GetMessageInfo(PSMessageInfo psMessageInfo)
{
  ...
  Trace(L0,_T("-CTransBroker::GetMessageInfo() retCode:%s"), csRetCode);
  return nRetCode;   
}
TBRC CTransBroker::CreateReportEntry(PSCreateReportEntry psCreateReportEntry)
{
  ...
  Trace(L0,_T("-CTransBroker::CreateReportEntry() retCode:%s"), csRetCode);
  return nRetCode;   
}
TBRC CTransBroker::GetCurrentBusinessDate(CString &csBusinessDate)
{
  ...
  Trace(L0,_T("-CTransBroker::GetCurrentBusinessDate() retCode:%s"), csRetCode);
  return nRetCode;   
}
TBRC CTransBroker::SetBusinessDate(LPCTSTR szBusinessDate)
{
  ...
  Trace(L0,_T("-CTransBroker::SetBusinessDate() retCode:%s"), csRetCode);
  return nRetCode;   
}
...
TBRC CTransBroker::GetCustomerMessage(LPTSTR* psCustomerMessageText, LPTSTR* psCustomerMessageAudio)
{
  ...
  Trace(L0,_T("-CTransBroker::GetCustomerMessage() returns %s"), csRetCode);
  return nRetCode;
}
...
TBRC CTransBroker::GetRewardLineDetails(PSRewardLineDetails psRewardLineDetails)
{
  ...
  Trace(L0,_T("-CTransBroker::GetRewardLineDetails() returns %s"), csRetCode);;
  return nRetCode;
}
...
TBRC CTransBroker::NeedStartOfDay(TBSTARTOFDAYSTATE &eState)
{
  ...
  Trace(L0,_T("-CTransBroker::NeedStartOfDay() returns %s"), csRetCode);
  return nRetCode;
}
...
TBRC CTransBroker::GetPrinterError(LPTSTR* psErrorMessage)
{
  ...
  Trace(L0,_T("-CTransBroker::GetPrinterError() returns %s"), csRetCode);
  return nRetCode;
}
...
TBRC CTransBroker::EnterMICR(
   ...
   Trace(L0,_T("-CTransBroker::EnterMICR() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetDocEndorsementInfo(
   ...
   Trace(L0,_T("-CTransBroker::GetDocEndorsementInfo() returns %s, code:%d"), csRetCode, nRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::SetCashDrawerState(
   ...
   Trace(L0,_T("-CTransBroker::SetCashDrawerState() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetExpectedCashDrawerState(long *plExpectedState)    // Bitmapped (lsb is drawer 1, etc.)
   ...
   Trace(L0,_T("-CTransBroker::GetExpectedCashDrawerState() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::SetSecurityLevel(
   ...
   Trace(L0,_T("-CTransBroker::SetSecurityLevel() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetExpectedSecurityLevel(long *plExpectedLevel)
   ...
   Trace(L0,_T("-CTransBroker::GetExpectedSecurityLevel() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetCurrentSecurityLevel(long *plCurrentLevel)
   ...
   Trace(L0,_T("-CTransBroker::GetCurrentSecurityLevel() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::EndorsementCompleted(
   ...
   Trace(L0,_T("-CTransBroker::EndorsementCompleted() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetAssistModeStatus(
   ...
   Trace(L0,_T("-CTransBroker::GetAssistModeStatus() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetMICRPrompt(
   ...
   Trace(L0,_T("-CTransBroker::GetMICRPrompt() returns %s"), csRetCode);
   return nRetCode;
   }
...
TBRC CTransBroker::GetDataNeededView(PSDataNeededView psDataNeededView)
{
  ...
  Trace(L0,_T("-CTransBroker::GetDataNeededView() retCode:%s"), csRetCode);
  return nRetCode;   
}
...
TBRC CTransBroker::SendDataNeeded(LPCTSTR szDataNeededDetails)
{
  ...
  Trace(L0,_T("-CTransBroker::SendDataNeeded() retCode:%s"), csRetCode);
  return nRetCode;   
}
   ...
   LPTSTR *pszLoyaltyCardNumber) 
{
   ...
   Trace(L0,_T("-CTransBroker::GetLoyaltyCardNumber() returns %s"), csRetCode);
   return nRetCode;
}
   ...
   TBTENDERTYPE nTenderType) 
{
   ...
   Trace(L0,_T("-CTransBroker::DisableTenderButton(%d) returns %d"), nTenderType, nRetCode);
   return nRetCode;
}
   ...
   long *plBalance) 
{
   ...
   Trace(L0,_T("-CTransBroker::GetGiftCardBalance() returns %s"), csRetCode);
   return nRetCode;
}
...
TBRC CTransBroker::GetExtensionStateSetup(LPTSTR szUICtx, LPTSTR szDevCtx, psPropChangeRequestList pExtChangeList)
{
	...
	Trace(L0,_T("-CTransBroker::GetExtensionStateSetup retCode:%s"), csRetCode);
	return nRetCode;   
}
...
TBRC CTransBroker::SyncTBorPOS()
{
   ...
   Trace(L0,_T("-CTransBroker::SyncTBorPOS() returns %s"), csRetCode);
   return nRetCode;
...
TBRC CTransBroker::PowerShutdown()
{
  ...
  Trace(L0,_T("-CTransBroker::PowerShutdown retCode:%s"), csRetCode);
  return nRetCode;   
}
...
BOOL CTransBroker::ValidateHeadCashier( LPCTSTR szCashierID, LPCTSTR szCashierPw )
{
	...
	Trace(L0,_T("-CTransBroker::ValidateHeadCashier() retCode ="), bRetCode);
...
// (+) SR797 ====================
TBRC CTransBroker::TenderSelected(TBTENDERTYPE nTenderType)
{
   Trace(L0, _T("+CTransBroker::TenderSelected"));
   TBRC nRetCode;

   if(sm_lpfnTenderSelected)
   {
   		nRetCode = (*sm_lpfnTenderSelected)(nTenderType);
   }
   else
   {
	   nRetCode = TB_E_FUNCTIONNOTSUPPORTED;
   ...
   ::GetTBReturnCodeName(nRetCode, csRetCode);
	...
	Trace(L0, _T("-CTransBroker::TenderSelected retCode: [%s] "), csRetCode);
