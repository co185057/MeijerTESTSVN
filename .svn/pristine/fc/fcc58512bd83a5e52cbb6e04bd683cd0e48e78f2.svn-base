...
CString SMStateBase::lvSMLastState = _T("");	// 071904 TAR 273325 BMediano (storing the last state)
...
bool SMStateBase::bInSuspendCall = false;
...
bool SMStateBase::m_bIsTenderStarted = false; //SR797

bool SMStateBase::bIsMaxCouponLimitInterventionDone = false;	//SR678

CString SMStateBase::csVoidedItemID =_T("");;
...
SMStateBase *SMStateBase::TBParse(MessageElement *me)
{
    ...
    fItemScanned = false; //SSCOP-3184 (SSCOP-3139)
	...
	else if(TBIsAllowed(TB_FT_QUICKITEMENTRY) && (me->nDeviceID == TB_DEV_DISPLAYPRIMARY) && (me->lParam != NULL))
	{
		CString csTBState = (LPTSTR) me->pData;
		nTBLastState = (TBSTATE) (_ttol(csTBState));
		if(nTBLastState != TB_MESSAGESCREEN &&
           nTBLastState != TB_SENDMESSAGETOSCOT && 
           nTBLastState != TB_ITEMUNKNOWN &&
           nTBLastState != TB_PREFERENCEDATA) //TAR 182862
		{
...
//USSF End
	...
	if (nTBLastState != TB_UNKNOWN && nTBLastState != TB_IGNORE) //RFC 275377 MIGRATED-BM185018
	{
		...
		GetTBReturnStateName(nTBLastState, csTBStateName);
		if ((nTBLastState == TB_TENDERVOIDED) ||      //Do not save these as nTBLastRealState
			(nTBLastState == TB_CREATE_REPORT_ENTRY))
		{
			trace(L3, _T("!!!! TB State id=%d, name:%s, nTBLastRealState not changed, value %d"), nTBLastState, csTBStateName, nTBLastRealState);
		}
		else
		{
			nTBLastRealState = nTBLastState;
			trace(L3, _T("!!!! TB State id=%d, name:%s, nTBLastRealState changed to %d"), nTBLastState, csTBStateName, nTBLastRealState);

            CLaneStateData laneStateData(m_pAppPSX);
            laneStateData.SendNextGenUIData(_T("NextGenUITBLastState"), (long)nTBLastState );
		}
  ...
  case	TB_OPTIONLOADING:
		if (fInSignedOn || fInSignedOff || fStateAssist || fSAInTransaction)
		{
            ...
            SendTBCommand(TBCMD_CMDSTRING, _T("LOAD_DELAYED"));
			return STATE_NULL;
		}
		else
		{
            ...
            SendTBCommand(TBCMD_CMDSTRING, _T("LOAD_COMPLETE"));
			if (pStartState)
			{
			   return pStartState;
			}
		...
		break;
  case TB_CLOSED:
		m_cmMgr.SetInTransaction(false); //TAR 450629
	  // at ****** CLOSED ******
...
//		setAnchorState(BES_START);
...
#ifndef _CPPUNIT
	    m_cmMgr.SetInTransaction(true);//TAR 450629
	  // at SELECT PROCEDUURE
		...
		{
            trace(L6, _T("fInSignedOn and/or fInSignedOff is true, returning STATE_NULL"));
			//Tar 124345
			return STATE_NULL;
		...
		{
            trace(L6, _T("fInLoyaltyCardProcessing is true, returning STATE_NULL"));
			return STATE_NULL;
		}
		...
		if (fItemIsWaiting)
		{
            if ( g_bIsSecurityTransactionPolicyReady )
            {
                //+degraded mode tars
            ...
            {
                trace(L6, _T("not calling TBItemSale, waiting for KEY_OSC_OPERATION_TRXPOLICY_READY"));
                return STATE_NULL;
            }
		}
        ...
        if (!g_bIsSecurityTransactionPolicyReady)
        {
            trace(L6, _T("Waiting for KEY_OSC_OPERATION_TRXPOLICY_READY"));
            return STATE_NULL; 
        }
  ...
  case TB_NEEDWEIGHT:
		...
		{
		    //simply return, SSF will handle this to not allow weight required items 
			return STATE_NULL;
		}
		...
		if (fSAInItemSale || co.IsInAssistMenus()) //RFQ 986
		{
			...
			if (!dm.IsScaleEnable())
			{
                io.d.bPickListItem = ms_bIsCurrentItemFromPickList;
				SASendSCOTInput(CANCEL_ITEM, io.csItemCode, 0, 0, 1);
				io.fNotForSale = true;
				// (+)SSCOB-18794
				...
				//(-)SSCOB-18794, Commented the below line.
  ...
  case TB_NEEDPRICE:
		...
		sDescription.Format(_T("%s"),csTemp + io.d.csDescription);  //SSCOB-2536 -- correct crash when unusual item descriptions are used
...
//USSF END
  ...
  case TB_ITEMSOLD:
	{
#ifndef _CPPUNIT
    if((io.fVoid) && (g_bPicklistAssistVoid))
    {
        g_bPicklistAssistVoid = false;
    ...
    //+ SSCOB-309
    if(io.d.bLinkedItem!=true && io.csOrigCode.Left(1)==LINK_ITEM)
    {
        trace(L6,_T("SSCOB-309 fix : Striping preceding # signs because item is NOT a LINKED ITEM"));
        int nOrigCodeLen = io.csOrigCode.GetLength();
        if(nOrigCodeLen)
        {       
            for (int nCtr = 0; nCtr < nOrigCodeLen; nCtr++)
            {
                if (io.csOrigCode[nCtr] == LINK_ITEM)
                {
                    io.csOrigCode.SetAt(nCtr, _T(' '));
	...
	{
		g_bAllowLateBagging = false; //SSCOADK-5284 in case if the previous item is AllowLateBagging and the current item is sold without a call to TBItemSale
    ...
    if (g_bHavePicklist)
    {
        g_bHaveSoldFromPicklist = true;
    }
    // - Personalization picklist
    //+ Dynamic Quickpick
    if (g_bHaveQuickPick)
    {
        g_bHaveSoldFromQuickPick = true;
    }
    //- Dynamic Quickpick
	...
	//////////////////////////////////////////////
     ...
     if(io.fVoid && ps.GetCurrentItemMobileItemFlag())
     {
        ms_lMobileShopperItemCount -= io.d.lQtySold;
     }
     ...
     {
         ms_lMobileShopperItemCount += io.d.lQtySold;
         ps.SetCurrentItemAsMobileItem(true);
     }
  ...
  case TB_TENDERDECLINEDKEEPCARD:
    ...
    trace(L6, _T("TB_TENDERDECLINED nTenderType=%d nDMCashInserted=%d"),nTenderType,nDMCashInserted);
	...
	if (nTenderType == TB_TENDER_CASH)
	{
        ...
        nDMCashTendered=0;
        trace(L6, _T("CASH tender DECLINED nDMCashInserted=%d, m_nDMCashPaid=%d, nDMCashTendered=%d"),nDMCashInserted,m_nDMCashPaid,nDMCashTendered);
        trace(L6, _T("Waiting for TB to guide the app to the next state"));
        return STATE_NULL;
	}
	else
	{
  ...
  case TB_TENDERCANCELLED:
	...
	//TAR376815+
	else if  ( (short(lTotalPaid)==(nDMCashInDrawer+nDMCashInserted)) && isAnySoldVoidedItems() ) 
	{
...
#ifndef _CPPUNIT
      rp.SetItemCount( GetDisplayedItemCount() );
  ...
  case TB_TENDERACCEPTED:
	{
		...
		//+SR832, store the lowest threshold tender value. 
		//In mix tender, replace the previous value if the new threshold tender value is less than the old threshold tender value.
		int nTempIndex = nTenderButtonIndex;
		nTenderCount++;
        
        //SSCOB-18168: Remove, nTenderButtonIndex will not be the original since TenderType was already processed in TBPrcessPIPTenderType
        //nTenderButtonIndex = GetButtonIndex(nTenderType); // Get tender index for TB Tender type
        if ( nTenderButtonIndex != -1)
        {
            nTempIndex = nTenderButtonIndex;   // Use index for TB Tender Type
        }
        else
        {
            trace(L0, _T("ERROR: TB returned a tenderType: %d not configured on ScotTend"), nTenderType);
        }
		if ( 0 < nTempIndex ) 
		{
			if ( 1 == nTenderCount ) 
			{ //Only one tender is used.
				nLowestPrintThresholdTenderIndex = nTempIndex;
				nLowestPrintThresholdTenderVal = co.TenderList[ps.m_languageUsed][nTempIndex - 1].nPrintReceiptTrxOverAmount;
			}
			else 
			{ //Mix tender is used.
				if ( 0 == co.TenderList[ps.m_languageUsed][nTempIndex - 1].nPrintReceiptTrxOverAmount ) 
				{ //Override existing value, the new tender PrintReceiptTrxOverAmount value is equal to zero.
					nLowestPrintThresholdTenderIndex = nTempIndex;
					nLowestPrintThresholdTenderVal = 0;
				}
				else if ( 0 < co.TenderList[ps.m_languageUsed][nTempIndex - 1].nPrintReceiptTrxOverAmount ) 
				{ 
					if ( nLowestPrintThresholdTenderVal > co.TenderList[ps.m_languageUsed][nTempIndex - 1].nPrintReceiptTrxOverAmount ) 
					{ //Override existing value, the new tender PrintReceiptTrxOverAmount value is less than the existing value.  
						nLowestPrintThresholdTenderIndex = nTempIndex;
						nLowestPrintThresholdTenderVal = co.TenderList[ps.m_languageUsed][nTempIndex - 1].nPrintReceiptTrxOverAmount;		
					}
				}
		...
		trace(L6, _T("SMStateTBBase::TBParse() - Threshold Tender Index = %d, Threshold Tender Value = %d"), nLowestPrintThresholdTenderIndex, nLowestPrintThresholdTenderVal);
		//-SR832

        if ( 1 > nTempIndex ) 
            ps.EchoAmountPaidToReceipt(nTenderType, lLastTender); // SR804
        else
            ps.EchoAmountPaidToReceipt(nTempIndex, lLastTender);
...
#endif //_CPPUNIT
  case TB_GENERIC_DELAYED:
	  {
		trace(L7, _T("SMStateBase::TBParse - 0x%08.8x <%s>"), nTBLastState, _T("TB_GENERIC_DELAYED"));
//USSF START
		USSFHOOK(_T("TBXNeedMoreData"), nextState = OnTBGenericDelayedIntervention(), return nextState;);
		//return OnTBNeedData();
//USSF END
	  }
  case TB_GENERIC_DELAYED_COMPLETE:
      {
         //ra.OnGenericDelayedApproval();
  ...
  case TB_TRXEND:
	  {
        ...
        long lItemCount = GetDisplayedItemCount();
		trace(L7, _T("+rp.SetItemCount"));
        if (!fInVoidTrans)
        {
            rp.SetItemCount( lItemCount ); // RFC 371075
        }
        ...
        trace ( L6, _T("TotalNumberOfItems = %d, TransactionTotalAmount = %d."), lItemCount, lTotalSale);
        if(lTotalSale > 0)
        {
            custInfo.SetMetricLong( _T("TransactionTotalAmount"), lTotalSale);
            trace ( L6, _T("Set the Value of TransactionTotalAmount: %d"),custInfo.GetMetricLong(_T("TransactionTotalAmount")));
        }
        if( lItemCount > 0)
        {
            custInfo.SetMetricLong( _T("TotalNumberOfItems") , lItemCount);
            trace ( L6, _T("Set the Value of TotalNumberOfItems: %d"),custInfo.GetMetricLong(_T("TotalNumberOfItems")));
        }
        //-SR814.5 SR815.6 SR851 Personalization and Security
...
//USSF END
  ...
  case TB_ITEM_QUANTITY_RESTRICTED:
    ...
    if(!co.IsInAssistMenus() && !fStateAssist && !g_bInRecallMode)
    {
...
//USSF End
			...
			if (pLoyaltyCardNumber && _tcsclen(pLoyaltyCardNumber))
			{
                ...
                delete [] pLoyaltyCardNumber;
                pLoyaltyCardNumber = NULL;
                ...
                //send only loyalty card number to StopLift if both options are enabled
                if(g_bReportTrustLevel && g_bIsSecureCamEnable)
                {
                    CKeyValueParmList parmList1;
                    CKeyValue<const _TCHAR*>::Put(parmList1, KEY_OPERATION, KEY_AC_SEND_LOYALTY_STOPLIFT);
                    CKeyValue<const _TCHAR*>::Put(parmList1, KEY_CUSTOMER_ID, csLoyaltyCardNumber);
                    CKeyValue<const _TCHAR*>::Put(parmList1, KEY_TRANSACTION_ID, rp.m_csTransactionID);
                    _bstr_t bstrParmList((const _TCHAR *)parmList1);
                    bstrResult = NULL;
                    SecMgr.AppControl(lSecMgrCookie, bstrParmList, &bstrResult);
                    ::SysFreeString(bstrResult);
                }
...
//	 		    || co.fOperationsPartialIntegratedPINPad  ) //TAR235030 CHu 042103 for PIP: swapped loyalty card
            if(!m_bCouldBeScannedLoyaltyCard)  //TAR242494
			{
				...
				else
				{
                    ...
                    // TAR 398436 release object if not used
                    SMStateBase *tempSMReturn = TBGetTotalDetails();//Tar 219951
                    if (!((tempSMReturn >= STATE_NULL) && (tempSMReturn <= STATE_ERROR)))
                    {
                        delete tempSMReturn;
                    }
...
//USSF End
			...
			else	// no track data, must have been a scan
			{
                ...
                // TAR 398436 release object if not used
                SMStateBase *tempSMReturn = TBGetTotalDetails();//Tar 124992
                if (!((tempSMReturn >= STATE_NULL) && (tempSMReturn <= STATE_ERROR)))
                {
                    delete tempSMReturn;
                }
...
//USSF End
				...
				if(fInMultiSelectPickList)
				{
					...
					//SSCOADK-4590 Fix Migrated from ADK 5.0.1_Accumalative.
					//SSCOADK-4589
					//SSCOB-17375               
                    
                   DMSaySecurity(SEC_NONE, ps.GetSoundName(FREQSHOPACCT));	
                   bAlreadyPlayedLCReminder = true;
				   RETURNSTATE(ProduceFavorites)  //Security manager won't send anything if MultiSelectPickList flag is true so don't wait.  Go back
                                                   //to ProduceFavorites so user has to press Finished button and will exist MultiSelect mode cleanly
				}
				//410299+
				if(co.IsInAssistMenus() || fStateAssist || co.IsAttendantModeOn())  //440440
				{
					// by default goto S&B
					RETURNSTATE(ScanAndBag)
				}
...
//Fiscal Printer end
  ...
  case TB_TRXSUSPENDED:
  {
    ...
    g_bInRecallMode = false;
    recallList.Reset();
    SASendSCOTInput( TENDER_START ); //sscoadk-3612: tell SM to change state to Finalize
    //SSCOADK-6337: send this message to SM so SM will handle weight events at Finialize state correctly. 
  ...
  case TB_REWARDLINE:
	  {
          ...
          memset(&RewardItemDetails, 0, sizeof(SRewardLineDetails));
		  ...
		  if (RewardItemDetails.szDescription)
		  {
			  ...
			  try
			  {
                  ...
                  RewardItemDetails.szDescription = NULL;
			  }
			  catch(...)
			  {
  ...
  case TB_PRINTER_VERIFY:
	  {
		...
		if (((!ps.RemoteMode() && !co.IsInAssistMenus() && !storeMode()) && fSAInItemSale))
		{	
			fItemSalePending = true;
			iItemUPCtype = io.cScanType;
			csPendingItemCode =io.csItemCode;
			SASendSCOTInput(CANCEL_ITEM, io.csItemCode, 0, 0, 1);		
		}
...
//Fiscal Printer end
  ...
  case TB_LANEOPEN:
        ...
        if(csCSN.Find(_T("TakeCard")) != -1 ||
            csCSN.Find(_T("TakeChange")) != -1 ||
            csCSN.Find(_T("TakeReceipt")) != -1 ||
            csCSN.Find(_T("ThankYou")) != -1)
        {
            trace(L5, _T("TB_LANECLOSED received during completion of tranx.  Will go to LaneClosed after Attract is reached."));
            return STATE_NULL;
        }
        else
        {
  ...
  //-RFC 986
  case TB_PREFERENCEDATA:
        {
            if (me->lParam)
            {
                CString csPreferenceCommand = (LPCTSTR) me->lParam; 	//csLastDisplay;
                return TBPreferenceData(csPreferenceCommand);
            }
            else
            {
                trace(L5, _T("TB_PREFERENCEDATA recieved with no data"));
                return STATE_NULL;
            }

        }
  //+SR797 
  case TB_TENDERSTARTED:
	{
		if (m_nTenderSelected != TB_TENDER_NONE)
			SetTenderStarted(true);
		else
		...
		return ProcessTBTender(m_nTenderSelected);
	}
  case TB_CASHMANAGEMENTCOMPLETED:
    {
        trace(L5, _T("CM did not handle it (TB_CASHMANAGEMENTCOMPLETED) so ignore it here too"));
        return STATE_NULL;
    }
...
TBSTATE SMStateBase::TBProcessMessage(MessageElement* me)
{
    ...
    if(m_cmMgr.IsOpen()) 
    {
        return m_cmMgr.ProcessMessageHook(me);
    }
    // SR752-
#endif //_CPPUNIT
...
SMStateBase *SMStateBase::TBItemDetails(void)
{
  ...
  //sscoadk-6989 if( ( nTBLastState == TB_ITEMSOLD) && !fSAInItemSale && !io.d.bLinkedItem && (io.d.lRequireSecBagging != TB_OVERRIDE_SECURITY_WITH_NOWEIGHT) 
  if( ( nTBLastState == TB_ITEMSOLD) && !fSAInItemSale && !io.d.bLinkedItem
			&& SMStateBase::m_nTBOutOfSyncError == 0)  //sscoadk-4556 //sscoadk-6003
  {
  ...
  else
  {
	  ...
	  //item as restricted.	  
	  bAutoApprovedItem =  ( co.IsInAssistMenus() || fStateAssist ) && !io.d.fRestricted; //sscoadk-7256
  ...
  if (io.d.fVisualVerify)
  {
    ...
    int nEntryID;
    bool bApprovedVisualVerifyItem;

	if (io.fVoid) // voiding a visual item
	{
      CString csEntryID;
      csEntryID = csVoidedItemID;
      nEntryID   = _ttoi(csEntryID);
      trace(L6, _T("Voided visual verify item: %d"), nEntryID);
	  ...
	  {
        if (g_mapTransactionVisualVerify.Lookup( nEntryID, bApprovedVisualVerifyItem))
        {
            if(!bApprovedVisualVerifyItem)
               lUnapprovedVisualVerifyItems--;
            g_mapTransactionVisualVerify.RemoveKey(nEntryID);
        }
	...
	else // selling a visual item
	{
		...
		if (!bAutoApprovedItem || co.IsInAssistMenus() )  //TAR#125677 Only worry about approving vv items if not in assist mode
		{
            ...
            trace(L6, _T("Purchased visual verify item: %d"), nEntryID);
            if (!g_mapTransactionVisualVerify.Lookup( nEntryID, bApprovedVisualVerifyItem) || io.lUpdateFields != ITEMUPDATE_NONE)
            {
                g_mapTransactionVisualVerify.SetAt(nEntryID, false);
                lUnapprovedVisualVerifyItems++;
            }
            else
            {
                trace(L6, _T("Duplicate Item ID"));
            }
            ...
            //sscop-1704+
            CString csAnchorStateName;
            if (co.IsAttendantModeOn())
		    {
			    csAnchorStateName = ps.GetPSText(MSG_ATTENDANT, SCOT_LANGUAGE_PRIMARY);
		    }
		    else if (co.IsInAssistMenus())
		    {
			    csAnchorStateName = ps.GetPSText(MSG_ASSISTMODE, SCOT_LANGUAGE_PRIMARY);
		    }
            else
            {
			    csAnchorStateName = getSCOTStateName();//Tar 247393
            }
            //sscop-1704-

			CString csInstruction;
			csInstruction.Format(ps.GetPSText(SCOT_LANE_STATUS, SCOT_LANGUAGE_PRIMARY), csDesc, ps.GetPSText(RA_LANEBTT_VISUALVALIDATIONITEM, SCOT_LANGUAGE_PRIMARY), csAnchorStateName, 
				co.IsSecurityDisableOn()==true || co.GetSecurityDisableSubCheckOn() ? ps.GetPSText(RA_SECONDLINE_SECURITYDISABLED, SCOT_LANGUAGE_PRIMARY) : _T(""), ps.GetPSText(RA_THIRDLINE_VISUALVALIDATIONITEM, SCOT_LANGUAGE_PRIMARY));
  ...
  if (io.d.fRestricted)
  {
	...
	if (io.fVoid) // voiding a restricted item
	{
	  ...
	  if (io.lRestCode != 3)
	  {
		  if (lRestrictedItems)
			lRestrictedItems--;
		  if (lUnapprovedRestrictedItems && !bAutoApprovedItem && io.d.nRestrictedAge > nCustomerAge)
			lUnapprovedRestrictedItems--;
		  if (lRegularYellowNeedingApproval && !bAutoApprovedItem && io.d.nRestrictedAge > nCustomerAge)
			lRegularYellowNeedingApproval--;

		  //TAR 211080 LPM 080202 begin: if two restricted items but only one voided,
		  if ((nCustomerAge == 0) || (nCustomerAge < io.d.nRestrictedAge)) //SSCOADK-756
		  ...
		  if (m_lAgerestrictQueuedtoRAP == 0)
		  {
			...
			{	trace(L7, _T("+ra.OnNoSaleApproval"));
				ra.OnNoSaleApproval();
				trace(L7, _T("-ra.OnNoSaleApproval"));
			}
			//sscoadk-321-		  
	  ...
	  io.lRestCode = 2;
#ifndef _CPPUNIT
      if (restrictedItemNotAllowed() && (lUnapprovedRestrictedItems > 0))
	  {
	...
	{
	  if (restrictedItemNotAllowed() && ( co.GetfOperationsAutoVoidUnderageRestrict()))	 //Feng122299
	  {
	  ...
	  else
	  {
		  ...
		  //must void this item
		  if(nCustomerAge && co.GetfOperationsAutoVoidUnderageRestrict())
			io.lRestCode = 3;
		  else
		  {
			lRestrictedItems++;
            ...
            if(!co.IsInAssistMenus())
            {
                fUnapprovedRestrictedItemFromCustomerMode = true;
            }
			...
			if (     co.GetfOperationsAllowSaleofRestrictedItems() 
				|| ( co.IsAttendantModeOn() && co.GetfOperationsAllowSaleofRestrictedItemsInAssist() )
				|| ( co.IsInAssistMenus() || fStateAssist ) //sscob-8563/sscop-5297
			   )
			{
			    int nYearRequired;
                ...
                ra.OnAgeRestrictedItem(csInstruction, io.d.nRestrictedAge, lUnapprovedRestrictedItems, FALSE, co.fStateDOBNotNeededButton ); //SR803
			...
			{ //SSCOADK-321+
				CString csAnchorStateName = _T("");//Tar 211737
				CString csDesc, csInstruction;
				csDesc = ps.GetPSText(LTT_UNSELLABLEITEM, SCOT_LANGUAGE_PRIMARY);
						
				csInstruction.Format(ps.GetPSText(SCOT_LANE_STATUS, SCOT_LANGUAGE_PRIMARY), csDesc, ps.GetPSText(RA_LANEBTT_TIMERESTRICTEDITEM, SCOT_LANGUAGE_PRIMARY), csAnchorStateName, co.IsSecurityDisableOn()==true ? ps.GetPSText(RA_SECONDLINE_SECURITYDISABLED, SCOT_LANGUAGE_PRIMARY) : _T(""), _T(""));

				trace(L7, _T("+ra.OnNoSale, Message=%s"), csInstruction);
				ra.OnNoSale(csInstruction);
				trace(L7, _T("-ra.OnNoSale"));
			}
			//SSCOADK-321-
  ...
  if (io.d.fCoupon)
  {
    if (io.fVoid)
    {
      ...
      if( !co.fOperationsIncludeCouponsInItemCount )
        ...
        if (g_mapTransactionCoupon.Lookup( csTmp, nCouponCount ))
        {
            ...
            int nNewMaxCount = 0;
            POSITION nPos = g_mapTransactionCoupon.GetStartPosition();
            while (nPos != NULL)
            {
                CString csKey;
                int nValue;
                g_mapTransactionCoupon.GetNextAssoc(nPos, csKey, nValue);
                if( nValue > nNewMaxCount)
                    nNewMaxCount = nValue;
                
            }
            nCountPossibleDuplicateCoupon = nNewMaxCount;
        }
        else
        {
            trace(L6, _T("Coupon Code: %s is not found on Coupon's Map"), csTmp);
        }
    ...
    //+SR678 moved here for TAR 441252, checking also changed
    if (io.d.fCoupon && (-io.d.lUnitPrice > co.nOperationsMaxCouponLimit) && (co.nOperationsMaxCouponLimit >= 0)
        && !bIsMaxCouponLimitInterventionDone && !co.IsInAssistMenus() && !fStateAssist)
    {
  ...
  if (!io.d.bLinkedItem) //431367 & 445164
  {
  if (io.fVoid ) 
  {
  ...
  //
  //Examples:
  //SubstCkPriceEmbeddedWeights=Y, lUnitPrice > 0, lWeight == 0, lQtySold <=1 will use calculated wt from unit price and will NOT update the WLDB
  //SubstCkPriceEmbeddedWeights=Y, lUnitPrice > 0, lWeight == 0, lQtySold >1 will use WLDB entries and WILL update the WLDB
  ...
  if (io.b_gIsThisItemPriceEmbedded && !io.fVoid)	// if this is a price embedded (and not a void)
  {
	...
	{
	  if (!co.GetfStateForceWeighingOFPriceEmbedded())
	  {
		...
		else if ( 0 == io.d.lWgtSold )
		{
			...
			// if a unit price exists, then calculate the Price Embedded expected wt
			if ((io.lUnitPriceEntered > 0) && (io.d.lQtySold <= 1))
			{
				//possible loss of data compile warning on line below is intentional and OK : DE200005
				io.l_gPriceEmbeddedWt = (long)(((double)io.lExtPriceEntered / io.lUnitPriceEntered) * 1000.0);
	  ...
	  {
		trace(L6, _T(" SCOTOption ForceWeighingOFPriceEmbedded = %d. Using scanner scale data for Wt Comparison."), co.GetfStateForceWeighingOFPriceEmbedded());
	  }
	}
  ...
  if (io.lRestCode<3)
  {
			...
			{ // if a coupon
				...
				{ // normal item
						...
						&& (TB_OVERRIDE_SECURITY_WITH_YES != io.d.lRequireSecBagging) )) //413251
					{
                         if (!io.d.bLinkedItem) 
                         {
                             // SSCOP-3821: Removed fix of TAR 409946
                            trace(L5, _T("TBItemDetails: SASendSCOTInput CANCEL_ITEM"));
                            SASendSCOTInput(CANCEL_ITEM, io.csItemCode, 0, 0, 1);
                         }
					}
	...
	if (fInMultiSelectPickList && !io.fVoid && io.lRestCode !=3)//SR773 //sscop-3345
	{
		RETURNSTATE(ProduceFavorites);
	}
  ...
  {
    //sscoadk-7083: for vv bags, no need to say ApprovalNeeded; it will be played at WaitforApproval immediately
	if (!co.IsImmedInterventionOn() && getAnchorState() != BES_SELLBAGS )  //Don't say it if intervention will be immediately handled 
    {
  ...
  if (io.fVoid && !io.d.bLinkedItem)
  {
	if (io.lRestCode==4 || io.lRestCode==0 || io.lRestCode == 2 )
	  DMSaySecurity(SEC_NONE, ps.GetSoundName(ITEMCANCELED)); // SSCOADK-613
  ...
  if (io.fVoid && !fStateAssist && !io.d.bLinkedItem && !co.IsInAssistMenus())
  {
	  ...
	  if (fInMultiSelectPickList)
	  {
          if(io.lRestCode==2)
          {
              CREATE_AND_DISPENSE(VoidApproval)(BES_STATERETURN, BES_PRODUCEFAVORITES);
          }
  ...
  if (io.lRestCode==4)
  {
    ...
    if (io.fVoid && (csStateName == _T("SMSmVoidAndRemoveItem")) ) 
    {
      //enable OK button
  ...
  //+ SSCOB-309
  //do not append # if item is not a linked item
  if(io.d.bLinkedItem==true)
  {
...
//409479+ No need to tell SM to resume. 
 ...
 //409469-
  ...
  if (io.d.fCoupon && bEnterTenderState && !io.d.fVoid)
  {
      ...
      //+SSCOADK-6526
      else if((!fStateAssist) && co.IsInAssistMenus())
      {
          RETURNSTATE(SmAssistMenu);
      }
      //-SSCOADK-6526
  ...
  //+SR803 stay in SmVoidAndRemoveItem state and wait for pressing "OK" button   
  CString csStateName = mp.sCurr->GetRuntimeClass()->m_lpszClassName;  
  if (io.fVoid && lUnapprovedRestrictedItems == 0 && (csStateName == _T("SMSmVoidAndRemoveItem")) ) 
  {
   	  ps.ButtonState(_T("SMButton1"), true, true);  //SSCOADK-321
	  return STATE_NULL;
  }
  //-SR803
  ...
  {
    //SSCOADK-7224+
 	  if (co.IsInAssistMenus() && lUnapprovedRestrictedItems > 0 && io.d.fVoid && io.d.fRestricted)
	  {

      RETURNSTATE(SmRestrictedItems) //staying in the assistmenu, no need to go to anchor state
    }
    //SSCOADK-7224-
	  return createAnchorState(); //if it is in AssistMode, mpprocedure will not go to anchorstate, will stay inside of assitsmode
  }
  ...
  //SR803 - Transferred declaration of csStatename above as CString
  //csStateName = mp.sCurr->GetRuntimeClass()->m_lpszClassName;
  ...
  //410299+
  if ( !co.IsInAssistMenus() && !fStateAssist && !co.IsSecurityBypassOn() )	//SSCOB-1306, SSCOP-969
  {
...
void SMStateBase::TBGetItemDetails(ItemDetails &iDetails)  //Get Item Details, transfer to item object.
{
    ...
    SetNextGenUICurrentItem((LPCTSTR)dItem.szItemCode, (LPCTSTR)dItem.szDescription, dItem.fRestricted, dItem.fVisualVerify, dItem.lQuantity);
   ...
   iDetails.csErrDescription = (LPCTSTR)dItem.szErrDescription;
	...
	if (!iDetails.csItemCode.IsEmpty())
	{
        ...
        if (co.IsInAssistMenus() && io.csItemCode.IsEmpty())
        {
            io.csItemCode = iDetails.csItemCode;
            fSAInItemSale = true;
        }
	}
	...
	if (dItem.szItemID) 
    {
        ...
        {
            trace(L6,_T("delete exception:dItem.szItemID"));
        } 
    }
	if (dItem.szDescription)
    {
		try
        {	
            delete [] dItem.szDescription;
            dItem.szDescription = NULL;
        }catch(...)
        {
            trace(L6,_T("delete exception:dItem.szDescription"));
        }
    }
	if (dItem.szErrDescription)
    {
		try
        {
            delete [] dItem.szErrDescription;
            dItem.szErrDescription = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:dItem.szErrDescription"));
        }
    }
	if (dItem.szItemCode)
    {
		try
        {
            delete [] dItem.szItemCode;
            dItem.szItemCode = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:dItem.szItemCode"));
        }
    }
...
void SMStateBase::TBGetItemExtendedDetails(ItemExtendedDetails &iXDetails)	//Get Item Extended Details, transfer to item object.
{
  ...
  //iXDetails members enter initialized so it is not necessary to call
	...
	if(dXItem.szCategory)
    {
		try
        {
            delete [] dXItem.szCategory;
            dXItem.szCategory = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:dXItem.szCategory"));
        }
    }
	if(dXItem.szExceptionWav)
    {
		try
        {
            delete [] dXItem.szExceptionWav;
            dXItem.szExceptionWav = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:dXItem.szExceptionWav"));
        }
    }
}
...
SMStateBase *SMStateBase::TBGetTotalDetails(void)
{
	...
	// +SR797
	if(dTotal.nVersion)
		lBalanceDue = dTotal.lBalanceDue;
	else
		lBalanceDue = dTotal.lTotal - lTotalPaid;
	// -SR797

	//lBalanceDue = dTotal.lTotal - lTotalPaid; // commented out by SR797
	...
	if (dTotal.szErrDescription)
    {
		try
        {
            delete [] dTotal.szErrDescription;
            dTotal.szErrDescription = NULL;
        } catch(...)
        {trace(L6,_T("delete exception:dTotal.szErrDescription"));
        }
    }
  ...
  //tar 350863 - end
	...
	int nAmountDue = nDMCashTendered ? nDMCashTendered : nDMCashInserted; // SSCOP-3887
	ps.UpdateAmountDueAmountPaid(lBalanceDue - nAmountDue, lTotalPaid + nAmountDue );
	
	CString csCurrentStateName = getStateName(this);
    ...
    if (   ((lBalanceDue - nDMCashInserted) <= 0)
        ...
        && (csCurrentStateName.Find(_T("SMCmDataEntry")) == -1 ) // SSCOADK-2945 - Don't move off DataEntry for TB_TOTAL
        && (csCurrentStateName.Find(_T("SMSmDataEntry")) == -1 ) // SSCOADK-2945
       )
    {
...
SMStateBase *SMStateBase::TBBalanceDetails(void)
{
	...
	*/
    ...
    //sscoadk-34121: save the tender description from TB
    ps.SetcsTenderDescription(dTender.szDescription);
    ...
    nTenderButtonIndex = GetButtonIndex(nTenderType); // SSCOB-18168: get the nTenderButtonIndex before TenderType will be processed
	//PIP
	...
	if( nDMCashInserted  && lBalanceDue <= 0) //SSCOADK-1532
	{
	...
	if (dTender.szDescription)
    {
		try
        {
            delete [] dTender.szDescription;
            dTender.szDescription = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:dTender.szDescription"));
        }
    }
	if (dTender.szErrDescription)
    {
		try
        {
            delete [] dTender.szErrDescription;
            dTender.szErrDescription = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:dTender.szErrDescription"));
        }
    }
...
SMStateBase* SMStateBase::TBGetTenderDetails(void)
{
	...
	if(!fTenderDeclined || (dTender.nTenderType != nTenderType))   //If tenderdeclined, do not need get these data
	{
...
SMStateBase *SMStateBase::TBEnterItem(const bool fOverride, int lUpdateFields)// sell item currently in the ItemObject
{
    ...
    if (io.fVoid && io.d.csItemID.IsEmpty() )
	{
        ...
        {
            CString csGenDataID;
            if (isItemGenericDelayed(io.d.csItemID, csGenDataID))
            {
                CQueuedActionGenericDelayedIntervention *pDelayedIntervention;
                pDelayedIntervention = (CQueuedActionGenericDelayedIntervention *)m_GenericDelayedInterventions.Get(csGenDataID);
                if (pDelayedIntervention != NULL)
                {
                    SendSmartAssistExceptionClear( _T("Lane"), csGenDataID.Mid(2));
                    ra.OnGenericDelayedApproval(csGenDataID, pDelayedIntervention->GetReportingID());
...
{
#ifndef _CPPUNIT
	//+SR078 Hold off selling item if allow late bagging
...
//USSF END
    ...
    else if (co.IsPicklistAssistEnable() && !fScannedItem)
    {
        io.d.bPickListItem = ps.IsPickListItem(io.csItemCode);
    }
	ms_bIsCurrentItemFromPickList = io.d.bPickListItem;

    if (g_lQty != 0)
    {
        io.lQuantityEntered = g_lQty;
        g_lQty = 0;
    }
...
/////////////////////////////////////////////////////////////////////////////
bool SMStateBase::IsCardTender(int nTenderType)
{
	int nButtonIndex = GetButtonIndex(nTenderType);
	
	if(nTenderType > TB_TENDER_CASH && nTenderType != TB_TENDER_VOUCHERPAYMENT && nTenderType != TB_TENDER_GIFTCARD)
	{
		return true;
	}
	else if(nTenderType == TB_TENDER_GIFTCARD
		&& co.GetnOperationsGiftCardRedemption() == 2
		&& !co.GetTenderButton(ps.GetLanguage(), nButtonIndex).bIntegratedMSR)
	{
		return true;
	}
...
//	SASendSCOTInput(TRANSACTION_COMPLETE );
	...
	if (IsCardTender(nType))  // credit or debit
	{
		RETURNSTATE(CardProcessing)
	}
	...
	// cash tender
	if(nTenderType == TB_TENDER_CASH) //SSCOP-5674 - don't change cash variables if tender is not CASH type
	{
		// cash tender
    ...
    if(co.fLocaleLeaveCashAcceptorsEnabledDuringTransaction && (lBalanceDue <= lTotalPaid + nDMCashInserted + nDMCashTendered))
	{
		DMCoinAcceptorDisable();	
		DMCashAcceptorDisable();	
	}
...
long SMStateBase::TBInitialize(HWND hwnd, LPCTSTR szFilePath)
{
    ...
    { 
        trace(L6,_T("No need to call tb.Initialize() since it has been done successfully)"));
        rc = TB_SUCCESS; 
        return rc;
    }
    //SSCOP-3606-
...
BOOL SMStateBase::TBValidateOperator(LPCTSTR szOper, LPCTSTR szPass)
{
    ...
    if(co.GetfOperationsEncodeOperatorIDPW())
    {
        ...
        csID = base64.GetBase64EncodeWithPrefix(_T("BASE64:"), csID);
        csPW = base64.GetBase64EncodeWithPrefix(_T("BASE64:"), csPW);
    }
    
    BOOL brc = tb.ValidateOperator(csID,csPW);
...
{
  //Clear Quiet Mode approval inside Recall Mode, the wt change should be belong to the recall mode items
  if(g_bInRecallMode && bQuietNeedSecApproval)
  {
  ...
  if(fRequestForHelp) // tar 242625
  {
      ...
      csInstruction.Format(ps.GetPSText(SCOT_LANE_STATUS, SCOT_LANGUAGE_PRIMARY), csDesc, ps.GetPSText(BTT_HELP, SCOT_LANGUAGE_PRIMARY), csAnchorStateName, 
		  co.IsSecurityDisableOn()==true || co.GetSecurityDisableSubCheckOn() ? ps.GetPSText(RA_SECONDLINE_SECURITYDISABLED, SCOT_LANGUAGE_PRIMARY) : _T(""), ps.GetPSText(RA_THIRDLINE_CALLFORHELP, SCOT_LANGUAGE_PRIMARY));
  ...
  // -Fix TAR246479
    ...
    //SSCOADK-2090+
    if (rc == TB_SUCCESS_IMMEDIATE && !g_bIsSecurityTransactionPolicyReady)
    {
        if (!fSAInItemSale)
        {
            trace(L3, _T("Got TB_SUCCESS_IMMEDIATE and waiting for Policy)"));
            rc = TB_SUCCESS;
  ...
  if(rc == TB_SUCCESS_IMMEDIATE && !fInLoyaltyCardProcessing && !fStartTrxInAM)
  {
	...
	{
		if (!fSAInTransaction)
			SASendSCOTInput( ITEMIZATION );//TAR450037 tell SM to start a transaction
...
SMStateBase *SMStateBase::TBFinish(void)
{
    if (g_bMobileAppEnabled)
        mobileCart.SendFinishTransaction(lTotalSale, TBGetCurrentTransactionNumber());
#ifndef _CPPUNIT
	fInSignedOn = false;
	fInSignedOff = true;
...
SMStateBase * SMStateBase::TBEndOfTransaction(void)
{
 ...
 // if(TenderRequireSignature())
	...
	//+SR832, don't print receipt if the lowest threshold tender value (PrintReceiptTrxOverAmount) is greater than or equal to the total transaction amount.
	//Never print receipt if the lowest threshold tender value (PrintReceiptTrxOverAmount) is equal to -1.
	if ( ( -1 == nLowestPrintThresholdTenderVal || nLowestPrintThresholdTenderVal >= lTotalSale ) && ( 0 < lTotalSale ) && bPrintReceipt ) 
	{
		bPrintReceipt = false;
	}
	trace(L6, _T("SMStateTBBase::TBEndOfTransaction() - Print Receipt = %d, Lowest Threshold Tender Value = %d, Total Transaction Value = %d"), bPrintReceipt, nLowestPrintThresholdTenderVal, lTotalSale);
	//-SR832

	//+SR Personalization Digital Receipt
    // If there is a conflict between the shopper's personalization option to print a paper receipt and 
    // the SCOTTend option to not print a receipt depending on tender/balance combinations, 
    // determine which one will have the priority. 
	if ( custInfo.PersonalizationActive() && io.bLoyaltyCard )
    {
        if ( co.fOperationsPersonalizationReceipt )
        {
            CString csReceiptPreference = custInfo.GetPreference(_T("Receipt"));
            trace(L6, _T("SMStateTBBase::TBEndOfTransaction() - Personalization Receipt Preference = %s"), csReceiptPreference);
        
            //csReceiptPreference = _T("Digital");
            if ( !bPrintReceipt )
            {
                if ( csReceiptPreference == _T("Paper") || csReceiptPreference == _T("Both") )
                {
                    bPrintReceipt = true;
                }
            }
            ...
            {
                if ( csReceiptPreference == _T("Digital") || csReceiptPreference == _T("None") )
                {
                    bPrintReceipt = false;
                }
            }
...
void SMStateBase::EndOfTransactionPrint()
{
	...
	if(AllowImmediateReceiptPrint())
	{
		...
		if (hFile!=NULL)
		{
			...
			{
				if(co.fOperationsAddPaperCutToTransReceipt && bPrintReceipt) //SSCOADK-5497	
				{
        ...
        {
            //TAR 453862 Do not show Print Receipt button for null receipt
            // 
            bPrintReceipt = true;
        }
				...
				TBPINENCODING& nPinBlockEncoding)
{
	...
	if(sPinKey.szWorkingKey)
    {
		try
        {
            delete [] sPinKey.szWorkingKey;
            sPinKey.szWorkingKey = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:sPinKey.szWorkingKey"));
        }
    }
	if(sPinKey.szAccountNumber)
    {
		try
        {
            delete [] sPinKey.szAccountNumber;
            sPinKey.szAccountNumber = NULL;
        } catch(...)
        {
            trace(L6,_T("delete exception:sPinKey.szAccountNumber"));
        }
    }
...
SMStateBase* SMStateBase::TBGetMessageInfo(const bool disableSA)  // TAR 127247
{
    ...
    {
		try
        {	delete [] sTempMessageInfo.szScreenText;
        sTempMessageInfo.szScreenText = NULL;
        }
		catch(...)
        {trace(L6,_T("delete exception:sTempMessageInfo.szScreenText"));}
    }
	if (sTempMessageInfo.szPlayWave)
    {
		try
        {
            delete [] sTempMessageInfo.szPlayWave;
            sTempMessageInfo.szPlayWave = NULL;
        }
		catch(...)
        {
            trace(L6,_T("delete exception:sTempMessageInfo.szPlayWave"));
        }
    }
	if (sTempMessageInfo.szLeadThruText)
    {
		try
        {
            delete [] sTempMessageInfo.szLeadThruText;
            sTempMessageInfo.szLeadThruText = NULL;
        }
		catch(...)
        {
            trace(L6,_T("delete exception:sTempMessageInfo.szLeadThruText"));
        }
    }
	if (sTempMessageInfo.szE_ReceiptText)
    {
		try
        {
            delete [] sTempMessageInfo.szE_ReceiptText;
            sTempMessageInfo.szE_ReceiptText = NULL;
        }
		catch(...)
        {
            trace(L6,_T("delete exception:sTempMessageInfo.szE_ReceiptText"));
        }
    }
	if (sTempMessageInfo.szRAP_MessageText)
    {
		try
        {
            delete [] sTempMessageInfo.szRAP_MessageText;
            sTempMessageInfo.szRAP_MessageText = NULL;
        }
		catch(...)
        {
            trace(L6,_T("delete exception:sTempMessageInfo.szRAP_MessageText"));
        }
    }
	...
	{
       //sscoadk-6740+: need to set the language right since scotapp could go from store mode to CustomMessage which is a customermode state
	   ...
	   if(bReturnToScanAndBag)   //tar 265812
	   {
		  //+TAR 322945
		   if(getAnchorState() == BES_START)
		   {
			   ...
			   if (co.GetfStateDualLanguage())
			   {
				   return TBStart(co.Getlanguage(ps.m_languageUsed).szCode);
			   }
			   else
			   {
				   return TBStart();
...
void SMStateBase::TBCreateReportEntry()
{
	...
	if (sTempCreateReportEntry.szRecordType)
    {
		try
        {
            delete [] sTempCreateReportEntry.szRecordType;
            sTempCreateReportEntry.szRecordType = NULL;
        }
		catch(...)
        {
            trace(L6,_T("delete exception:sTempCreateReportEntry.szRecordType"));
        }
    }
	if (sTempCreateReportEntry.szValue)
    {
		try
        {
            delete [] sTempCreateReportEntry.szValue;
            sTempCreateReportEntry.szValue = NULL;
        }
		catch(...)
        {
            trace(L6,_T("delete exception:sTempCreateReportEntry.szValue"));
        }
    }
}
//////////////////////////////////////////////
...
//				}
        ...
        CQueuedActionGenericDelayedIntervention *pDelayedIntervention;
        pDelayedIntervention = (CQueuedActionGenericDelayedIntervention *)m_GenericDelayedInterventions.GetLast();
        while(pDelayedIntervention != NULL)
        {
            ra.OnGenericDelayedApproval(pDelayedIntervention->GetActionID(), pDelayedIntervention->GetReportingID());
            delete pDelayedIntervention;
            pDelayedIntervention = (CQueuedActionGenericDelayedIntervention *)m_GenericDelayedInterventions.GetLast();
        }
   ...
   // Recalled Items
   lNotForSaleItems = 0; //SSCOADK-6985
   // Beats me.
...
SMStateBase *SMStateBase::OnTBNeedData(void)
{
    ...
    bool bReturn = GetDataNeededViewFromTB(sDescription, 
                                           sView,
                                           sINI,
                                           sSection);
    if(!bReturn)
    {
        trace(L4, _T("GetDataNeededViewFromTB failed"), rc);
    ...
    trace (L6, _T("-SMStateTBBase::OnTBNeedData()"));
    return GenerateRAPDataNeeded(sDescription, sView, sINI, sSection);
}

#endif //_CPPUNIT

bool SMStateBase::GetDataNeededViewFromTB(CString &csDescription, 
                                          CString &csView,
                                          CString &csIniFile,
                                          CString &csSection)
{
    ...
    long rc = tb.GetDataNeededView(&ViewInfo);
    ProcessTBReturn(rc);
    if(rc!=TB_SUCCESS)
    {
    ...
    {
        csDescription = ViewInfo.szDescription;
        try
        {
            delete [] ViewInfo.szDescription;
            ViewInfo.szDescription = NULL;
        }
        catch(...)
        {
            trace(L4,_T("Exception while deleting ViewInfo.szDescription"));
        }
    }
    
    if(ViewInfo.szView)     //String format DataNeeded parameters
    {
        csView = ViewInfo.szView;
        try
        {
            delete [] ViewInfo.szView;
            ViewInfo.szView = NULL;
        }
        catch(...)
        {
            trace(L4, _T("Exception while deleting ViewInfo.szView"));
        }
    }
    if(ViewInfo.szIniFile)
    {
        csIniFile = ViewInfo.szIniFile;
        try
        {
            delete [] ViewInfo.szIniFile;
            ViewInfo.szIniFile = NULL;
        }
        catch(...)
        {
            trace(L4, _T("Exception while deleting ViewInfo.szIniFile"));
        }
    }
    if(ViewInfo.szSection)
    {
        csSection = ViewInfo.szSection;
        try
        {
            delete [] ViewInfo.szSection;
            ViewInfo.szSection = NULL;
        }
        catch(...)
        {
            trace(L4, _T("Exception while deleting ViewInfo.szSection"));
        }
    }
    return bReturn;
}
...
SMStateBase *SMStateBase::OnTBGenericDelayedIntervention(void)
{
    trace (L6, _T("+SMStateTBBase::OnTBGenericDelayedIntervention()"));
    CString sDescription, sView, sINI, sViewString, sSection, csStateName;
...
SMStateBase *SMStateBase::HandleOutOfSyncAssist()
{
  ...
  //+ tell rap
  CString csSecInfo;
  //TAR 441378 - added SCOT_LANGUAGE_PRIMARY to parameter of GetPSText
  CString csWork = ps.GetPSText( MSG_POS_TB_OUTOFSYNC_NEEDASSISTMODE, SCOT_LANGUAGE_PRIMARY);
  ra.OnNormalItem(_T("[highlight]") + csWork);
  // save the previous view
  m_PrevView = m_View;
  ...
  return createLevel1Message(
    csWork, //TAR 441378 - changed to csWork to make code shorter and more efficient
    PS_NOTEXT,
    PS_NOTEXT,
    0,
    -1,
...
SMStateBase * SMStateBase::HandleVoidedTender(void)
{
...
#endif
    //SSCOADK-1724+
    //we should only update nDMCashInDrawer if tendertype is cash
    if (nTenderType == TB_TENDER_CASH)
    {
    ...
    //SSCOADK-1724-
#ifndef _CPPUNIT
    if((co.TenderList[ps.m_languageUsed][GetButtonIndex(dTender.nTenderType)-1].bCashBackIfVoided))
    {
        nDMCashInDrawer -= lamountVoided;
    ...
    ps.EchoAmountPaidToReceipt(nTenderType, -lamountVoided);
...
//	setAnchorState(BES_CONFIRMVOID);
	...
	if((co.TenderList[ps.m_languageUsed][GetButtonIndex(dTender.nTenderType)-1].bCashBackIfVoided) && !ps.RemoteMode())
	{
		fTenderDeclined = true;
		m_bCancellingCashTender=true;
...
BOOL SMStateBase::TBValidateHeadCashier(LPCTSTR szCashierID, LPCTSTR szCashierPW)
{
    ...
    {
        CBase64EncodeDecode base64;
        csID = base64.GetBase64EncodeWithPrefix(csPrefixBase64, csID);
        csPW = base64.GetBase64EncodeWithPrefix(csPrefixBase64, csPW);
    }

	BOOL brc = tb.ValidateHeadCashier(csID, csPW);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//    Function Name :  TBPreferenceData
//    Parameter     :  csPreferenceData - Semi-colon separated command string
//    Return Value  :  SMStateBase*
//    Description   :  Handle preference request from the TB
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
SMStateBase *SMStateBase::TBPreferenceData(CString csPreferenceCommand)
{
    CString csCommand;
    CString csResponseString;
    CString csName;
    CString csValue;
    long lValue;
    CString csFormatString = _T("command=%s_Response;name=%s;value=%s;privatedata=true");
 
    if (csPreferenceCommand.Find(_T("command=")) != 0)
    {
        //Bad string format
        return STATE_NULL;
    }
    
    csCommand = ExtractString(csPreferenceCommand, _T("command"));
    csName = ExtractString(csPreferenceCommand, _T("name"));


    if (csCommand == _T("GetPreference"))
    {
        csValue = custInfo.GetPreference(csName);
        csResponseString.Format(csFormatString,csCommand,csName, csValue);
        SendTBCommand(TBCMD_CMDSTRING, csResponseString);
    }
    else if (csCommand == _T("SetPreference"))
    {
        csValue = ExtractString(csPreferenceCommand, _T("value"));
        custInfo.SetPreference(csName, csValue);
    }
    else if (csCommand == _T("GetDataString"))
    {
        csValue = custInfo.GetDataString(csName);
        csResponseString.Format(csFormatString,csCommand,csName, csValue);
        SendTBCommand(TBCMD_CMDSTRING, csResponseString);
    }
    else if (csCommand == _T("SetDataString"))
    {
        csValue = ExtractString(csPreferenceCommand,_T("value"));
        custInfo.SetDataString(csName, csValue);
    }
    else if (csCommand == _T("GetMetricLong"))
    {
        lValue = custInfo.GetMetricLong(csName);
        csValue.Format(_T("%d"), lValue);
        csResponseString.Format(csFormatString,csCommand,csName, csValue);
        SendTBCommand(TBCMD_CMDSTRING, csResponseString);
    }
    else if (csCommand == _T("SetMetricLong"))
    {
        csValue = ExtractString(csPreferenceCommand, _T("value"));
        lValue = _ttol(csValue);
        custInfo.SetMetricLong(csName, lValue);
    }
    else if (csCommand == _T("IncrementMetricLong"))
    {
        csValue = ExtractString(csPreferenceCommand, _T("value"));
        lValue = _ttol(csValue);
        custInfo.IncrementMetricLong(csName, lValue);
    }


    return STATE_NULL;
}
//-SR815 - Personalizaiton

//+SR797
SMStateBase *SMStateBase::TBTenderSelected(TBTENDERTYPE nTenderType)
{
	trace(L0, _T("SMStateBase::TBTenderSelected"));
    nTenderButtonIndex = GetButtonIndex(nTenderType);
	if(!co.fStateTenderStartedMessage || IsTenderStarted())
		return STATE_NULL;

	m_nTenderSelected = nTenderType; //SSCOP-3887
	rc = tb.TenderSelected(nTenderType);
	ProcessTBReturn(rc);
	RETURNSTATE(InProgress);
}
bool SMStateBase::IsTenderStarted()
{
	return m_bIsTenderStarted;
}
void SMStateBase::SetTenderStarted(bool bTenderStarted)
{
	if (!bTenderStarted)
	{
		m_nTenderSelected = TB_TENDER_NONE;
	}
	m_bIsTenderStarted = bTenderStarted;
}
//-SR797
#endif //_CPPUNIT
