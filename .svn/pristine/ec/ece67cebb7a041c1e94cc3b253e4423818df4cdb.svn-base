...
return 0;
   ...
   PSInterfaceList1::PSInterfaceList1(LPCTSTR szWName,
    ...
    long PSInterfaceList1::Add(LPCTSTR szDesc,const bool fHighLight)
    {
      ...
      // Check option to see if we need to make current item has larger font.
	  // Currently. item with modifiers like QTY, weight will have more than
	  // one line with large font. Linked item will also have large font. 
	  if (co.fStateLargeFontOnCurrentItem)
      {
        sEle.nItemLength = 0;
        if(m_nTwoLine != 0)
          sEle.nItemLength = m_nTwoLine;
        if(bStrikethru == 1)
          sEle.fShow = PS_SF_STRIKETHRU ;
	  }
	  else
        sEle.nItemLength = -1;
...
return 0;
							   ...
							   HBITMAP hBmp)
    {
      ...
      // Check option to see if we need to make current item have larger font.
	  // Currently. item with modifiers like QTY, weight will have more than
	  // one line with large font. Linked item will also have large font. 
	  if (co.fStateLargeFontOnCurrentItem)
      {
...
return 0;
...

    {
      ...
      if (fVoid)
      {
        ...
        if (co.fStateLargeFontOnCurrentItem)
		{
	      CString szCode = csItemCode.GetAt(lSelect-1);
          //if (*szCode!=INFO_ITEM && *szCode!=VOID_ITEM && *szCode!=LINK_ITEM)
          if (*szCode!=INFO_ITEM && *szCode!=LINK_ITEM)
		  { 
            sEle.nItemLength = 0;
    	  }  
