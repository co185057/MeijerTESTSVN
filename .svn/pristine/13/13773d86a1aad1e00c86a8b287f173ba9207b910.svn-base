...
// RFC 445940:  Moved constructor from header file to here so that diagnostic
//              tracing could be added.
CDeviceSet::CDeviceSet(long nDeviceClass, long nOffset) :
     m_nDeviceClass( nDeviceClass ),
     m_nOffset( nOffset )
{
    CString csTrace;
    csTrace.Format(
            _T("CDeviceSet::CDeviceSet(%10s (%2d); nOffset=%d; this=%#X)"),
            GetDCString(m_nDeviceClass), m_nDeviceClass, m_nOffset, this);

    DCAP_API_TRACE((LPCTSTR)csTrace);   // RFC 445940 - Diagnostic tracing.
}

// RFC 445940:  Moved destructor from header file to here so that diagnostic
//              tracing could be added.
CDeviceSet::~CDeviceSet(void)
{
    ...
    {
        CGenericOposIF* pDevice = GetAt( nIndex );
        if ( pDevice != NULL )
        {
            pDevice->CloseDevice();
        }
...
void CDeviceSet::CreateDevices( CString  sKey, CDeviceManagerEx* pDMX,
                                    int nRetries, int nRetrySleep )
   {
    CString csTrace;
    csTrace.Format(
        _T("CDeviceSet::CreateDevices(%15s (%d); pDMX: %#X)"),
        GetDCString(m_nDeviceClass), m_nDeviceClass, pDMX); 
    DCAP_API_TRACE((LPCTSTR)csTrace);   // RFC 445940 - Diagnostic tracing.
...
CRemoteDeviceConfiguration::CRemoteDeviceConfiguration() :
         CDeviceConfiguration(REMOTE_ID_OFFSET, GetRemoteOpenRetries(), 
                              GetRemoteOpenSleep())  
{
    // RFC 445940 - Diagnostic tracing.
    DCAP_API_TRACE(
            _T("CRemoteDeviceConfiguration::CRemoteDeviceConfiguration"));
}
...
CDeviceConfiguration::CDeviceConfiguration(
   ...
   {
    // RFC 445940 - Diagnostic tracing.
    DCAP_API_TRACE(_T("CDeviceConfiguration::CDeviceConfiguration"));
   Close();
   return;
   }
...
CDeviceManagerEx::CDeviceManagerEx() : m_bShutdownInProgress(FALSE),
                                       m_bCreateDeviceFailure(false)
{
}

CDeviceManagerEx::~CDeviceManagerEx()
{
    // +TAR 453422
    // Make sure that we don't try to trace after the logs have been closed.
    CDCAPMethodTrace::SetDC(NULL);   
    // -TAR 453422
}
...
void CDeviceConfiguration::Open(
   ...
   {
    // RFC 445940 - Diagnostic tracing.
    DCAP_API_TRACE(_T("CDeviceConfiguration::Open"));
...
void CDeviceConfiguration::Close( 
   ...
   {
    // RFC 445940 - Diagnostic tracing.
    DCAP_API_TRACE(_T("CDeviceConfiguration::Close"));
...
void CDeviceManagerEx::Open(
   ...
   // RFC 445940:  Pass on DC object for our API tracing.
   CDCAPMethodTrace::SetDC(&m_cmDc);
   DCAP_API_TRACE(_T("CDeviceManagerEx::Open"));
...
void CDeviceManagerEx::Close(
   ...
   {
    // RFC 445940 - Diagnostic tracing.
    //              Use a pointer so we can control when the object is 
    //              destructed.
    CDCAPMethodTrace *pTracer = 
        new CDCAPMethodTrace(_T("CDeviceManagerEx::Close"));
   ...
   // Clear DC object to insure that late tracing doesn't occur when the app
   // is shutting down.
   CDCAPMethodTrace::SetDC(NULL);
...
long CDeviceManagerEx::Reload(long nDeviceClass)
{
    DCAP_API_TRACE(_T("CDeviceManagerEx::Reload"));
    long lRc = OPOS_E_ILLEGAL;

    assert(nDeviceClass != -1);
    if(nDeviceClass < 0)
    {
        m_cmDc.DCPrintf(DCTRACE_NORMAL, _T("Invalid device class [%d]!"),
                        nDeviceClass);
        return lRc;
    }

    m_cmDc.DCPrintf(DCTRACE_NORMAL, 
                    _T("Reloading Device class %d [%s]"),
                     nDeviceClass, GetDCString(nDeviceClass));

    // If we failed to create one or more OPOS controls, then we need to 
    // resort to a complete shutdown and reload of the DMX components as 
    // before.  This can also result in a crash, but this scenario should be
    // much less likely to happen.
    //
    // Don't like this approach here, but we would be stuck otherwise.
    if(IsCreateDeviceFailure())
    {
        m_cmDc.DCPrintf(DCTRACE_NORMAL, 
                        _T("CreateOPOSControl failed. Doing full DMX reload!"));
        Close();
        Open();
    ...
    {
        // Reload all devices that are associated with this class.  
        // Find out how many devices we have.  
        // CDeviceConfigurationEx::GetDeviceCount is private, so we can't call
        // it directly.
        
        long nDeviceCount = m_cfgLocal.GetAt(nDeviceClass)->GetSize();
        m_cmDc.DCPrintf(DCTRACE_NORMAL, 
                        _T("Device class %d has %d devices"), nDeviceClass,
                        nDeviceCount);
        
        for(long i=0; i<nDeviceCount; i++)
        {
            // Note:  Did not use CDeviceManagerEx::GetDevice because that 
            //        method calls IsAvailable().  This will not succeed if 
            //        the device has not been loaded successfully.
            CGenericOposIF* pDevice = 
                m_cfgLocal.GetDevice(nDeviceClass,i);

            assert(pDevice != NULL);
            if(! pDevice)
            {
                m_cmDc.DCPrintf(DCTRACE_NORMAL, 
                    _T("ERROR:  Failed to get wrapper for class/id %d/%d!"),
                    nDeviceClass, i);   
                lRc = OPOS_E_ILLEGAL;
                continue;
            }

            lRc = pDevice->ReloadDevice();
            if(lRc != OPOS_SUCCESS)
            {
                break;
            }
    ...
    else // Not a DMX device.  In this case, reload SCOTDevMgr.ocx as before.
    {
        m_cmDc.DCPrintf(DCTRACE_NORMAL, 
              _T("DeviceClass %d is not a DMX device.  Reloading CDevMgr ..."),
              nDeviceClass);
        CDevMgr::Close();
        CDevMgr::Open();
        lRc = OPOS_SUCCESS;
    ...
    if(nDeviceClass >= DMCLASS_RECEIPT && nDeviceClass <= DMCLASS_PRINTER)
    {
        m_cfgLocal.LoadPrinterStations();
    }

    m_cmDc.DCPrintf(DCTRACE_NORMAL, _T("CDeviceManagerEx::Reload returns %d"),
                    lRc);
    return lRc;
}

// RFC 445940:  Note that the code in this method was once part of 
// CDeviceConfiguration::Open().  Broke out into a separate method so that 
// it could be referenced from >1 place.
void CDeviceConfiguration::LoadPrinterStations(void)
{
   DCAP_API_TRACE(_T("CDeviceManagerEx::LoadPrinterStations"));

   // This code sets the printer stations for single station printers.  
   for ( long nDeviceClass = DMCLASS_RECEIPT; 
      ...
      for (int nIndex = 0; nIndex < pDeviceSet->GetSize(); nIndex++ )
         {
         ...
         if ( pDevice != NULL
            ...
            if ( nDeviceClass != DMCLASS_PRINTER )
               ...
               {
			   	CString sProfile = pDevice->GetDeviceProfile();// RFQ 7477
               ...
               if ( pPrinter->GetCapSlpPresent() != FALSE  && (!sProfile.CompareNoCase(_T("ScotSlip")) ||  !sProfile.CompareNoCase(_T("RAPPOSPrinter")) || pDeviceSet->IsSamePrinterReceiptSlip())  )//BAM
...
bool CDeviceManagerEx::UseLegacyStartupErrHandling(void)
{
    CString csKey = DMX_KEY_ROOT; 
    HKEY hKey = NULL;

    bool bResult = false;
    LONG nRc = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, csKey, 0, KEY_READ, &hKey);
    if(nRc == ERROR_SUCCESS)
    {
        CString sLeg = ReadRegString( hKey, 
                                 _T("UseLegacyStartupErrorHandling") ).Left(1);
        if ( sLeg.CompareNoCase(_T("Y")) == 0 )
        {
            bResult = true;
        }
...
void CDeviceManagerEx::OnCreateDeviceFailure(void)
{
    m_bCreateDeviceFailure = true;
}

bool CDeviceManagerEx::IsCreateDeviceFailure(void) const 
{
    return m_bCreateDeviceFailure;
}
...
CGenericOposIF* CDeviceManagerEx::GetDevice( 
   ...
   if ( pDevice == NULL )
      ...
      ThrowDmException( OPOS_E_DISABLED );  
...
CWnd* CDeviceManagerEx::GetThreadWindow( 
   ...
   {
   // RFC 445940 - Diagnostic tracing.
   DCAP_API_TRACE(_T("CDeviceManagerEx::GetThreadWindow"));
