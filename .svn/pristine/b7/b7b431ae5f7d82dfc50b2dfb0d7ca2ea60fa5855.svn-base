...
{
    //+SSCO Mobile  
    trace(L6, _T("SMScanAndBagBase::Initialize, g_bInRecallMode= %d"),g_bInRecallMode);
    //+SSCO Mobile
    ps.DeleteAllLevels();
  ...
  // process weight on scale when weight is added from Attract screen
  if (co.GetAllowPLAAutoAdvance() && fSSWtFromAttract)
  {
	  ProcessWeightOnScale();
  }
  ...
  SMStateBase::m_nTBOutOfSyncError = 0;  //SSCOP-3827
  ...
  //SSCOB-2221/SSCOP-3550 start
  if(g_savedAnchorStateFromAttract == BES_PUTBAGONSCALE)
  {
     g_savedAnchorStateFromAttract = BES_START;//back to default value
	 // enable the sensormatic EAS deactivator because going into a transaction
     dm.EASDeactivatorEnable( EASEnable );
  }
  //SSCOB-2221/SSCOP-3550 end

  //+ Dynamic QuickPick
  // item was not sold, remove it from the tail of the list
  if ((!g_bHaveSoldFromQuickPick) && (g_bHaveQuickPick) && (g_csQuickPickItemSold.GetSize()))
  {
      g_csQuickPickItemSold.RemoveAt(g_csQuickPickItemSold.GetSize()-1);

  }
    g_bHaveQuickPick = false; 
  //- Dynamic QuickPick
  ...
  if ( co.IsAttendantModeOn() && fRAPIsInStoreMode && !storeMode() && !co.IsInAssistMenus() && !ps.RemoteMode() )
  {
      ...
      trace(L6, _T("+ra.OnExitStoreMode in SMScanAndBagBase::Initialize"));
      ra.OnExitStoreMode();
  ...
  //+SSCOADK-836
  if (nDMCashInserted>0 && !bIsInPreTenderState && lBalanceDue==0)	
  {
	  CREATE_AND_DISPENSE(TakeChange)(false);
  }
  else if (nDMCashInserted>0 && !bIsInPreTenderState && !co.IsInAssistMenus() )
  {
  //-SSCOADK-836
  ...
  // TAR 251195
  if (bQuietNeedSecApproval && !co.GetfAutoRunOn()) // SSCOP-159
  {
	  CREATE_AND_DISPENSE(SecUnExpectedIncrease)(BES_SCANANDBAG, BES_SCANANDBAG);
  }
  ...
  if (!co.IsInAssistMenus())  //sscop-1704
  {
  ...
  TellApplicationStateToSecurity(KEY_AC_SET_APP_STATE_BEGIN, _T("ScanAndBag") );  //SSCOADK-5451
  // BeginCodeWLDBSA11
  if(!co.IsInAssistMenus() && !fItemIsWaiting && !bPOSatmPerksPromotions) //if mPerks was entered after couple of items, SCO has to keep the scanner disabled until promotion prompts are acknowledged
  {
  ...
  {
      if ((custInfo.GetDataString(_T("LangSelectionMethod")) != _T("Button")) &&
  ...
  {   
	...
	//TAR 279197+
	 ...
	 //TAR 279197-
        ...
        if (io.d.fDetails)            // if we have details to show, show them
		{
          if (io.nLoyaltyCardScanned)
		  {
			...
			else
			{
                ...
                //SSCOADK-4590 , Fix migrated from 5.0.1_Accumlative branch.
                //SSCOB-17375
                if(co.GetfOperationsAllowMultiPick())
                {
                    if(bAlreadyPlayedLCReminder == false)
                    {                        
          ...
          // restricted items and visual items require special treatment, this takes precedence over coupons
            ...
            else if(((io.lCoupCode==1) || (io.lCoupCode==3)) && !IsUsingCouponSensor())//TAR374217
			{
              if(io.xd.ExceptionCode==XAPPROVALTHRESHOLDS ||
                 (io.xd.ExceptionCode==XAPPROVALALWAYS && lExceptionItemsRequiringApproval > 0)) //TAR#438140
			  {
			     //ps.Message12(MSG_RESTWARN, false);
			     lMessage12Id = MSG_RESTWARN;  //fix screen blinking
      ...
      ps.ShowAmountDueAmountPaid(lBalanceDue, lTotalPaid);//sr804
   }
   else
   {
      ms_nNumberOfItems = 0;
	 ...
	 if ((io.d.fDetails) && (io.nLoyaltyCardScanned) && (co.fStateAllowLoyaltyCard))
	 {
		...
		else
		{
            ...
            //SSCOADK-4590 , Fix migrated from 5.0.1_Accumlative branch.
            //SSCOB-17375
            //Issue happens only when AllowMultiPicklist = Y
...
//    ps.ShowCMRewardInfoBox(lTrxTotalSavings, 0, CustomerMessage);
  ...
  else  // Tar 149414
  {
	...
	if (co.fTenderIsMotorizedMSR )
    {
        ...
        csScanBagTextArea = _T("MSG_CARDACCEPTED_SCANITEMS");
    }
	else
    {
	   lMessage12Id = MSG_CARDSWIPED_SCANITEMS;
       csScanBagTextArea = _T("MSG_CARDSWIPED_SCANITEMS");
    }
  }
  ...
  {
	/* TAR 346592 begin */
	if (TBIsAllowed(TB_FT_SUSPEND) && isAnySoldVoidedItems() & !bSuspendRequested){//TB allows Suspend Button to be enabled so App can too 
  ...
  CLaneStateData laneStateData(m_pAppPSX);
  laneStateData.SendNextGenUIData(_T("NextGenUICardMessageScanBag"),(CString)csScanBagTextArea);
  //+tar 275064
  if( IsMINIHardware() )
  {
    if( lTotalSale < 0 )
...
// <-- Change Language During Transaction
  ...
  if (co.IsInAssistMenus())
  {
      //g_bValidHeadCashier = bInAssistMenuValidHeadCashier; //SR819: SSCOP - 366 & 368
  ...
  if ((bAlreadyInSnB == false) || (bIsSysMsgScreenState == true)) 
  {
      m_pAppPSX->SetTransactionVariable( _T("NextGenData"), COleVariant(_T("ScanAndBag")) );
	  ps.ShowCMFrame();
...
SMStateBase *SMScanAndBagBase::PSButton1(void)
{
    ...
    {
        CLaneStateData laneStateData(m_pAppPSX);
        laneStateData.SendNextGenUIData( _T("IsQuickPick"), false);
        #ifndef _CPPUNIT
		if (co.GetfOperationsAllowMultiPick())
		{
        ...
        SMLookUpItemBase::m_HKStatus = IPSProcedures::HK_NORMAL;
        SMLookUpItemBase::m_bShiftOn = false;
        SMLookUpItemBase::m_bAltGrOn = false;
        #endif //CPPUNIT
        if (co.IsPicklistAssistEnable())
        {
            m_bIsPLALookUpTransition = true;
...
//////////////////////////////////////
// PSButton6 - "Own Bag" button - SR880
//////////////////////////////////////
SMStateBase  *SMScanAndBagBase::PSButton6()
{
	//+SR078 Send Clear Allow Late Bagging message to SM
	if(g_bAllowLateBagging)
	{
	...
	//-SR078
	trace(L6, _T("+SMScanAndBagBase: Own Bag bttn pressed"));
	  TellApplicationStateToSecurity(KEY_AC_SET_APP_STATE_END, _T("ScanAndBag"));
...
SMStateBase *SMScanAndBagBase::DMScale(void)
{
  ...
  if (co.GetAllowPLAAutoAdvance() && !m_bWaitForPicklist)
  {
	  ProcessWeightOnScale();
  }
...
void SMScanAndBagBase::ProcessWeightOnScale(void)
{
  trace(L6,_T("+SMScanAndBagBase::ProcessWeightOnScale"));
...
void SMScanAndBagBase::InitializeBasedOnOption(void)
{
	 ...
	 if (co.fOperationsDisplayQuickPickItems)
	 {
        ...
        if ( (_ttoi(custInfo.GetOption(_T("PersonalizedPicklistItems"))) > 0) && (_ttoi(custInfo.GetOption(_T("DynamicQuickPickItems"))) > 0) )
        {
            csPicklistPref = custInfo.GetPreference(_T("PicklistFavorites"));
            trace(L6, _T("PersonalizeItems = %s"), csPicklistPref);

            if (/*csPicklistPref.GetLength() && */(g_csPreviousPersonalPicklist != csPicklistPref))
            {
               ps.LoadPersonalizedQuickPickList(csPicklistPref);
               g_csPreviousPersonalPicklist = csPicklistPref;
	 ...
	 if(!co.fStateAllowLookUpItem)
	 {
		 ...
		 ps.SetButtonText(_T("CMButton1LookUpIn3ButtonRow"), PS_NOTEXT);
	 }
	 if(!co.fStateAllowKeyInCode)
	 {
		 ps.SetButtonText(_T("CMButton2KeyInCode"), PS_NOTEXT);
		 ps.SetButtonText(_T("CMButton2KeyInCodeIn3ButtonRow"), PS_NOTEXT);
	 }
...
SMStateBase* SMScanAndBagBase::DMTakeawayBagAreaBackup(bool isBackedUp)
{
	...
	//-SR078
    SMStateBase* baseNextState = STATE(State)::DMTakeawayBagAreaBackup(isBackedUp);
    if (isBackedUp)
    {
        RETURNSTATE(SecBagAreaBackup);
    }
...
SMStateBase* SMScanAndBagBase::OnWtIncreaseNotAllowed()
{
	...
	if(g_bAllowLateBagging)
	{
		CREATE_AND_DISPENSE(SecMisMatchWeight)(BES_SCANANDBAG, getNextStateForGoodItem());
	}
...
SMStateBase *SMScanAndBagBase::OnMatchedWt()
{
	if(g_bAllowLateBagging)
	{
		trace(L2, _T("SMScanAndBagBase::OnMatchedWt, calling base function"));
		return STATE(State)::OnMatchedWt();
	}
...
void SMScanAndBagBase::ChangeLanguage(SCOT_LANGUAGE_TYPE langNew)
{
	...
	ps.SetCMReceiptToCurrentLanguage();


	// Get the language name
	CString csLanguageName = co.Getlanguage(langNew).csName;
	if(_istdigit(csLanguageName[0])) //if its a msg ID
	{
		int iMsgID = _ttoi(co.Getlanguage(langNew).csName);
...
SMStateBase *SMScanAndBagBase::OnLookupComplete (long byteCount)
{
    ...
    trace(L6, _T("+SMScanAndBagBase::OnLookupComplete byteCount = %d"), byteCount);
    if (byteCount >= 0) //Lookup completed
    {
        ...
        */
        ...
        InitializeBasedOnOption();  //SSCOP-1292 - QuickPick items need to be refreshed
        if ((custInfo.GetOption(_T("PersonalizeLanguage")) == "Y") &&
			(scotLangTemp != GetPreferredLanguage()))
        {
    ...
    trace(L6, _T("-SMScanAndBagBase::OnLookupComplete"));
    return retState;
}
...
{
    trace(L6,_T("PSListLookup csItemInfo[%s]"), csItemInfo);
    CLaneStateData laneStateData(m_pAppPSX);
...
#endif //_CPPUNIT
		if (VerifyQtyItem(csItemSent) && !g_bAllowLateBagging )     
		{
			io.Reset();
	...
	else if (csSubCategory.CompareNoCase(_T("True")) == 0) //subcategory item
	{
        ...
        {
            csItemSent = _T("HIDDENCATEGORY_SUBCATEGORY") + csItemSent;
        }	

        trace(L6,_T("PSListLookup subcategory item[%s][%s]"), csItemSent, csDesc);
...
#ifndef _CPPUNIT
// +TAR 421737
long SMScanAndBagBase::DMSayPhrase(LPCTSTR sPhrase)
{
	long rc = 0;
	if (!co.IsInAssistMenus())
	{
...
void SMScanAndBagBase::TBGetItemDetails(ItemDetails &iDetails) 
{
    ...
    STATE(State)::TBGetItemDetails(iDetails);
...
//TAR407959-
//////////////////////////////////////
//PSCmdButton - Search Button - SR841
//////////////////////////////////////
SMStateBase *SMScanAndBagBase::PSCmdButton(CString csContext, CString csControl, CString csCommand)
{

    trace(L6,_T("+SMScanAndBagBase::PSCmdButton(%s, %s, %s)"), csContext, csControl, csCommand);
	//+SR078 Send Clear Allow Late Bagging message to SM
	if(g_bAllowLateBagging)
	{
	  CKeyValueParmList parmList;
	...
	//-SR078    
    SMStateBase *retState = STATE_NULL;
    if (csControl.CompareNoCase(_T("CmdBtn_ScanAndBagSearch")) == 0)
    {
		...
		g_csStateBeforeLookUp = _T("SMScanAndBag"); //SR841
...
SMStateBase *SMScanAndBagBase::OnPLAHandInView()
{
    CREATE_AND_DISPENSE(PLAHandInView)(BES_SCANANDBAG);
}

SMStateBase *SMScanAndBagBase::CUParse(MessageElement *me)
{/*
    int nEvent = me->cuInfo.Msg;
    CString csStateName;
    SMStateBase *retState = STATE_NULL;

    trace(L6,_T("+SMScanAndBagBase::CUParse"));
    switch(nEvent)
    {
    ...
    trace(L7,_T("-SMScanAndBagBase::CUParse"));
  return retState;*/
    return STATE(State)::CUParse(me);
}

SMStateBase *SMScanAndBagBase::OnMobileTransactionStart(CString csTransactionInfo)
{
    SMStateBase *retState = STATE_NULL;

    bool bIsQBItemList = (csTransactionInfo.Find(QBFMT_001) == 0);
    if ( bIsQBItemList && recallList.LoadQBBarcodes(csTransactionInfo) )
    {
    ...
    {
        retState = GenerateRAPDataNeeded(ps.GetPSText(TTL_CUSTOMER_AUDIT, SCOT_LANGUAGE_PRIMARY), _T(""), co.GetcsDataEntryINIFile(), _T("MobileTransactionAudit"));
    }
    return retState;
}

SMStateBase *SMScanAndBagBase::SASecMgrParse( const MessageElement *me)
{

    int nEvent = me->secmgrInfo.nEvt;
    trace(L6, _T("SMScanAndBagBase::SASecMgrParse nEvent = %d"), nEvent);
    LPTSTR szParms;
    ...
    case SmOnSecurityControl:
        if (csParms.Find(KEY_OSC_OPERATION_GOTO_RECALLINGITEMS) > -1)
        {
            trace(L6, _T("KEY_OSC_OPERATION_GOTO_RECALLINGITEMS is received"));
...
//+SSCOB-18879
bool SMScanAndBagBase::isBarcodeValidOperatorPassword(const CString csItemCode, CString *pcsOperatorID)
{
    //+SR078 Send Clear Allow Late Bagging message to SM
	
    trace(L6,_T(" isBarcodeValidOperatorPassword in ScannandBagBase..."));
    ...
    return STATE(State)::isBarcodeValidOperatorPassword(csItemCode, pcsOperatorID);
