...
#include "SMAttract.h"
#include "SMInProgress.h"		//SSCOADK-493
#include "SMFinish.h"
...
SMStateBase  *SMRAPDataNeededBase::Initialize(void)
{
  ...
  //SSCOADK-493+
  if( m_View == DATANEEDED_NOVIEW)
  {
    m_View = m_origView;
  }
  if (!g_bTBNeedMoreData && m_View == DATANEEDED_GOTOINPROGRESS)
  {
      trace(L6, _T("-SMRAPDataNeeded::Initialize, go to Inprogress"));  
  ...
  if(dm.getTriColorLightColor() != DM_TRICOLORLIGHT_GREEN) 
  {
...
//	  DMSayPhrase(WAITFORASSISTANCE);
  ...
  else if (g_csDataNeededHandler == _T("SecureCam") ) // make to to call enable AM for SmartAssist
  {
...
void SMRAPDataNeededBase::showScreen(bool passwordVerified)
{
  ...
  if (!csWork.GetLength())
  {
	  if(m_View == DATANEEDED_INVALIDBARCODE) //SR879
	  {
		csWork = ps.GetPSText(m_ViewText);
	  }
	  else
	  {
...
SMStateBase *SMRAPDataNeededBase::HandleRAPData()
   {
	...
	switch (m_View)
      ...
      case DATANEEDED_UNEXPECTEDDECREASE:
        trace(L6, _T("recevied more than one OnRequestApproval for security event while SSCO has already moved forward from security state to DataNeeded, just ignore "));
      ...
      default:
         ...
         pNextState = STATE(OperatorPasswordState)::HandleRAPData();
      }
...
void SMRAPDataNeededBase::SetupViewData()
   ...
   switch (m_View)
     ...
     case DATANEEDED_ASSISTEDTENDER:    // Tender requiring RAP assistance.
	  ...
	  //+SR879
	  case DATANEEDED_INVALIDBARCODE:	
		m_sView         = _T("InvalidBarcode");
		m_sViewName		= ps.GetPSText(MSG_INVALIDCODE, SCOT_LANGUAGE_PRIMARY);
		m_ViewText      = MSG_WAITFORCASHIER;
		break;
	  //-SR879      
     default:
         ASSERT(!m_sView.IsEmpty());
...
#ifndef _CPPUNIT
//+SR879
SMStateBase *SMRAPDataNeededBase::HandleInvalidBarcode()
{
	trace(L6, _T("+SMRAPDataNeededBase::HandleInvalidBarcode"));

	if(m_sButton == _T("cApprove"))
	{
        ...
        {
            trace(L6,_T("Go to state %s saved in g_pRememberStateBeforeHelp"), getStateName(g_pRememberStateBeforeHelp));
            //set the global to NULL
            SMStateBase *tmpPtr = g_pRememberStateBeforeHelp;
...
SMStateBase *SMRAPDataNeededBase::OnRAPDataReceived(LPCTSTR sData)
{
    g_bFromRAP = true;
    return STATE(OperatorPasswordState)::OnRAPDataReceived(sData);
}

#endif // _CPPUNIT
SMStateBase *SMRAPDataNeededBase::SASecMgrParse( const MessageElement *me)
{

    int nEvent = me->secmgrInfo.nEvt;
    trace(L6, _T("SMRAPDataNeededBase::SASecMgrParse nEvent = %d"), nEvent);

    if(nEvent == SmOnSecurityExceptionCleared)
    {
        if(g_csDataNeededHandler == _T("SecureCam"))
        {
    ...
    else if(nEvent == SmItemPickFinish)
    {
        delete [] me->secmgrInfo.szParms;
        return STATE_NULL;
    }
...
void SMRAPDataNeededBase::OverrideSecurityException(LPCTSTR lpszAuthorization, LPCTSTR lpszOperation)
{
    if( g_csDataNeededHandler == _T("SecureCam") && sm_pDataNeededEntry.ExtractString(_T("ID")) != _T("SMTrustLevelAlert"))
    {
        SendSmartAssistExceptionClear( lpszAuthorization, sm_pDataNeededEntry.ExtractString(_T("EXCEPTIONID")));

        sm_pDataNeededEntry.Clear();
        trace(L6, _T("-SMRAPDataNeededBase::OverrideSecurityException"));
    }
    else
    {
        STATE(OperatorPasswordState)::OverrideSecurityException(lpszAuthorization, lpszOperation);
    }
}

SMStateBase *SMRAPDataNeededBase::RAParse(MessageElement *me)
{

    trace(L4,_T("SMRAPDataNeededBase::RAParse: me->raInfo.msg = %d on thread 0x%08.8x"), me->raInfo.msg, GetCurrentThreadId());

    if (me->raInfo.msg == WMRA_REQUEST_APPROVAL)
    {
        trace(L5, _T("WMRA_REQUEST_APPROVAL is received, g_csSecureCamInterventionType = %s"), g_csSecureCamInterventionType);

        if(g_csDataNeededHandler == _T("SecureCam") && !bSecurityTrustLevelAlert && g_csSecureCamInterventionType == _T("SMART ASSIST TRUST LEVEL"))
        {
            ...
            ra.OnRemoteApproval(me->raInfo.handle,
    ...
    return STATE(OperatorPasswordState)::RAParse(me);
}

//SSCOADK-874+
SMStateBase* SMRAPDataNeededBase::HandleScannerData(const MessageElement* message)
{
   return STATE(OperatorPasswordState)::HandleScannerData( message);
}
//SSCOADK-874-
