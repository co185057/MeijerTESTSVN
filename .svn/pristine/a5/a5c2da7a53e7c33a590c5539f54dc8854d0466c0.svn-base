...
static bool bTABIsFlushing = false;
...
{	
	...
	else
	{
		...
		{
			if ((m_PrevState == BES_WAITFORAPPROVAL) && !g_bgVerifySig)
			{
...
void SMSecUnExpectedIncreaseBase::HandleShowScreenTimedOut()
{
	...
	if (!co.fOperationsSuppressAudioInUnexpectedIncrease)
	{
	   SMStateBase::DMSaySecurity(SEC_NONE, ps.GetSoundName(SEC_UNEXPECTEDITEMONBELT));	// play wave here so it doesn't replay on the key pad screen
	}
