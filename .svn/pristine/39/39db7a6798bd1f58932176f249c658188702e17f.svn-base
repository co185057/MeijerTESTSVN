...
//
// $Archive: /Fastlane/4.2/Development/Core/Source/SCOTAPP/dmx/printer.cpp $
//
// $Revision: 8 $
//
// $Date: 3/03/08 8:50a $
...
static const struct 
   ...
   */
   ...
   {
   PTR_SUE_COVER_OPEN,        DMCLASS_PRINTER,     PTR_SUE_COVER_OPEN,
   PTR_SUE_COVER_OK,          DMCLASS_PRINTER,     PTR_SUE_COVER_OK,
   PTR_SUE_JRN_EMPTY,         DMCLASS_JOURNAL,     PTR_SUE_JRN_EMPTY,
   PTR_SUE_JRN_NEAREMPTY,     DMCLASS_JOURNAL,     PTR_SUE_JRN_NEAREMPTY,
   PTR_SUE_JRN_PAPEROK,       DMCLASS_JOURNAL,     PTR_SUE_JRN_PAPEROK,
   PTR_SUE_REC_EMPTY,         DMCLASS_RECEIPT,     PTR_SUE_REC_EMPTY,
   PTR_SUE_REC_NEAREMPTY,     DMCLASS_RECEIPT,     PTR_SUE_REC_NEAREMPTY,
   PTR_SUE_REC_PAPEROK,       DMCLASS_RECEIPT,     PTR_SUE_REC_PAPEROK,
   PTR_SUE_SLP_EMPTY,         DMCLASS_SLIP,        PTR_SUE_SLP_EMPTY,
   PTR_SUE_SLP_NEAREMPTY,     DMCLASS_SLIP,        PTR_SUE_SLP_NEAREMPTY,
   PTR_SUE_SLP_PAPEROK,       DMCLASS_SLIP,        PTR_SUE_SLP_PAPEROK,
   };
...
CPrinterWrapper::CPrinterWrapper(
   ...
   m_bUseJournal( TRUE ),
   m_bUseSlip( TRUE ),
   m_bAsynchronousPrint(TRUE),
   m_bPrintSynchronously(false),
   m_bRetrying(false),
   m_nCurrentStation( 0 )
   {
                                  ...
                                  CString  sData )
{
    ...
    if (!m_bRetrying && GetAsyncMode() == FALSE && !m_bPrintSynchronously)
    {
        //Don't print yet,  just add to list of print requests for RetryOutput call later.
    ...
    else
    {
        ...
        result = TOPOSPOSPrinter<CGenericOposIF>::PrintNormal( nStation, sData );
        if ( result == OPOS_SUCCESS  && GetAsyncMode() == TRUE)
        {
            AddRequest( nStation, new CPrintTextRequest( sData ) );
        }
        else if (result != OPOS_SUCCESS && GetAsyncMode() == FALSE)
        {
            long resultCode = TOPOSPOSPrinter<CGenericOposIF>::GetResultCodeExtended();
            OnOposErrorEvent(result, resultCode);
        }
...
long CPrinterWrapper::PrintBarCode(
   ...
   { 
       SetFreezeEvents( TRUE );
       result = TOPOSPOSPrinter<CGenericOposIF>::PrintBarCode(
                                  ...
                                  long     nAlign )
{
    ...
    { 
        SetFreezeEvents( TRUE );
        result = TOPOSPOSPrinter<CGenericOposIF>::PrintBitmap( nStation, sFileName, nWidth, nAlign );
        if ( result == OPOS_SUCCESS && GetAsyncMode() == TRUE)
        {
            AddRequest( nStation, new CPrintBitmapRequest( sFileName, nWidth, nAlign ) );
        }
...
long CPrinterWrapper::SaveSetLineCharRequest(long lStation, int nLineChars)
{
    if ( GetAsyncMode() )
    {
        //set property immediately
        SetRecLineChars(nLineChars);
    }
    AddRequest(PTR_S_RECEIPT, new CSetRecLineCharRequest(nLineChars));
    return OPOS_SUCCESS;
}
...
void CPrinterWrapper::ReadRegistry( 
   ...
   m_nPaperCutLinesToAdvance = _ttoi( ReadRegString( hKey, _T("PaperCutLinesToAdvance")) );
    ...
    CString sSyncValue = ReadRegString( hKey, _T("SynchronousPrint") );
    sSyncValue.MakeUpper();
    m_bAsynchronousPrint = (sSyncValue.Left( 1 ) == _T("Y")) ? FALSE :TRUE;
#endif
   return;
   }
										...
										long  nStatus )
{
	try{
        ...
        if(! ( nIndex < sizeof( g_mapOposStatus ) / sizeof( g_mapOposStatus[0])) )
        {
            trace(L0, _T("Could not fire event for status event:%d"), nStatus);
            return;
        }
        ...
        if(nDeviceClass == DMCLASS_PRINTER)
        {
            trace(L0, _T("Could not fire event for printer status event:%d (it did not get mapped to a station)"), nStatus);
            return;
        }
	...
	// RFQ 7477
		...
		{
            trace(L0, _T("Could not fire event for devclass:%d, status event:%d, nIndex:%d because the station has not been created (yet?)"),
                nDeviceClass ,nStatus, nIndex);
		}
	}
	catch(...)
	{
...
void CPrinterWrapper::OnOposErrorEvent(
   ...
   if (nResult == OPOS_E_EXTENDED)
		nStatus = nRcEx;
   else if ( nResult == OPOS_E_FAILURE && nRcEx != 0)
		nStatus = nRcEx;
	else
		nStatus = nResult;
   ...
   trace(L6, _T("DMX:OnOposErrorEvent??? PTR-")
      _T("%d (%d,%d) Posting Error event"), nDeviceId, nResult, nRcEx );
...
long CPrinterWrapper::RetryOutput(
   ...
   m_bRetrying = false;
   SetAsyncMode( m_bAsynchronousPrint );
   return lResult;
   }

long CPrinterWrapper::PrintSynchronous(long nStation )
{
    m_bPrintSynchronously = true;
    SetAsyncMode( FALSE );
    long lResult = OPOS_SUCCESS;
    
    CPrintStation* pPrinter = m_aStations[ GetDeviceClass( nStation ) ];
    POSITION pos = m_listOfRequests.GetHeadPosition();
    for (int i = 0; i < m_listOfRequests.GetCount(); i++)
    {
        CPrintMapElement* pElement = m_listOfRequests.GetNext(pos);
        ASSERT( pElement->m_nStation  == nStation );
        long nPrintStation = pElement->m_nStation;
        
        lResult = pElement->Process( *pPrinter );
        if ( lResult != OPOS_SUCCESS )
            break;
    }
    SetAsyncMode( m_bAsynchronousPrint );
...
long CPrinterWrapper::DoOneTimeInitialization(
   ...
   DirectIO(NCRDIO_PTR_SET_BARCODE_WIDTH, &nBarCodeWidth, &bstrUnused); 
   ...
   if (m_nDefaultLineChars > 0)	  
   {
        ...
        int nRecLineChar = GetRecLineChars();
        if (m_nDefaultLineChars != nRecLineChar)   //Set was not successfully,  bad value
        {
            m_nDefaultLineChars = nRecLineChar;
            trace(L6, _T("Unable to set recLineChars to registry value,  retaining default printer setting %d"), m_nDefaultLineChars);
        }
   }
   else
   {
       m_nDefaultLineChars = GetRecLineChars();
       trace(L6, _T("Default RecLineChars not found in registry.  Remembering print initial setting %d"), m_nDefaultLineChars);
   }
