...
VCONTROLOBJECTLib::IFormPtr SMState::m_pVForm;// SR875
...
SMStateBase  *SMRequestSigBase::Initialize(void)
{
  ...
  // ncr_barn - 01/27/2000
    ...
    else if (gpSCOTSigCap->IsOnScreenCapture())
    {
        DMSayPhrase(ONSCREENSIGNSIGNATURE) ;
    }
	else
	{
	  ...
	  {	  
        /*
          ...
          */
		  ...
		  SysFreeString(pData); // cs185088 mem leak 2012.09.11
          RETURNSTATE(AcknowledgeSig)
      }
      else  // if do not verify, print receipt and take card
      {
              //Send signature data to TB
      if(co.fOperationsSendSignatureDataToTB && !IsVerifoneConfigured())// SR875
