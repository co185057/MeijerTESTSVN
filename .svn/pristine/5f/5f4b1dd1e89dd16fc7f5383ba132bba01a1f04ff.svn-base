...
namespace RapNet
{
	...
	public class ConnectThread : IDisposable
	{
        ...
        protected virtual void GenerateConnectionString(LanePsx lane, StringBuilder connectionString)
        {
            ...
            connectionString.Append(ConfigDataRapNet.IsMobileUI ? "1;" : "0;");            
            ...
            if (ConfigDataRapNet.GetConfigData().EnableRAPLite)
            {
                ...
                if (ConfigDataRapNet.IsProxyServer)
                {
                    connectionString.Append("XRAPLite;");
                }
                else
                {
                    connectionString.Append(ConfigDataRapNet.IsMobileUI ? "HHRAPLite;" : "Desktop RAP;");
                }
            }
            else 
            {
                ...
                connectionString.Append("DeviceType=");
                connectionString.Append(ConfigDataRapNet.IsMobileUI ? "Mobile RAP;" : "Desktop RAP;");
            }
        }
        ...
        protected virtual void HandleResponseString(LanePsx lane, String responseString)
        {
            ...
            string str = ConfigDataRapNet.ExtractRapEventData("acquire", responseString);
            if (str != null)
            {
                lane.DisableContextRemote = Convert.ToBoolean(str);                
            }
            str = ConfigDataRapNet.ExtractRapEventData("encrypt-scan-data", responseString);
            if (str != null)
            {
                ScannerGeneric.EncryptScanData = Convert.ToBoolean(str);
            }            
            lane.RefreshContext();

            if (ConfigDataRapNet.GetConfigData().EnableRAPLite
                && (Intervention.Type.Immediate == lane.LaneIntervention.type) && !ConfigDataRapNet.IsProxyServer)
            {
            ...
            Remote.SendMessage(Remote.LaunchpadApplication, "operation=set-active-host;host=" + ConfigDataRapNet.ExtractRapEventData("host", lane.HostName));                
        }
                ...
                for (int n = 0; n < lanePsxArray.Length; n++)
                {
                    ...
                    if (null != lane && lane.RemoteConnection.Equals("invalid"))
                    {
                        try
                        {
                            ...
                            hostName = ConfigDataRapNet.ExtractRapEventData("host", lane.HostName);
                            service = ConfigDataRapNet.ExtractRapEventData("service", lane.HostName);
                            if (service == null)
                            {
