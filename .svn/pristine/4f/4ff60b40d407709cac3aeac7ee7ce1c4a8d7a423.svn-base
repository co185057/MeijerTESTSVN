...
SMStateBase  *SMSecMisMatchWeightBase::Initialize(void)
{
	...
	if (g_bOnItemOK)
	{
  	   trace(L5, _T("g_bOnItemOK is already true, going to BES_SCANANDBAG"));
...
void SMSecMisMatchWeightBase::showScreen(bool passwordVerified)
{
    ...
    {  // Display Wait for Approval....   
		...
		else if (!co.GetfOperationsSuppressAudioInMismatch())				
        {
	...
	{
		if(co.GetfOperationsAllowContainerBtnForMismatching() && !secEventParms.m_bMultiPackItem)
		{
   			nBtn1 = BTT_BAGBOX;
			v = VARIANT_TRUE;
...
SMStateBase *SMSecMisMatchWeightBase::stateAfterPasswordVerify()
{
	...
	if ( !m_bScanOnAttractAtQuietMode ) 
    {
	    ...
	    // items just clear
		//+TAR424824 -- let security decide if weight update is appropriate
        /*if (io.cScanType != 0)
	    {
...
void SMSecMisMatchWeightBase::HandleShowScreenTimedOut()
{
	...
	if (!co.fOperationsSuppressAudioInMismatch)				
    {
		...
		STATE(SecOperatorPWState)::DMSaySecurity(SEC_NONE, ps.GetSoundName(ASSISTANCENEEDED));
