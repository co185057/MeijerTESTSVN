...
namespace RapNet
{
	...
	public class SharedPsx : IDisposable
	{
		...
		{
            this.psx = psx;
            HandScannerAcquiredLane = string.Empty;
            ReceiptLogoStatus = MeijerLogoStatus.NotLoaded; //Start with not loaded status on RAP startup.
        }
        ...
        public virtual bool Initialize()
        {
            ...
            try
            {
                ...
                if (!LoadLanePsxArray(configData))
                {
                    return false;
                }
        ...
        public void PlaySound(string sound)
        {
            if (sound.Length > 0)
            {
                try
                {
                    psx.PlaySound(ConfigData.GetConfigData().AudioPath + "\\" + sound);
                }
                catch (PsxException caught)
                {
        ...
        public int CurrentExclusiveLane
        {
            get
            {
                if (ConfigData.MultiExclusiveLanes)
                {
                    ...
                    try
                    {
                        ...
                        if (assistLane > 0)
                        {
                            ...
                            if (null != lane)
                            {
                                ...
                                CmDataCapture.Capture(CmDataCapture.MaskError, string.Format("SharedPsx::AssistLane() - AssistLane changing to Lane= {0}    Hostname= {1}", lane.LaneNumber, lane.HostName));
                                lane.psx.GenerateEvent(lane.RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap,
                                    "operation=assist-granted");

                                if (ReceiptLogoStatus == MeijerLogoStatus.NotLoaded)
                                {
                                    CmDataCapture.Capture(CmDataCapture.MaskError, "Generating bitmap not loaded event to notify the lane.");
                                    //generate event to notify lane to load the bitmap onto the RAP printer
                                    lane.psx.GenerateEvent(lane.RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap, "operation=bitmap-notloaded");
                                }
                        ...
                        if (oldAssistLane > 0)
                        {
                            ...
                            CmDataCapture.Capture(CmDataCapture.MaskError, string.Format("SharedPsx::AssistLane() - Resetting the AssistLane Lane= {0}    Hostname= {1}", lane.LaneNumber, lane.HostName));
                            if (!ConfigData.IsMobileUI)
                            {
                                lane.psx.SetControlConfigProperty(assistModeButton, Psx.PropertyFlash, false);
                            }
		...
		{
			if ((ExclusiveLaneCount < 1 ) || !ConfigData.MultiExclusiveLanes || (findColumn < 1) || (findColumn > 2))
			{
				return 0;
			}
            lock (exclusiveLaneArray.SyncRoot)
        ...
        public bool SmallExclusive
        {
            get
            {
                return ConfigData.MultiExclusiveLanes && smallExclusive;
            }
        }
        ...
        public string OperatorId
        {
            ...
            set
            {
                ...
                text += (operatorId.Length > 0) ? opId : psx.GetString("SignedOffButtonText");
        ...
        /// </summary>
        public bool LoadLanePsxArray(ConfigData configData)
        {
            ...
            for( int n = 0; n < configData.LaneConfigList.Count; n++ )
            {
                ...
                lane = (host.Length > 0 && String.Compare(host, "host=", true) != 0) ?
                            ConfigData.LoadClass("LanePsx", new object[] { host, n + 1, this }) as LanePsx : null;	
                LanePsxArray[n] = lane;
                if (null != lane)
                {
                    if (!lane.Initialize())
        ...
        public virtual void TileLanes()
		{
			...
			for( int n = 0; n < lanePsxArray.Length; n++ )
			{
				...
				if( null != lanePsxArray[n] )
				{
                    ...
                    if (ConfigData.GetConfigData().EnableRAPLite && ConfigData.IsMobileUI)
                    {
        ...
        /// </summary>
        protected virtual void ShowTime(object state)
        {
            try
			{
        ...
        private bool StartScannerThread()
        {
            ...
            scanner = ConfigData.LoadClass("ScannerThread", new object[] { this, ConfigData.GetConfigData().EncryptScanData }) as ScannerThread;
        ...
        {
            if (ConfigData.GetConfigData().EnableRAPLite && !ConfigData.IsProxyServer)
            {
                RefreshLaneButtons();
            }
        ...
        public void WaitForResponse(int laneNumber, bool bFreezeRedraw)
        {
			...
			lock (lanePsxArray.SyncRoot)
			{
				...
				waitForResponseTimer = new Timer(new TimerCallback(WaitResponseTimeout), null, ConfigData.GetConfigData().LaneResponseTimeout, 0);
			}
        }
                ...
                if (e.EventName.Equals(Psx.EventChangeContext))
                {
                    ...
                    if (newState.Length > 0 && (RapState == null || -1 == RapState.GetType().Name.IndexOf(newState)))
                    {
                        try
                        {
                            ...
                            RapState = ConfigData.LoadClass(newState, new object[] { this, this.psx, true }) as State;
                        }
                        catch (Exception)
                        {
        ...
        /// </summary>
        protected internal ScannerThread scanner = null;
		...
		private bool StartBiometric()
		{
			...
			if (configData.BiometricDeviceConfigured)
			{
				...
				biometric = ConfigData.LoadClass("Biometric", new object[]{this}) as Biometric;			
				if (null != Biometric) 
				{				
					Biometric.BiometricEvent += new BiometricDelegate(Biometric_BiometricEvent);
				}									
        ...
        public void RefreshLaneButtons()
        {
            ...
            do // dummy loop
            {
                if (interventionLanesIndexArray.Count >= 1)
                {
                    ...
                    if (ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite >= interventionLanesIndexArray.Count)
                    {
                        displayLanesCount = interventionLanesIndexArray.Count;
                    }
                    else
                    {
                        displayLanesCount = ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite;
                    ...
                    if (ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite <= 5)
                    {
                        laneSizeColumn1.Height /= 5;
                    }
                    else if (ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite <= 8)
                    {
                        use2Columns = true;
                        laneSizeColumn1.Width /= 2;
                        laneSizeColumn1.Height /= 4;
                    ...
                    else if (ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite <= 10)
                    {
                        use2Columns = true;
                        laneSizeColumn1.Width /= 2;
                        laneSizeColumn1.Height /= 5;
                    ...
                    {
                        int lane_configuration;
                        if (ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite % 2 == 1)
                        {
                            lane_configuration = ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite + 1;
                        }
                        else
                        {
                            lane_configuration = ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite;
                        }
                        use2Columns = true;
                        laneSizeColumn1.Width /= 2;
                        laneSizeColumn1.Height /= lane_configuration / 2;
        ...
        /// </summary>
        protected TransactionCount trxCount = null;
        ...
        #region Meijer customization for Meijer Logo loading into RAP receipt printer device memory

        public MeijerLogoStatus ReceiptLogoStatus { get; private set; }

        public void UpdateMeijerLogoStatus(MeijerLogoStatus status)
        {
            ReceiptLogoStatus = status;
        }

        #endregion Meijer customization for Meijer Logo loading into RAP printer device memory

        #region Meijer customization for sharing scanner with lanes - Begin
        ...
        public string HandScannerAcquiredLane
        {
            get;
            private set;
        }

        public void AssignHandScannerToLane(string laneHostName)
        {
            CmDataCapture.Capture(CmDataCapture.MaskInfo, string.Format("SharedPsx::AssignHandScannerToLane(): Changing the hand scanner to lane from : {0} to {1}", HandScannerAcquiredLane, laneHostName));
            HandScannerAcquiredLane = laneHostName;
            this.scanner.ScannerEvent -= scanner_ScannerEvent; //Extra cautious, not to endup with multiple subscriptions
            this.scanner.ScannerEvent += scanner_ScannerEvent;
        }

        void scanner_ScannerEvent(object sender, ScannerEventArgs e)
        {
            if ((!string.IsNullOrEmpty(HandScannerAcquiredLane)) && (!(e.ScanDataLabel.StartsWith("TER", StringComparison.OrdinalIgnoreCase)))) //CRD124746
            {
                LanePsx ScannerLane = LanePsxArray.SingleOrDefault(lane => lane.HostName.Equals(HandScannerAcquiredLane, StringComparison.OrdinalIgnoreCase));
                if (null != ScannerLane && !(ExclusiveLane > 0))
                {
                    string param = ScannerThread.ScanDataToOperation("operation=scanner-data", e);
                    CmDataCapture.Capture(CmDataCapture.MaskError, string.Format("SharedPsx::State_ScannerEvent(): Passing the scanner event to Lane= {0}, Hostname= {1}", ScannerLane.LaneNumber, ScannerLane.HostName));
                    ScannerLane.psx.GenerateEvent(ScannerLane.RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap, param);
                    CmDataCapture.Capture(CmDataCapture.MaskInfo, "SharedPsx::State_ScannerEvent(): Sending scanner data with param: " + param);
                }
            }
            //this.scanner.ScannerEvent -= scanner_ScannerEvent;  // CRD196900: Only De-register the HHScanner by invoking release scanner
        }

        public void ReleaseHandScanner()
        {
            CmDataCapture.Capture(CmDataCapture.MaskError, "SharedPsx::ReleaseHandScanner(): Setting scanner acquired lane to Release Hand scanner");
            LanePsx ScannerLane = LanePsxArray.SingleOrDefault(lane => lane.HostName.Equals(HandScannerAcquiredLane, StringComparison.OrdinalIgnoreCase));
            if (null != ScannerLane)
            {
                CmDataCapture.Capture(CmDataCapture.MaskError, "SharedPsx::ReleaseHandScanner(): Deactivating scanner from " + HandScannerAcquiredLane);
                ScannerLane.psx.GenerateEvent(ScannerLane.RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap, "operation=scanner-deactivated"); //CRD149415
                HandScannerAcquiredLane = string.Empty;
            }
            this.scanner.ScannerEvent -= scanner_ScannerEvent;
        }
        ...
        {
            CmDataCapture.Capture(CmDataCapture.MaskError, "SharedPsx::ReleaseScanner(): Setting scanner acquired lane to Release scanner");
            this.scanner.ScannerEvent -= scanner_ScannerEvent;
        }

        public void AttachScanner(object sender, ScannerEventArgs e)
        {
            CmDataCapture.Capture(CmDataCapture.MaskError, "SharedPsx::AttachScanner(): Setting scanner acquired lane to attach scanner");
            this.scanner.ScannerEvent -= scanner_ScannerEvent;
            this.scanner.ScannerEvent += scanner_ScannerEvent;
        }
        ...
        public string GetExpectedEnterIDContext()
        {
            ...
            {
                if (ConfigData.GetConfigData().AllowHandKeyEMPLID)
                {                    
                    return ChangePwd ? "ScanIDOrKeyChangePWD": "ScanIDOrKey";                    
                }
                else
                {
                    return ChangePwd ? "ScanIDOnlyChangePWD" : "ScanIDOnly";                    
                }
            }
            else
            {
                return ConfigData.GetConfigData().AlphaNumericKeyboard ? "EnterAlphaNumericID" : "EnterID";                
            }
        }
        ...
        public string GetExpectedPasswordContext()
        {
            ...
            if (ConfigData.GetConfigData().AllowEmplIdSignOn)
            {
                return ConfigData.GetConfigData().AllowHandKeyEMPLID ? "EnterPasswordOrUseBiometricFromScanIDOrKey" : "EnterPasswordOrUseBiometricFromScanIDOnly";                
            }
            else
            {
                return ConfigData.GetConfigData().AlphaNumericKeyboard ? "EnterAlphaNumericPassword" : "EnterPassword";                
            }
        }
        ...
        {
            return ConfigData.GetConfigData().AllowEmplIdSignOn ? ConfigData.GetConfigData().AllowEmplIdSignOn ? "ScanIDOrKeyLeadthru" : "ScanIDOnlyLeadthru" : "EnterIDLeadthru";
        }
        ...
        {
           return ConfigData.GetConfigData().UseBiometrics? "EnterPasswordOrUseBiometricLeadthru": "EnterPasswordLeadthru";
        }

        // -RFQ 7477

        #endregion Meijer customization for sharing scanner with lanes - End

    }

    public enum MeijerLogoStatus { NotLoaded = 0, Loaded }
}
