...
SMStateBase *SMStateBase::PSParse(const MessageElement *me)
{
  ...
  { // these brackets help the auto-format
    ...
    case PS_ASSISTKEY:
        {
        // SASA: Prevent Assist mode key buffering  
...
SMStateBase *SMStateBase::PSConnectRemote( CString& strMachineName, CString& strRemoteContext )
{
	...
	{
        CString csCurrentStateName= this->GetRuntimeClass()->m_lpszClassName;  //430696

	    // TAR 269869 start
		//if (!ra.GetRequestEnableAssistMode()|| (csCurrentStateName.Find(_T("ThankYou")) != -1)) //430696
		if ( !fStartTrxInAM && (!ra.GetRequestEnableAssistMode()|| (csCurrentStateName.Find(_T("ThankYou")) != -1))) //SSCOB-257
		{
...
SMStateBase *SMStateBase::PSCmdList(CString csContext, CString csControl,  const long nIndex )
{
    ...
    trace(L6,_T("-SMStateBase::PSCmdList() Base implementation doing nothing"));
    return STATE_NULL;                // discard any input
}
...
long SMStateBase::PSInitialize(void)
{
  ...
  // copy SCUDMSGS file 
  _tcscpy(szFileName, SCUDMSGS _T(".DAT"));
  _tmakepath(szFile3,NULL, co.csConfigFilePath,SCUDMSGS, _T("DAT"));
  CopyFileFromServer(szFileName,csDestPath+szFileName,false);
  ...
  if (co.fStateDualLanguage)
  {
      ...
      for(int i=SCOT_LANGUAGE_PRIMARY; i<co.m_iNumberOfLanguagesSupported; i++)
      {
          //copy message text file for specific language
          _tcscpy(szFileName, (co.language[i].csMessageFile));
          CopyFileFromServer(szFileName,csDestPath+szFileName,false);
          //Copy SCOT User Defined messages for specific language
          csSCUDMsgs.Format(SCUDMSGFILE(i));
          _tcscpy(szFileName, csSCUDMsgs);
          CopyFileFromServer(szFileName,csDestPath+szFileName,false);
          //copy language specific tare description file
          _tcscpy(szFileName, (co.language[i].csTareFile));
          CopyFileFromServer(szFileName,csDestPath+szFileName,false);
  ...
  _tcscpy(szFileName, co.csDataTareFile + _T(".DAT"));

  _tmakepath(szFile5,NULL, co.csConfigFilePath,co.csDataTareFile,_T("DAT"));
  CopyFileFromServer(szFileName,csDestPath+szFileName,false);
 //TAR262985+
  ...
  if (fDoOptions)
  {
	  ...
	  return ps.InitializeConfigData(szFile4, szFile5);
  }
  ...
  long lReturn = ps.Initialize(
                ...
                szFile4,
                szFile5,
                co.nOperationsTareContainer1,
                co.nOperationsTareContainer2,
                co.fOperationsScaleMetric,
                co.fStateLiveVideo);
  if(lReturn < 0) 
  {
      return lReturn;
  ...
  // NewUI
  ReadOperatorLoginBarcodeMask(co.csOperationsOperatorBarcodeMask);  //FZ20020730
