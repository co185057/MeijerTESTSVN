...
//SR672+
#include "SMVoidItem.h"                 //sscop-2854
...
bool bFlagResetInInitialize = false;
...
SMStateBase  *SMSmAssistModeBase::Initialize()
{
    ...
    //+SR803    
    /*if( (lUnapprovedRestrictedItems == 1) && bStoreModeApprovals)//SSCOP-2336
    {
        // if it goes here with an age-restricted item, then automatically void the item
    ...
    //sscop-2854+
    if( lUnapprovedRestrictedItems == 1 && !bStoreModeGoBack && bStoreModeApprovals ) //SSCOP-4103
    {
  ...
  //-dp185016
   ...
   /*
   if((!(!bStoreModeGoBack && bStoreModeApprovals) && !ps.RemoteMode()) ||
      (ps.RemoteMode() && !isAnyApprovalRequired()))
   {
       bFlagResetInInitialize = true;
       ResetFlagsInAM(); //TAR446923
   }
   */
   ...
   if((!(!bStoreModeGoBack && bStoreModeApprovals) && !ps.RemoteMode()) ||
      (ps.RemoteMode() && !(isRestrictedItemApprovalRequired() || isCouponRatioApprovalRequired() || isVisualItemApprovalRequired() || (isSecurityApprovalRequired() && !g_bClearAllDelayedSecIntervention))))
	{
		// remote approval
		if (lNotForSaleItems)
    ...
    UpdateStateName(ps.GetPSText(MSG_ASSISTMODE, SCOT_LANGUAGE_PRIMARY));
	// -SSCOP-1703
   ...
   tb.SetSecurityLevel(nNewLevel);
	...
	if (ps.RemoteMode() && (fInEndorsementState == ENDORSE_WAITING))
	{
        ...
        // End NewUI
		//fInEndorsementState = ENDORSE_INPROGRESS;  //this is supposed to be set in the OnEndorseDocument()
...
void SMSmAssistModeBase::UnInitialize()
{
  ...
  //SSCOADK-5989 + set g_bCustomerLanguageChanged before returning to customer mode
...
trace(L6, _T("SMSmAssistModeBase::TimedOut"));
  ...
  if(co.GetfAutoRunAssistModeOn() && nAutoListIndex != -1)
  {
  ...
  if (fWaitingToExit)
  {
    ...
    if (getAnchorState() == BES_KEYINCODE)
      STATE(StoreModeState)::setAnchorState( BES_SCANANDBAG );
		//-dp185016
		...
		if (isAnyApprovalRequired())
		{
			...
			//SSCOADK-7293 CREATE_AND_DISPENSE(WaitForApproval)(false,false);
      bStoreModeApprovals = false; //SSCOADK-7293
      RETURNSTATE(SmAuthorization) //SSCOADK-7293
		}
...
SMStateBase  *SMSmAssistModeBase::PSButton1()// return to shopping mode
{
   ...
   // before returning to quad.
	...
	if(!g_bAllowRemoteBtn1 || fItemScanned)  //SSCOP-3184 (SSCOP-3139)
	{
		return STATE_NULL;
	}
  ...
  /*
   if((!(!bStoreModeGoBack && bStoreModeApprovals) && !ps.RemoteMode()) ||
      (ps.RemoteMode() && bFlagResetInInitialize))
  {
      ResetFlagsInAM(); //TAR446923
  }
  */
...
SMStateBase  *SMSmAssistModeBase::DMScanner()// scanned item
{
  if (!bIsTransactionStarted)  // TAR 241351
   ...
   //SSCOP-3184 (SSCOP-3139)+
  if (fWaitingToExit)
  {
	  fWaitingToExit = false;
	  return STATE(StoreModeState)::DMScanner();
  }
...
SMStateBase  *SMSmAssistModeBase::IssueChange(void)
{
  ...
  if (lChangeDue>0)
  {
    ...
    if (lChangeDue>0)
    {
      ...
      //FZ Error Handling
	  //SR672+
	  CString csTitle1, csWork, sSubModel;
	  
	  bool  lCoinStatus = false;
	  bool  lBillStatus = false;
      BillOrCoinErrorStatus(&lCoinStatus, &lBillStatus);
	  if (lCoinStatus)
			sSubModel = _T("Coin");
	  if (lBillStatus)
			sSubModel = _T("Bill");
	  ...
	  CString s1=dm.GetDeviceModelName(DMCLASS_CASHCHANGER, 0);
	  
	  csWork = DMGetDeviceErrorHTML(DMCLASS_CASHCHANGER, s1, sSubModel, EVENTID_ANY, lExtCode, 0, NULL, &fCheckHealth, nDeviceErrorContext, co.csPrimaryLanguageCode );
	  ...
	  if ( dm.m_csCashRecyclerModel.IsEmpty() ) 
	  {
		...
		if (csWork.Find(_T("1003")) != -1 ||  csWork.Find(_T("1004")) != -1 || csWork.Find(_T("1005")) != -1 )
		{
			...
			sParms.Format(_T("%s&%s&%s"), ps.FmtDollar(lChangeDue), ps.FmtDollar(lChangeDue), sHopperStatusLine );
		}
		else
		{
			long lcode = dm.GetResultCodeExtended(DMCLASS_CASHCHANGER, 0);
			sParms = Get563xCassetteParms(lcode,nDeviceErrorContext );
			if (sParms.IsEmpty())
  ...
  CString csSecInfo;
	...
	{
		csSecInfo.Format(ps.GetPSText(RA_SECVIOLATIONINFO, SCOT_LANGUAGE_PRIMARY),csTitle1 ,ps.GetPSText(BUTTON_RA_DEVICE_ERROR, SCOT_LANGUAGE_PRIMARY), _T(""), _T(""), _T(""), _T(""));
	}
...
//      ps.Button(IDC_BUTTON1, BTT_GOBACK, false);  //tar 205586
      ...
      {
      trace (L0, _T(" SMSmAssistModeBase::OnNeedMICR() - Application wants MICR data, but we are not is RemoteDeviceMode."));
      }
   trace (L6, _T("-SMSmAssistModeBase::OnNeedMICR()"));
   delete pMICRPrompt;
   return STATE_NULL;
...
SMStateBase *SMSmAssistModeBase::DMSlipPrinter(DWORD dwEvent)
   ...
   switch(dwEvent)
      ...
      case PTR_SUE_SLP_PAPEROK: //SR672
      ...
      case DM_PTR_REQUEST_COMPLETED :
         ...
         tb.EndorsementCompleted();
         break;
	  case PTR_SUE_SLP_EMPTY: //SR672
         trace (L4, _T("SMSmAssistModeBase::DMSlipPrinter() slip empty."));
         break;
	  case PTR_SUE_SLP_NEAREMPTY: //SR672
...
SMStateBase *SMSmAssistModeBase::PSAlphaKey(const long lKeystroke)
{
    ...
    STATE(StoreModeState)::PSAlphaKey(lKeystroke);
...
SMStateBase *SMSmAssistModeBase::PSKeystroke(const long wParam, const long lParam)
{
    ...
    SMStateBase* retState = STATE(StoreModeState)::PSKeystroke(wParam, lParam);
    return retState;
}
...
*/
   ...
   return STATE(StoreModeState)::OnRAPUnsolicitedDataReceived(sData);
   }
...
SMStateBase * SMSmAssistModeBase::TBEndOfTransaction(void)
{
    ...
    retState = STATE(StoreModeState)::TBEndOfTransaction();
...
SMStateBase* SMSmAssistModeBase::DMRemoteScannerConnect(bool bConnect)
{
    ...
    if (bConnect)
    {
        ...
        {
            bool bIsAttendantModeOn = co.IsAttendantModeOn();
            if (!bIsAttendantModeOn)
            {
...
//+TAR 446923
void SMSmAssistModeBase::ResetFlagsInAM(void)
{
...
SMStateBase *SMSmAssistModeBase::DepositCouponsIfNecessary()
{
