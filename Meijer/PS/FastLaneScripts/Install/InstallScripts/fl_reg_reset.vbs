...
'  SCOT Security Server and Report Server Registry Updates
 	.RegWrite scotIpath & "CoreSecurityServer\ServerName", savedsecserver
 	.Regwrite scotIpath & "CoreReportServer\ReportServerName", savedRptServer
        .Regwrite scotCpath & "SCOTAPP\ClientName", savedsecserver
 	.Regwrite scotCpath & "SCOTAPP\Reporting\ReportServer", savedRptServer
 	.Regwrite scotCorepath & "ObservedOptions\ServerName", savedsecserver
        .Regwrite scotCorepath & "ObservedOptions\ReportServerName", savedRptServer
'  ACS Specific parameters
        .Regwrite scotCpath & "SCOTTB\TerminalNumber", savedterminalno
...
If configfileFSO.DriveExists("c:\") Then
	If configfileFSO.FileExists("c:\install\" & "flsavedreg" & ".cfg") Then 
                configfilefound = "Y"
        else
                configfilefound = "N"
	end if
...
'****************GET security server
	...
	section= "config"
	key= "SecServer"
	savedSecServer = readini(file, section, key)

'****************GET report server
	file = "c:\install\" & "flsavedreg" & ".cfg"
	section= "config"
	key= "RptServer"
	savedRptServer = readini(file, section, key)
	
'****************GET Terminal Number
	file = "c:\install\" & "flsavedreg" & ".cfg"
	section= "config"
	key= "terminalnumber"
	savedterminalno = readini(file, section, key)
	
'****************GET TAB Flag
	file = "c:\install\" & "flsavedreg" & ".cfg"
	section= "config"
	key= "TABflag"
	savedTABFlag = readini(file, section, key)
