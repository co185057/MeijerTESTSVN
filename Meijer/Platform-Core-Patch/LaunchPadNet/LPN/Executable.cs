...
namespace LaunchPadNet
{
	...
	public class Executable 
	{
		...
		/// <summary>
        /// Get/Set whether process has exited
        /// </summary>
        public bool HasProcessExited
        {
            ...
            private set
            {
                hasProcessExited = value;
            }
        }

        /// <summary>
        /// Method for monitoring when a process exits. Should be in a thread.
        /// </summary>
        private void MonitorExit()
        {
            LocalProcess.WaitForExit();
            string logMessage = string.Format("Process {0} has exited", processName);
            CmDataCapture.Capture(CmDataCapture.MaskError, logMessage);
		...
		public Process LocalProcess
		{
			...
			set
			{
				...
				if (localProcess != null)
				{
                    ...
                    string logMessage = string.Format("Starting thread to monitor exit for process {0}", processName);
                    CmDataCapture.Capture(CmDataCapture.MaskInfo, logMessage);

                    HasProcessExited = false;
					Thread thread = new Thread(new ThreadStart(MonitorExit));
					thread.Start();
				}
			}
		}
		...
		/// </summary>
        private void CheckMinimize(object stateInfo)
		{
            while (LocalProcess != null && !HasProcessExited)
			{         
				try
				{
					CallBack EnumWindowsFunc = new CallBack(RestoreToNormalIfMinimize );
		...
		private bool Start()
		{
			...
			{       
				...
				if (IsAWindow())
				{
					...
					{         
                        ThreadPool.QueueUserWorkItem(new WaitCallback(CheckMinimize));
					}       
#endif
				} 
#if	( !WINCE )
                if ( retVal && IsAWindow() && WaitForWindow )
                {
                    String []windowTitles = WindowTitle.Split(',');
                    CmDataCapture.Capture( CmDataCapture.MaskError,  
                        String.Format("Waiting for [{0}] Window to Load", windowTitles[0]));
                    bool bIsWindowNotLoaded = true;
                    while (bIsWindowNotLoaded)
                    {
                        IntPtr hWnd = PInvoke.FindWindow(null,windowTitles[0]);
                        if( hWnd == IntPtr.Zero)
                        {
                            Thread.Sleep( 200 );
                        }
                        ...
                        {
                            bIsWindowNotLoaded = false;
                            CmDataCapture.Capture( CmDataCapture.MaskError,  
                                String.Format("[{0}] Window to Loaded", windowTitles[0]));
                        }
                    }
                }
#endif
				...
				if ( retVal && WaitForExit )
				{
					...
					if ( LocalProcess != null && !HasProcessExited )
					{
						LocalProcess.WaitForExit();
					}
				}
				...
				if ( LocalProcess != null )
				{
                    if (HasProcessExited == false)
					{
								...
								&& ( this.Filename.ToLower().IndexOf( "rapnet" ) >= 0 ) )
						{
							...
							MyApplication.SendProcessMessage( ConfigData.rapApplication, "operation=stop-app;" );
						}
						...
						if ( PInvoke.PostMessage(hWnd, message, wParam, lParam) )
						{
                            ...
                            while (!HasProcessExited && elapsedMS < configData.ShutDownTimeout)	// 15000 ms is 15 seconds
							{
								Thread.Sleep(100);
								elapsedMS += 100;
							}
					...
					// SSCOB-795: Keep LocalProcess valid until process exits
					if (HasProcessExited)
					{
						LocalProcess = null;
					}
		...
		{     
			if ( LocalProcess == null || HasProcessExited || !IsAWindow())
			{         
					...
					if (sbProcessText.ToString().EndsWith(windowTitles[count]) )
					{
                        ...
                        if((bResult) && (winPlace.showCmd != (uint)PInvoke.ShowWindowFlags.SW_SHOWMAXIMIZED ) )
                        {
                            ...
                            if ( ( Maximize ) )
                            {
                                ...
                                CmDataCapture.Capture( CmDataCapture.MaskInfo, "MaximizeWindows(): Setting ShowWindowFlags to SW_SHOWMAXIMIZED");
                            }
                            else
                            { 
                                //restore normal window even if minimized
                                nFlag = (int) PInvoke.ShowWindowFlags.SW_SHOWNORMAL;
                                CmDataCapture.Capture( CmDataCapture.MaskInfo, "MaximizeWindows(): Setting ShowWindowFlags to SW_SHOWNORMAL");
                            }    
            ...
            private bool MaximizeNoShowWindows(IntPtr hWnd, int iProcessIndex) 
        {     
            if ( LocalProcess == null || HasProcessExited || !IsAWindow())
            {         
                return false; //stop enumeration
            ...
            {   
                ...
                { 
                    if (sbProcessText.ToString().EndsWith(windowTitles[count]) )
                    {
                        ...
                        IntPtr hWndInsertAfter = (IntPtr) PInvoke.SetWindowPosInsertAfter.HWND_BOTTOM;
                        PInvoke.SetWindowPos(hParentWnd,hWndInsertAfter,0,0,0,0,(uint)(PInvoke.SetWindowPosFlags.SWP_NOMOVE|PInvoke.SetWindowPosFlags.SWP_NOSIZE|PInvoke.SetWindowPosFlags.SWP_HIDEWINDOW));
                        break;
                    }
                }
            }
		...
		public void MinimizeWindow()
		{
			try
			{
				CallBack EnumWindowsFunc = new CallBack(MinimizeWindows);
				PlatformEnumWindows(EnumWindowsFunc, 0 );
			}
			catch ( Exception e )
			{
        ...
        public void MaximizeNoShowWindow()
        {
            try
            {
                CallBack EnumWindowsFunc = new CallBack(MaximizeNoShowWindows);
                PlatformEnumWindows(EnumWindowsFunc, 0 );
            }
            catch ( Exception e )
            {
		...
		public void MaximizeWindow()
		{
			try
			{
				CallBack EnumWindowsFunc = new CallBack(MaximizeWindows);
				PlatformEnumWindows(EnumWindowsFunc, 0 );  
			}
			catch ( Exception e )
			{
		...
		private void PlatformEnumWindows( CallBack EnumWindowsFunc, int valInt )
		{
#if	( !WINCE ) 
		...
		/// </summary>
		/// <param name="EnumWindowsProc">CallBack function</param>
		/// <param name="Val">Value to pass to callbacks in LParam</param>
		private void EnumCEWindows( CallBack EnumWindowsFunc, int valInt )
		{
			// unsafe
			{
        ...
        /// <summary>
        /// Inidcates if LaunchPad should wait til this executable window is loaded before continuing (used for SSCOUI.exe)
        /// </summary>
        ...
        /// <summary>
        /// Inidcates if the Executable is still alive, then it should never be restarted
        /// </summary>
        public bool NoRestartIfAlive
        {
            get
            {
                return noRestartIfAlive;
            }
            set
            {
                noRestartIfAlive = value;
            }
        }
        ...
        /// <summary>
        /// Let executable loads the window before continuing...
        /// </summary>
        private bool waitForWindow = false;

        /// <summary>
		/// Don't restart executable if it's alive
		/// </summary>
        private bool noRestartIfAlive = false;
        ...
        /// <summary>
        /// backing field for HasProcessExited
        /// </summary>
        bool hasProcessExited = true;

        /// <summary>
        /// Name of executable process
        /// </summary>
