    ...
    public class LanePsx : IDisposable
    {
        ...
        public virtual void LanePsx_PsxEvent(object sender, PsxEventArgs e)
        {
            ...
            try
            {
                ...
                else if (e.EventName.Equals(Psx.EventClick))
                {
                    ...
                    if (ConfigData.MultiExclusiveLanes && (e.ControlName.Equals("LaneButton") || e.ControlName.Equals(LanePsx.LANE_STATUS_BUTTON)) &&
                    ...
                    if (e.ControlName.Equals("LaneButton") || e.ControlName.Equals(LanePsx.LANE_STATUS_BUTTON) ||
                    ...
                    if (e.ContextName.Equals("Alert") && e.ControlName.Equals("LaneButton"))
                    {
                        try
                        {
                            ...
                            {
                                oldExclusiveLane.psx.GenerateEvent(oldExclusiveLane.RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap,
                                    "operation=release");
                            }
                            if (Owner.AssistLane != 0)
                            {
                                ...
                                if (null != assistLane)
                                {
                                    assistLane.psx.GenerateEvent(assistLane.RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap,
                ...
                else if (e.EventName.Equals(ConfigData.eventRap))
                {
                    ...
                    if (operation != null)
                    {
                        ...
                        {
                            Owner.ReleaseHandScanner(); // CRD209937: Release HHScanner from lane
                            Owner.CancelWaitForResponse(LaneNumber);
                            Owner.ExclusiveLane = LaneNumber;
                            handled = true;
                        }
                        ...
                        else if (operation.Equals("assist-release"))
                        {
                            ...
                            //Owner.ReleaseHandScanner(); //CRD124746
                            CmDataCapture.Capture(CmDataCapture.MaskError, "Setting scanner acquired lane to blank");
                            handled = true;
                        }

                        #region Meijer customization for sharing scanner with lanes - Begin

                        else if (operation.StartsWith("scanner-status"))
                        {
                            ...
                            LanePsx requestingLane = Owner.LanePsxArray.SingleOrDefault(lane => lane.HostName.Equals(requestingLaneHostName, StringComparison.OrdinalIgnoreCase));
                            if (requestingLane != null)
                            {
                                if (!string.IsNullOrEmpty(Owner.HandScannerAcquiredLane)) //POS181195 - Removed extra condition as it is disabling the button everytime.
                                {
                                    if (requestingLaneHostName.Equals(Owner.HandScannerAcquiredLane)) //POS181195 - changed condition to not to display unavailable message when AM accessed from RAP.
                                    {
                        ...
                        else if (operation.StartsWith("request-scanner") || operation.StartsWith("release-scanner"))
                        {
                            ...
                            if (substr.Equals("request-scanner"))
                            {
                                ...
                                if (requestingLane != null)
                                {
                                    Owner.AssignHandScannerToLane(requestingLane.HostName);
                                    CmDataCapture.Capture(CmDataCapture.MaskError, "Setting scanner acquired lane to :" + Owner.HandScannerAcquiredLane);
                                    requestingLane.psx.GenerateEvent(RemoteConnection, String.Empty, String.Empty, ConfigData.eventRap, "operation=scanner-engaged");
                                }
                                else
                                {
                        ...
                        #region Meijer customization for Meijer Logo loading into RAP printer device memory

                        else if (operation.Equals("bitmap-loaded"))
                        {
                            this.Owner.UpdateMeijerLogoStatus(MeijerLogoStatus.Loaded);
                            CmDataCapture.Capture(CmDataCapture.MaskInfo, string.Format("Notification from lane: {0} that the printer bitmap load was successful.", this.HostName));
                            handled = true;
                        }
                        else if (operation.Equals("bitmap-loadfailed"))
                        {
                            this.Owner.UpdateMeijerLogoStatus(MeijerLogoStatus.NotLoaded);
                            CmDataCapture.Capture(CmDataCapture.MaskInfo, string.Format("Notification from lane: {0} that the printer bitmap load has failed.", this.HostName));
                            handled = true;
                        }
        ...
        static public void StartConnectionThread(LanePsx[] paramPsx)
        {
            try
            {
                ...
                {
                    connectThread = ConfigData.LoadClass("ConnectThread") as ConnectThread;
                }
        ...
        public virtual string RapSingleContext
        {
            get
            {
                ...
                {
                    int LaneDisplayCount = ConfigData.GetConfigData().EnableRAPLite ? ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite : configData.LaneConfigList.Count;
                    CmDataCapture.Capture(CmDataCapture.MaskInfo, "MaxNumberOfLanesShownForRAPLite =" + ConfigData.GetConfigData().MaxNumberOfLanesShownForRAPLite);
