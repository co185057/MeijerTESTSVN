...
/*
 * MD5 implementation. Written by Simon Tatham, wrapped to class by Kreso
 */
...
void MD5::MD5_Core_Init(MD5_Core_State *a) {
    a->h[0] = 0x67452301;
    a->h[1] = 0xefcdab89;
    a->h[2] = 0x98badcfe;
    a->h[3] = 0x10325476;
...
void MD5::MD5_Block(MD5_Core_State *x, uint32 *block) {
    uint32 a,b,c,d;

    a = x->h[0]; b = x->h[1]; c = x->h[2]; d = x->h[3];
...
void MD5::MD5Init(void) {
    MD5_Core_Init(&s->core);
    s->blkused = 0;
    s->lenhi = s->lenlo = 0;
}

void MD5::Update(_TUCHAR const *p,unsigned len) {
    _TUCHAR *q = (_TUCHAR *)p;
    uint32 wordblock[16];
    uint32 lenw = len;
    int i;
...
void MD5::Final(_TUCHAR output[16]) {
    int i;
    unsigned pad;
    _TUCHAR c[64];
    uint32 lenhi, lenlo;
