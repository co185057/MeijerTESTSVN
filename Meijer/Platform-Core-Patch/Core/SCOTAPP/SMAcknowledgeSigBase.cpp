...
SMStateBase  *SMAcknowledgeSigBase::PSButton1(void) // Yes, 
{
    ...
    if( fnTemp != NULL )
    {
        ...
        m_pAppPSX->SendCommand( _T("CMSigCap"), UI::COMMANDCAPTUREPICTURE, vReturn, 1, COleVariant( csTempFile ) );
        if (vReturn.boolVal == VARIANT_TRUE)
        {
            //csTempFile.Replace(_T("C:"), _T("\\C$"));
            //CString csFilePath = _T("\\\\") + co.csLocalMachineName + csTempFile ;
                
            // m_pAppPSX->SetConfigProperty(_T("SigCap"), _T(""), UI::PROPERTYPICTURE, csFilePath);
			...
			SMStateBase::SCOTDeleteFile( ( LPCTSTR ) csTempFile );   //TAR 348946
				  ...
				  || (co.TenderList[ps.m_languageUsed][nTenderButtonIndex-1].nVerifySignatureOverAmount != -1 && co.TenderList[ps.m_languageUsed][nTenderButtonIndex-1].nVerifySignatureOverAmount  <= lLastTender) ) // if verify, saving signature and printing receipt will be delayed to confirmSig
	{
		...
		if (cDMLastTrack==NULL)
		{
			  ...
			  csInstruction.Format(ps.GetPSText(SCOT_LANE_STATUS, SCOT_LANGUAGE_PRIMARY), 
			    _T(" "),
		        ps.GetPSText(RA_LANEBTT_SIGCAPTURE, SCOT_LANGUAGE_PRIMARY), ps.GetPSText(LTT_SIGNCONFIRM, SCOT_LANGUAGE_PRIMARY), co.IsSecurityDisableOn()==true ? ps.GetPSText(RA_SECONDLINE_SECURITYDISABLED, SCOT_LANGUAGE_PRIMARY) : _T(""), 
