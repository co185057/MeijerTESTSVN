...
IMPLEMENT_DYNCREATE(CReporting, CObject)// MFC Runtime class/object information
...
{
  trace(L7, _T("+CReporting::InitData"));
...
void CReporting::ResetCurrencyList(void)
{
	CReporting::m_SCurrencyList.clear();
}
///////////////////////////////////////////////////////////////////////////////
...
{
    trace(L7, _T("+CReporting::TransactionStart"));
    ...
    {  
	   ...
	   //Attaching difference between local time and universal time in minutes into Transaction start record
		...
		ret=GetTimeZoneInformation(&stTimeZone);		
     ...
     {
         trace(L7, _T("-CReporting::TransactionStart Not InProgress"));
     }
         return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////
long CReporting::PickListItemSold( const CString& strItemUPC, int nTimesSold /*= 1*/ )
{
	trace( L7, _T("+CReporting::PickListItemSold") );
	...
	trace( L7, _T("-CReporting::PickListItemSold<%s>"), strRecord );
...
{
    long lRetCode = RPT_INVALIDPARM;
    trace(L7, _T("+CReporting::WriteToCollector"));
    if(co.fOperationsLogDataForSystemReports && !TrainingModeObj.IsTMOn()) //Capture Reporting data 
    {
        ...
        lRetCode = WriteReportData(xmlString);
        trace(L7, _T("-CReporting::WriteToCollector<%s>"), csXmlFormatted);
        return lRetCode;
    }
    else 
        return lRetCode;
...
{
    trace(L7, _T("+CReporting::TBCreateReportEntry"));
    ...
    lRetCode = WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::TBCreateReportEntry<%s>"), m_csRecord);
    return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
...
{
   trace(L7, _T("+CReporting::InterventionStart")); 
   CString csReportPrority, cshID, csClass,csInterType,csItemCount,csTime,csItemID;  
   long  lRetCode = RPT_INVALIDPARM;   
   if(csDesc == ATTSUSPEND)
   {
       trace(L7, _T("-CReporting::InterventionStart<RPT_INVALIDPARM><%s> SUSPENDs are Not included in rpt.xml"),csDesc ); 
   ...
   else
   {
    ...
    lRetCode = WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::InterventionStart<%s>"),m_csRecord ); 
    return lRetCode;
   }
...
{
   long  lRetCode = RPT_INVALIDPARM;
   trace(L7, _T("+CReporting::InterventionEnd")); 
   if (hlD == -1)
   {
       trace(L7, _T("-CReporting::InterventionStart<RPT_INVALIDPARM><%s> -1 ID Detected"),csDesc ); 
       return RPT_INVALIDPARM;
   }
   CString cshID,csOperator,csPagerEvent,csStation,csTime, csActive, csPager, csStatus, csTerminator;
   ...
   {
	   //+TAR409050
	   if (((csDesc == _T("VISUAL VERIFIED")) || (csDesc == _T("UNKNOWN"))) && (m_csTenderOperator != EMPTY_STRING))
	   {	
		   csClearingOperator = m_csTenderOperator;
		   if (m_bTransactionInProgress)
			   m_bTransAssisted = true;   
   ...
   else
   {
       ...
       csTerminator = _T("/>");
       m_csRecord = m_csStart + csTime + cshID + m_csTypeEnd + csPagerEvent + csActive + csStation;
       if (csClearingOperator != EMPTY_STRING)
       {
       ...
       lRetCode= WriteToCollector(m_csRecord);
       trace(L7, _T("-CReporting::InterventionEnd<%s>"),m_csRecord ); 
...
{
    trace(L7, _T("+CReporting::ItemException")); 
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::ItemException<%s>"), m_csRecord); 
    return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
...
long CReporting::ItemException(CString csItemDesc, CString csItemSku, long lItemPrice,CString csObservedWts, CString csExpected)
{
    ...
    m_csStart.Format(_T("<%s"), m_csNameStart);
    csSKU.Format(_T("%s=\"%s\" "),ATTITEMSKU, csItemSku);
    csMisMatchDesc.Format(_T("%s=\"%s\" "),ATTMSMATCHDESC, csItemDesc);
...
{
	trace(L7, _T("+CReporting::ModeChange"));
	...
	trace(L7, _T("-CReporting::ModeChange<%s>"),m_csRecord );
	return lRetCode;
...
{
	trace(L7, _T("+CReporting::OperatorLogin"));
	...
	trace(L7, _T("-CReporting::OperatorLogin<%s>"),m_csRecord );
	return lRetCode;
}
...
{
    trace(L7, _T("+CReporting::TransactionEnd"));
    ...
    if(m_bTransactionInProgress || m_bForceEnd)
    {
        ...
        m_csRecord = m_csStart + csTime + csItemCount + csAmount + csDisposition + csOffline 
    ...
    trace(L7, _T("-CReporting::TransactionEnd<%s>"),m_csRecord );
    return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
...
{   
    trace(L7, _T("+CReporting::TenderStart"));
    ...
    case TB_TENDER_COUPON:
        csCat = ps.GetPSText(MSG_TB_TENDER_COUPON);
        break;
    case TB_TENDER_OTHERPAYMENT:
        csCat = ps.GetPSText(MSG_TB_TENDER_OTHERPAYMENT);
        break;
    case TB_TENDER_NONE:
        csCat = ps.GetPSText(MSG_TB_TENDER_NONE);
        break;
    case TB_TENDER_CASH:
        csCat = ps.GetPSText(MSG_TB_TENDER_CASH);
        break;
    case TB_TENDER_CREDIT:
        csCat = ps.GetPSText(MSG_TB_TENDER_CREDIT);
        break;
    case TB_TENDER_DEBIT:
        csCat = ps.GetPSText(MSG_TB_TENDER_DEBIT);
        break;
    case TB_TENDER_LOYALTY:
        csCat = ps.GetPSText(MSG_TB_TENDER_LOYALTY);
        break;
    case TB_TENDER_DEBITSAVING:
        csCat = ps.GetPSText(MSG_TB_TENDER_DEBITSAVING);
        break;
    case TB_TENDER_DEBITCHECKING:
        csCat = ps.GetPSText(MSG_TB_TENDER_DEBITCHECKING);
        break;
    case TB_TENDER_EBT:
        csCat = ps.GetPSText(MSG_TB_TENDER_EBT);
        break;
    case TB_TENDER_FOODSTAMPS:
        csCat = ps.GetPSText(MSG_TB_TENDER_FOODSTAMPS);
        break;
    case TB_TENDER_GIFTCARD:
        csCat = ps.GetPSText(MSG_TB_TENDER_GIFTCARD);
        break;
    case TB_TENDER_PIP_GENERIC:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_GENERIC);
        break;
    case TB_TENDER_PIP_CREDIT:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_CREDIT);
        break;
    case TB_TENDER_PIP_DEBIT:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_DEBIT);
        break;
    case TB_TENDER_PIP_DEBIT_CASHBACK:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_DEBIT_CASHBACK);
        break;
    case TB_TENDER_PIP_LOYALTY:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_LOYALTY);
        break;
    case TB_TENDER_PIP_DEBITSAVING:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_DEBITSAVING);
        break;
    case TB_TENDER_PIP_DEBITCHECKING:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_DEBITCHECKING);
        break;
    case TB_TENDER_PIP_EBT:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_EBT);
        break;
    case TB_TENDER_PIP_FOODSTAMPS:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_FOODSTAMPS);
        break;
    case TB_TENDER_PIP_GIFTCARD:
        csCat = ps.GetPSText(MSG_TB_TENDER_PIP_GIFTCARD);
        break;
    case TB_TENDER_USERDEFINED1:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED1);
        break;
    case TB_TENDER_USERDEFINED2:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED2);
        break;
    case TB_TENDER_USERDEFINED3:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED3);
        break;
    case TB_TENDER_USERDEFINED4:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED4);
        break;
    case TB_TENDER_USERDEFINED5:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED5);
        break;
    case TB_TENDER_USERDEFINED6:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED6);
        break;
    case TB_TENDER_USERDEFINED7:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED7);
        break;
    case TB_TENDER_USERDEFINED8:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED8);
        break;
    case TB_TENDER_USERDEFINED9:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED9);
        break;
    case TB_TENDER_USERDEFINED10:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED10);
        break;
    case TB_TENDER_USERDEFINED11:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED11);
        break;
    case TB_TENDER_USERDEFINED12:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED12);  // TAR 248036 - Fix typo error
        break;
    case TB_TENDER_USERDEFINED13:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED13);
        break;
    case TB_TENDER_USERDEFINED14:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED14);
        break;
    case TB_TENDER_USERDEFINED15:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED15);
        break;
    case TB_TENDER_USERDEFINED16:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED16);
        break;
    case TB_TENDER_USERDEFINED17:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED17);
        break;
    case TB_TENDER_USERDEFINED18:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED18);
        break;
    case TB_TENDER_USERDEFINED19:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED19);
        break;
    case TB_TENDER_USERDEFINED20:
        csCat = ps.GetPSText(MSG_TB_TENDER_USERDEFINED20);
        break;
    case TB_TENDER_CHECK:     //RFC 356020 - Tender check at rap
	case TB_TENDER_CHECK_LANE://RFC 356020 	- Tender check at Lane
        csCat = ps.GetPSText(MSG_TB_TENDER_CHECK);
        break;
    case TB_TENDER_CASH_RAP:
        csCat = ps.GetPSText(MSG_TB_TENDER_CASH_RAP);
        break;
    case TB_TENDER_ASSISTTENDER:
        csCat = ps.GetPSText(MSG_TB_TENDER_ASSISTTENDER);
        break;
    case TB_TENDER_VOUCHERPAYMENT:
        csCat = ps.GetPSText(MSG_TB_TENDER_VOUCHERS);
        break;
        }
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::TenderStart<%s>"), m_csRecord);
    return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
// Function:    TenderEnd                                        
...
{
    trace(L7, _T("+CReporting::TenderEnd"));
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::TenderEnd<%s>"), m_csRecord);
    return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
...
{
    trace(L7, _T("+CReporting::CurrencyAccepted"));
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::CurrencyAccepted<%s>"), m_csRecord);
    return lRetCode;
}
///////////////////////////////////////////////////////////////////////////////
...
{
    trace(L7, _T("+CReporting::CurrencyDispensed"));
    CString csAmount,csCurrencyType,csquantity,csDenomination,csID,csTime;
    ...
    m_csRecord = m_csStart + csTime + csID + csCurrencyType + csDenomination +  csquantity + csAmount;
    lRetCode=WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::CurrencyDispensed<%s>"), m_csRecord);
    return lRetCode;
}

//+SOTF 5572
CString CReporting::GetGurrencyList(void)
{
...
{
    trace(L6, _T("+CReporting::CurrencyCashManagement"));
...
long CReporting::CurrencyCashManagement(CString csTime,long ID, bool bBill, CString csDen,long lChangeInCount, CString csUserID, CString csCashierID)
{
    ...
    //PW500001
    if (bBill == true)
        csDeviceType.Format(_T("%s=\"%d\" "),ATTDEVICETYPE, 4 );
    else
        csDeviceType.Format(_T("%s=\"%d\" "),ATTDEVICETYPE, 5 );
...
long CReporting::SaveSignatureFileData(CFile* cfPointer , BSTR pSigData, LPCTSTR lpReceiptType)
{
	...
	trace(L7, _T("+CReporting::SaveSignatureFileData"));
    long  lRetCode = RPT_INVALIDPARM;
    UINT iCount;
    BSTR bOutString;
    ...
    trace(L7, _T("-CReporting::SaveSignatureFileData<%s>"), m_csRecord);
    return lRetCode;
...
{
    trace(L7, _T("+CReporting::SaveReceiptData"));
	CFile *pCFile = NULL;
	long lRetCode;
	...
	if (lpFileName != NULL)
	{
      	...
      	catch(CFileException *e)      
		{
		  ...
		  trace(L7, _T("-CReporting::SaveReceiptData - lRetCode:<%d>"),lRetCode);
		  return lRetCode;
		}
...
{
	trace(L7, _T("+CReporting::Signatures"));
	CFile *pCFile = NULL;
	long lRetCode;
	try
	{
	...
	catch(CFileException *e)      
    {
      ...
      e->Delete();
	  lRetCode = RPTERROR_CANTCREATEFILE;
	  trace(L7, _T("-CReporting::Signatures - lRetCode:<%d>"),lRetCode);
	  return lRetCode;
	}
...
{
    trace(L7, _T("+CReporting::HexToAscii"));
    BYTE cIn;
    BYTE cNibble;
	TCHAR cChar;
...
void CReporting::RPBusinessDate()
{
	...
	trace(L7, _T("+CReporting::RPBusinessDate"));
	CTime ctCurrentTime;
    ctCurrentTime = CTime::GetCurrentTime();
	CString csTime;
...
{
    trace(L7, _T("+CReporting::GetCurrentTBBusinessDate"));
...
{
    trace(L7, _T("+CReporting::RAPDispenseChange"));
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L7, _T("-CReporting::RAPDispenseChange<%s>"), m_csRecord);
    return lRetCode;
}
...
{
    trace(L6, _T("+CReporting::LogDeviceError"));
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L6, _T("-CReporting::LogDeviceError<%s>"), m_csRecord);
    return lRetCode;
}
        ...
        CString csUserID, CString csCashierID )
{
    ...
    //For nDeviceType 4 records, we display them at "BILL" section for BILL RECYCLER. 
    //For nDeviceType 5 records, we display them at "COIN" section for COIN RECYCLER.
...
// Returns:     long
//PW500001/////////////////////////////////////////////////////////////////////////////////
long CReporting::CurrencyCashManagement( int nDeviceType, CArray<CString,CString>& caDenomList, CArray<int,int>& caChangeInCount, CArray<int,int>& caBalanceCount, CString csUserID, CString csCashierID )
{
	trace(L6, _T("+CReporting::CurrencyCashManagement"));
...
// Returns:     long
//PW500001/////////////////////////////////////////////////////////////////////////////////
long CReporting::CurrencyCashManagement(CString csTime, long ID, int nDeviceType, CString csDen, long lBalanceInCount, long lChangeInCount, CString csUserID, CString csCashierID, int nTerminalNumber )
{
    trace(L6, _T("+CurrencyCashManagement. Per element"));
    long lRetCode = 0;
...
{
    trace(L6, _T("+CReporting::LogItemDetails"));
	...
	csDescription.Format(_T("%s=\"%s\" "),ATTDESCRIPTION,io.d.csDescription);
    ...
    lRetCode=WriteToCollector(m_csRecord);
    trace(L6, _T("-CReporting::LogItemDetails %s"), m_csRecord);
    return lRetCode;
}
...
//+TAR 343053
bool CReporting::IsTransactionInProgress(void)
{
