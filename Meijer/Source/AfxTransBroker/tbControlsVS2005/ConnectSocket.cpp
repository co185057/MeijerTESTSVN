...
switch(WSAGetLastError()) {
	case WSAEADDRNOTAVAIL:
	    	_stprintf_s(Log_Error, _T("The specified address is not available from the local computer"));
	        break;
	case WSAEADDRINUSE:
			_stprintf_s(Log_Error, _T("The specified address is already in use"));
 			break;
	case WSAEAFNOSUPPORT:
			_stprintf_s(Log_Error, _T("Addresses in the specified family cannot be used with this socket"));
 			break;
	case WSAECONNABORTED:	
			_stprintf_s(Log_Error, _T("Connection aborted due to timeout or other failure"));
			break;
	case WSAECONNREFUSED:
            _stprintf_s(Log_Error, _T("Connection refused, no server"));
    		break;
	case WSAECONNRESET:				
			_stprintf_s(Log_Error, _T("Connection was reset by the remote side"));
			break;
	case WSAEDESTADDRREQ:	
            _stprintf_s(Log_Error, _T("A destination address is required"));
		    break;
	case WSAEFAULT:
			_stprintf_s(Log_Error, _T("The namelen argument is incorrect"));
 			break;
	case WSAEINPROGRESS:
			_stprintf_s(Log_Error, _T("A blocking Windows Sockets call is in progress"));
 			break;
	case WSAEINTR:
			_stprintf_s(Log_Error, _T("The blocking call was canceled using WSACancelBlockingCall"));
 			break;
	case WSAENETDOWN:
			_stprintf_s(Log_Error, _T("The Windows Sockets implementation has detected that the network subsystem has failed"));
 			break;
	case WSAEINVAL:
			_stprintf_s(Log_Error, _T("The socket is already bound to an address"));
 			break;
	case WSAEISCONN:
		     _stprintf_s(Log_Error, _T("The socket is already connected"));
			 break;
	case WSAEMFILE:
			_stprintf_s(Log_Error, _T("No more file descriptors are available"));
			break;
	case WSAEMSGSIZE:	
            _stprintf_s(Log_Error, _T("The datagram was too large to fit into the specified buffer and was truncated"));
			break;
    case WSAENETUNREACH:
            _stprintf_s(Log_Error, _T("Network unreachable"));
		    break;
	case WSAENOBUFS	:
			_stprintf_s(Log_Error, _T("No buffer space is available. The socket cannot be connected"));
 			break;
	case WSAENOTCONN:	
			_stprintf_s(Log_Error, _T("The socket is not connected"));
 			break;
	case WSAENOTSOCK:	
		    _stprintf_s(Log_Error, _T("The descriptor is not a socket"));
			break;
	case WSAEOPNOTSUPP:	
			_stprintf_s(Log_Error, _T("MSG_OOB was specified, but the socket is not of type SOCK_STREAM"));
			break;
	case WSAEPROTOTYPE:	
			_stprintf_s(Log_Error, _T("The specified protocol is the wrong type for this socket"));
			break;
	case WSAEPROTONOSUPPORT:	
			_stprintf_s(Log_Error, _T("The specified protocol is not supported"));
			break;
	case WSAESHUTDOWN:	
            _stprintf_s(Log_Error, _T("The socket has been shut down; it is not possible to recv on a socket after shutdown has been invoked with how set to 0 or 2"));
			break;
	case WSAESOCKTNOSUPPORT:	
			_stprintf_s(Log_Error, _T("The specified socket type is not supported in this address family"));
			break;
	case WSAETIMEDOUT:
            _stprintf_s(Log_Error, _T("Connection timed out"));
		    break;
	case WSAEWOULDBLOCK:
			_stprintf_s(Log_Error, _T("The socket is marked as nonblocking and the connection cannot be completed immediately. It is possible to select the socket while it is connecting by selecting it for writing"));
			break;
	case WSANOTINITIALISED:
			_stprintf_s(Log_Error, _T("A successful WSAStartup must occur before using this function"));
 			break;
	default:
		    _stprintf_s(Log_Error, _T("Unknown error returned from WSAGetLastError()"));
  }		    
  ATLTRACE(Log_Error);
}
