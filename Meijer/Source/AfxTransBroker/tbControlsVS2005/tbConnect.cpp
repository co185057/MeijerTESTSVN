...
//Open the subkey (or create it if it does not exist)
  ...
  if (disposition == REG_CREATED_NEW_KEY) {
	...
	rc = RegQueryValueEx(hKey,csDefaultDeviceIDs,NULL,NULL, (L	PBYTE) szDeviceIDs2, &dwDataSize);
	ATLASSERT(rc==ERROR_SUCCESS);
		...
		if (m_ReadSocket.Connect(m_csServerName, m_nPortNumberWrite)) {
			...
			if (iLength > 0)	{
				...
				for(i=0; i<= iNumberOfDevices; i++)	{
					...
					_stscanf_s(csMessage2, _T("%x"),&iDeviceID);
					
					ptr[i] = iDeviceID;
				}

				if(csMessage.GetLength() >= 1) {
					_stscanf_s(csMessage,_T("%x"),&iDeviceID);
					ptr[i] = iDeviceID;
				}
