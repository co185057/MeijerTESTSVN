...
const CHAR *cTBMessageDef="c:\\scot\\config\\TBControls.xml";
...
void CTBGenMsg::Reset(ResetOptions option)
{
    switch (option) 
    {
    case ResetOptions::RESET_FIELD_DATA:
        {
            if (m_pFields) 
            {
        ...
        break;
    case ResetOptions::RESET_XML_DOC:
        {
            ClearAll();
        }
        break;
...
void CTBGenMsg::Load()
{
   ...
   {
      strResult = _bstr_t(OLESTR("Validation failed on ")) + strFile +
         _bstr_t(OLESTR("\nReason: ")) + _bstr_t(pError->Getreason()) +
         _bstr_t(OLESTR("\nSource: ")) + _bstr_t(pError->GetsrcText()) +
         _bstr_t(OLESTR("\nLine: ")) + _bstr_t(pError->Getline());
		throw CString((LPCTSTR)strResult);
   ...
   if (m_pMessageNode)
   {
    ...
    Reset(ResetOptions::RESET_FIELD_DATA);
	m_bXmlLoaded=true;
}
