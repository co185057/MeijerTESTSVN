    ...
    #include <atlconv.h>  // Added to support improvements for TAR 419719
...
// NewUI
...

    {
      ...
      m_MaxTransLogSize = 2097152; //2M   RFC422884 changed from (1048576 1MB)
      m_sTransitionBackup = _T(""); // Added for TAR 419719
    ...
    long MPProcedures::Initialize(void)
    {
	  ...
	  OpenTransLogFile( TRUE);  // Added a sleep between the first and second attempts to open the ".stf" file per TAR 419719
   ...
   return ProcessNextState();
    ...
    long MPProcedures::ProcessNextState(void)
    {
      ...
      bool bIsPreviousStateStoreMode = false;   // TAR 449476
      bool bIsAttendantModeChanged = false;     // TAR 449476
      ...
      while ( sNext != STATE_NULL )
      {
        ...
        // + TAR 453310 Move the change of tar 449476 inside while loop
        // TAR 449476 - Determine if current state is a store mode state before we move into a new state.
        if(sCurr->storeMode())
        {
        ...
        {
            bIsPreviousStateStoreMode = false;
        }

        // TAR 449476 - Determine if there is a change in the attendant mode of operation before we move into a new state.
        if(sCurr->m_bIsAttendantModeChanged)
        {
            bIsAttendantModeChanged = true;
        }
        // - TAR 453310
        trace(L6,_T("ProcessNextState() bIsPreviousStateStoreMode=%d bIsAttendantModeChanged=%d"),bIsPreviousStateStoreMode,bIsAttendantModeChanged);
        ...
        trace(L6,_T("bPoppedSystemMessage=%d csLastName=%s csNextStateName=%s sTemp=%s"),bPoppedSystemMessage,csLastName,csNextStateName,getStateName(sTemp));

		sCurr->SetStateBeforeHelp(csLastName,csNextStateName);//SR879
		
	    if (DelayOrIgnoreStateChangeCheck(bPoppedSystemMessage, csLastName, csNextStateName, sTemp))
        {
			trace(L6,_T("ProcessNextState() returned 0 Because DelayOrOgnoreStateChangeCheck() returned true"));
            return 0;
        }
        ...
        if ( sNext == STATE_RETURN || sNext==STATE_RESUME)
        {
            ...
            trace(L4,_T("Changing state to %s"),getStateName(sCurr)); //message is also logged in ProcessNewState below.
        }
       ...
       //+TAR 444373 MOVE THIS CODE TO PSProcedureBase::ShowFrame()
		if (sCurr->storeMode())
        {
           ...
           // the state.
		   //CPSXRedrawLock  csRedrawLock(_T("Display"), _T("MPProcedures::ProcessNextState Store Mode"));
 ...
 //       PSEnableNotification(hDC, true, PS_ETYPE_TOUCH);
        ...
        {
          SMStateBase::bFromAssistMenu = true; //SSCOADK-6286 additions to SSCOB-18178 
        ...
        // TAR 449476 - Add the bIsAttendantModeChange condition to ensure we don't miss an Enter/Exit Store Mode message.
        if (   !co.IsInAssistMenus()  //Do not record entry/exit if in assist menus.
			&& (!co.IsAttendantModeOn() || bIsAttendantModeChanged) ) //TAR426337 Don't record if in attendant mode
        {
        // TAR 449476 - Add the bIsPreviousStateStoreMode condition so we don't send needless Enter Store Mode messages.
        if (sCurr->storeMode() && (!sCurr->bSecMaintMode) && !bIsPreviousStateStoreMode && !ps.RemoteMode())
        {
            SMStateBase::fRAPIsInStoreMode = true;
        ...
        {
			//TAR 279092+
			if(SMStateBase::bInApprovalNeeded && sCurr->helpMode() && (csCurrName.Find(_T("SystemMessage")) != -1))
			{
				trace(L7, _T("ra.OnExitStoreMode will not be executed this time"));				
			}		
			//TAR 279092-	
            // TAR 449476 - Add the bIsPreviousStateStoreMode condition so we don't send needless Exit Store Mode messages.
            else if(!bInSystemMessage && //don't call OnExitStoreMode if going to systemmessage.
                (bIsPreviousStateStoreMode && !co.IsAttendantModeOn() || ps.RemoteMode())) //sscoadk-628
			{
                SMStateBase::fRAPIsInStoreMode = false;
...
//////////////////////////////////////////////////////////////////////////
 CString MPProcedures::getStateName(const SMStateBase * const pState)
 {
   ...
   // The following member functions in MPProcedures where re-written per TAR 419719
  // OpenTransLogFile, CloseTransLogFile, ArchiveTransLogFile, and LogTransition

  // Start of changes for TAR 419719
  BOOL MPProcedures::OpenTransLogFile( BOOL bRetrySleep)
  { 
    BOOL rc = FALSE;
    CFileException Ex;
    // Set Open Flags:
    //   - to guarantee that the ".stf" file will open, either as a newly created file or as an existing file.
    //   - plus for write only access and to deny write access to other processes
    UINT nOpenFlags = CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite | CFile::shareDenyWrite;

    rc = m_pTransLogFile.Open(m_TransLogName, nOpenFlags, &Ex );
    if (rc == 0)
    {
      trace(L2,_T("STF OpenTransLogFile - failed to open %s due to a code %d, attempt one retry"), m_TransLogName, Ex.m_cause);

      if(bRetrySleep)
      {
        // if during startup, sleep a half second before attempting second open operation.
        Sleep( 500);
      }
      ...
      if (rc == 0)
      {
        trace(L2,_T("STF OpenTransLogFile - failed to open %s a second time due to a code %d, wait"), m_TransLogName, Ex.m_cause);
      }
    }

    if (rc != 0)
    {
        trace(L2,_T("STF OpenTransLogFile - opened %s"), m_TransLogName);

        // To ensure that we append new transitions to the end of the ".stf" file.
        m_pTransLogFile.SeekToEnd();
    }
  ...
  void MPProcedures::CloseTransLogFile()
  {
    ...
    if (m_pTransLogFile.m_hFile != CFile::hFileNull)
    {
      try
      {
        m_pTransLogFile.Close();
      }
      catch(CFileException* pEx )
      {
        bClosed = false;
        trace(L2,_T("STF CloseTransLogFile - failed due to a code %d"), pEx->m_cause);
        pEx->Delete();
      }
    }

    if(bClosed)
      trace(L2,_T("STF CloseTransLogFile - closed %s"), m_TransLogName);
  }
  ...
  void MPProcedures::ArchiveTransLogFile()
  {
    ...
    try
    {
      ...
      CFile::Rename(m_TransLogName, strBakFileName);
    }
    catch(CFileException* pEx )
    {
      trace(L2,_T("STF ArchiveTransLogFile - failed to rename %s to %s due to a code %d"), m_TransLogName, strBakFileName, pEx->m_cause);
      pEx->Delete();
    }

    OpenTransLogFile(FALSE);
  }

  void MPProcedures::LogTransition(CString& stateName, TBSTATE lastTBState)
  {
    USES_CONVERSION;  // Needed to use the T2CA conversion macro

    UINT nLen = 0;

    // Now we will ignore some states we don't want to see as they just clutter things up
    ...
    CString sOutputLine;
    sOutputLine.Format(_T("%d,%s,%d\r\n"), timeNow, stateName, (int) lastTBState); // Added carriage return to properly format the ".stf" file.

    if (m_pTransLogFile.m_hFile == CFile::hFileNull)
      OpenTransLogFile( FALSE);   // re-attempt the re-opening of the ".stf" file until we have a valid log file handle

    if (m_pTransLogFile.m_hFile != CFile::hFileNull)
    {
      nLen = m_sTransitionBackup.GetLength();
      if( nLen > 0)
      {
        // attempt to log a backed up Transition string (if possible)
        try
        {
          // Text output to the ".stf" file needs to be ANSI only (i.e. single byte characters).
          // Therefore we have to use the T2CA MFC Macro to convert any two byte Unicode characters to ANSI characters.
          m_pTransLogFile.Write( T2CA(m_sTransitionBackup), nLen);
        }
        catch(CFileException* pEx )
        {
          trace(L2,_T("STF LogTransition - failed to log backed up Transition string (%s), due to a code %d"), m_sTransitionBackup.Left(nLen-1), pEx->m_cause);
          pEx->Delete();
        }

        // Clear backed up Transition
        m_sTransitionBackup.Empty();
      }

      nLen = sOutputLine.GetLength();
      try
      {
        // Text output to the ".stf" file needs to be ANSI only (i.e. single byte characters).
        // Therefore we have to use the T2CA MFC Macro to convert any two byte Unicode characters to ANSI characters.
        m_pTransLogFile.Write( T2CA(sOutputLine), nLen);
      }
      catch(CFileException* pEx )
      {
        trace(L2,_T("STF LogTransition - failed to write Transition due to a code %d"), pEx->m_cause);
        pEx->Delete();

        // Save the skipped Transition for an attempt to save log it during the next attempt.
        m_sTransitionBackup = sOutputLine;
      }

      try
      {
        // make sure that each transition is saved to the ".stf" file
        m_pTransLogFile.Flush();

        // if ".stf" file grows to large, archive it.
        if (m_pTransLogFile.GetLength() > m_MaxTransLogSize)
          ArchiveTransLogFile();
      }
      catch(CFileException* pEx )
      {
        trace(L2,_T("STF LogTransition - Either the flush or the GetLength call failed due to a code %d"), pEx->m_cause);
        pEx->Delete();
      }
    }
    else
    {
      // if we still do not have a valid file handle . . .
      
      // Check if m_sTransitionBackup contains an old transition that we have failed to log in the ".stf" file -- if there is, then log it as lost in traces.log
      nLen = m_sTransitionBackup.GetLength();
      if( nLen > 0)
      {
        trace(L2,_T("STF LogTransition - failed to log the last backed up Transition string (%s) -- it will now be lost"), m_sTransitionBackup.Left(nLen-1));
        m_sTransitionBackup.Empty();
      }

      // then save the current Transition in sOutputLine for an attempt to log it during the next attempt at logging a Transition.
      m_sTransitionBackup = sOutputLine;
    }
  }
  // End of changes for TAR 419719
...
// Note:  Do not use _T in pragma messages.
#pragma message ("MPProcedures::FromName Review this code when changing to visual studio.net")
#pragma message ("  Also review defines in common.h for DECLARE_DYNCREATE and IMPLEMENT_DYNCREATE")
...
void MPProcedures::RemoteStateProcessing()
{
       ...
       && (false == sCurr->PSRemoteLegalForState()))
   {
        ...
        //SSCOP-3965+
        if(SMStateBase::ms_bKeepRemoteDevices == true)
        {
            trace(L1,_T("Disconnecting remote devices but not going back to assist, tell sm that exiting assistmode"));
            SMStateBase::ms_bKeepRemoteDevices = false;
...
void MPProcedures::AssistMenuStateProcessing()
{
...
bool MPProcedures::StateFrozen()
{
    ...
    if ( (csNextName.Find(_T("SystemMessage")) != -1 ))
    {
        ...
        {
            //sscoadk-1571+: from AssistMode to SystemMessage; set fStateFreeze to true
            if  (sCurr->fStateAssist && !ps.RemoteMode())
            {
                sNext->fStateFreeze = true;
            }
            //sscoadk-1571-
            trace(L6,_T("StateFrozen() returning false ps RemoteMode()=%d sNext->PSRemoteLegalForState()=%d"),
                ps.RemoteMode(),sNext->PSRemoteLegalForState());
            return false;
        }
    }
    //TAR 408719-
      ...
      { 
        ...
        if (!(sCurr->fStateAssist && sNext->assistMode()))
        {
            if (!sNext->helpMode() 
               ...
               || sCurr->fStateAssist)
            {
              trace(L6,_T("StateFrozen() returning true sNext->assistMode()=%d sNext->helpMode()=%d"),sNext->assistMode(),sNext->helpMode());
    ...
    if ( (csCurrName.Find(_T("CustomMessage")) != -1 ) && 
      ...
      (sNext != STATE_STOP && sNext != STATE_RESTART) ) // tar 284768
      {
        ...
        trace(L6,_T("StateFrozen() returning true"));
        return true;
      }
    //-dp185016
    trace(L6,_T("StateFrozen() returning false"));
    return false;
...
void MPProcedures::VideoStateCheck(CString csLastName)
{
	...
	//TAR 432289 - This causes swipe at Attract to hang. 
	//It is not valid to set bAlreadyInSnB whenever InProgress state is entered.
	//InProgress is not only used for Itemization.
...
bool MPProcedures::DelayOrIgnoreStateChangeCheck(BOOL &bPoppedSystemMessage, CString &csLastName, CString &csNextStateName, SMStateBase  *sTemp)
{
...

    {
		if (!bRAPApprovalMsg)	// RAP didn't send an approval message
		{
			...
			if( ( csLastName.Find(_T("InProgress")) != -1 )					|| 
                ...
                //we only saved delayed approval for dataentry if security is disable
			   ( csLastName.Find(_T("DataEntry")) != -1 
			    && SMStateBase::bDataNeededSecurity ==false )			    ||//SR801+
...

			{
                if ( SetDelayWaitForApprovalState(csNextStateName, csLastName))
                {
                    ...
                    trace(L6,_T("DelayOrIgnoreStateChangeCheck() returned true"));
                    return true;
                }
    ...
    trace(L6,_T("DelayOrIgnoreStateChangeCheck() returned false"));
    return false;
}
...
void MPProcedures::ProcessReturnResumeStates(SMStateBase  *sTemp)
{
    ...
    // +SSCOP-3649
    // If RESUME and this is RAPDataNeeded, pop to last state in stack
    /*CString csPoppedStateName = getStateName(sTemp2);
    if ( sNext==STATE_RESUME && csPoppedStateName.Find(_T("RAPDataNeeded")) != -1)
    {
        trace(L6,_T("STATE_RESUME and this is the RAPDataNeeded state"));
        try
        {
            while (lGLOBALStackCnt > 0)
            {
                delete sTemp2;
                sTemp2 = helpStateStack.pop();
                CheckStatePointer(sTemp2);
                lGLOBALStackCnt--;
                trace(L2,_T("HelpStack.pop 3 AFTER  - number states stacked = %d"), lGLOBALStackCnt);
            }
        }
        catch ( CMemoryException *e)
        {
             trace(L2,_T("HelpStack.pop 3 mem exception Caught - number states stacked = %d"), lGLOBALStackCnt);
             e->Delete();
        }
    }*/
    // -SSCOP-3649
...
void MPProcedures::ProcessNewState(BOOL &bPoppedSystemMessage, CString &csLastName)
{
    ...
    CheckStatePointer(sCurr);
       trace(L4,_T("Changing state to %s, assistMenus=%d, attendantMode=%d, m_GenericDelayedInterventionStatus=%d"),
           getStateName(sCurr), co.IsInAssistMenus(), co.IsAttendantModeOn(), sCurr->m_GenericDelayedInterventionStatus);
...
void MPProcedures::QueryForStateOverride(CString &csLastStateName, CString &csNextStateName)
{
    ...
    LPTSTR lpOverrideStateName = NULL;
	...
	try
	{
		...
		if ((bOverride) && (!(lReturnCode < 0)))
		{
			...
			trace(L2,_T("State Overridden LS=[%s] NS=[%s] OS=[%s] PC=[%d]"), (LPCTSTR)csLastStateName, (LPCTSTR)csNextStateName,
                (LPCTSTR)csOverrideStateName, dlParameterList.GetDescriptorCount());
    ...
    AssistMenuStateProcessing();
    sCurr->SendNextGenUIApplicationState((LPCTSTR) csNextStateName, bOverride, (LPCTSTR) csOverrideStateName);
}
