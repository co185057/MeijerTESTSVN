...
SMStateBase *SMSellBagsBase::Initialize()
{
	...
	//ra.RequestEnableAssistMode(false); //commented for SSCOP-5873 fix
    ...
    csItemSent = m_csBagItemCode;

		UpdateStateName(ps.GetPSText(LTT_BAGSALE, SCOT_LANGUAGE_PRIMARY));
		ps.SetCMFrame(_T("SellBags"));  
		COleVariant v;
		m_pAppPSX->SetConfigProperty(_T("ButtonGoBack"), _T("SellBags"), UI::PROPERTYVISIBLE, VARIANT_TRUE); 
...
//+SR828
/////////////////////////////////////////////
SMStateBase  *SMSellBagsBase::PSBackSpaceKey(void)
{
    PSClearInput();
...
SMStateBase *SMSellBagsBase::ConfirmQtyForBags(void)
{
   ...
   //@@@INFOPOINT
	...
	ps.SetCMFrame(_T("SellBagsConfirmQuantity"));
