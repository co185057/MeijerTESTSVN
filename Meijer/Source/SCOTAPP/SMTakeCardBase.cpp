...
SMStateBase  *SMTakeCardBase::DMCardRemoved(void)
{
  ...
  if (!fEndOfTran)
  {
	  if (bIsInFinalization)    //TAR245485
  	  {
		  // +SR797
		  SMStateBase *retState = TBTenderSelected(TB_TENDER_NONE);
		  SetTenderButton(-1);
  ...
  else if (lChangeDue || nDMCashInserted || !DMWasCashTaken())	//Need to give back any money since tranx was completed with a card TAR 127340 + SSCOADK-836
  {
  ...
  //SSCOADK-1121; This should be done in TakeReceipt, since it skips TakeReceipt now, needs to add it here:
