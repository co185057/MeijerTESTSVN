...
BEST SMSmmEditAddBase::g_SavedAnchorState = BES_STATERETURN;
...
SMStateBase  *SMSmmEditAddBase::Initialize(void)
{
  ...
  if (g_SavedAnchorState != BES_STATERETURN)
  {
      // restore anchorstate
      setAnchorState(g_SavedAnchorState);
      g_SavedAnchorState = BES_STATERETURN;
  }
  ...
  if (bSMMFailure)		// Failure returned from the Security Maintenance Mode event sent to security
  {
      ...
      if(co.fOperationsAllowSMMKeyInCode){
        ps.Button(_T("SMButton7"), BTT_KEYINCODE, false); //SR821	  
      }
      ps.ResetSMMItemList();
  }
  else
  {
	if (bSMMPreScanState || io.b_gIsThisItemPriceEmbedded) //SSCOP-2468
	{
        // NewUI
		ps.Button(_T("SMButton1"),BTT_SMM_ADDWT,false);
		ps.Button(_T("SMButton2"),BTT_SMM_ADDZEROWT,false);
		...
		ps.Button(_T("SMButton6"),BTT_SMM_EDIT_WTTOL,false);
        if(co.fOperationsAllowSMMKeyInCode){
            ps.Button(_T("SMButton7"), BTT_KEYINCODE, true); //SR821
        }
        // End NewUI
		
		//SR821
		int nLTT = co.fOperationsAllowSMMKeyInCode ? LTT_SMM_SCAN_OR_KEYIN : LTT_SMMEDITADD_INSTR; 
		//ps.SetLeadthruText(nLTT, PS_BLANK);
		ps.SetLTTinto2(nLTT); // SSCOP-540
	...
	else  
	{
		if (bIsSMMItemAdded)
		{
            int nLTT = co.fOperationsAllowSMMKeyInCode ? MSG_SMM_ITEMADDED_KEYIN : MSG_SMM_ITEMADDED; 
            ps.SetLTTinto2(nLTT);			
		...
		{
            int nLTT = co.fOperationsAllowSMMKeyInCode ? MSG_SMM_ITEMDELETED_KEYIN : MSG_SMM_ITEMDELETED; 
		    ps.SetLTTinto2(nLTT);
		...
		{
			//ps.SetLeadthruText(TXT_SMM_EDITADD_INSTR, PS_BLANK);  
			ps.SetLTTinto2(TXT_SMM_EDITADD_INSTR);//TD RFC 203307 instructional txt  
            // NewUI
			ps.Button(_T("SMButton1"),BTT_SMM_ADDWT,true);
			ps.Button(_T("SMButton2"),BTT_SMM_ADDZEROWT,true);
			ps.Button(_T("SMButton3"),BTT_SMM_DELWT,true);
  ...
  //SR821	
  COleVariant v = (co.fOperationsAllowSMMKeyInCode ? VARIANT_TRUE : VARIANT_FALSE);
  m_pAppPSX->SetConfigProperty(_T("SMButton7"), _T("SmmEditAdd"), UI::PROPERTYVISIBLE, v);
  //SR821
...
SMStateBase  *SMSmmEditAddBase::PSButton1(void)
{
  ...
  {
	ps.Button(_T("SMButton1"),BTT_SMM_ADDWT,false);
	ps.Button(_T("SMButton2"),BTT_SMM_ADDZEROWT,false);  //SSCOP-2468
	return STATE_NULL;
  }
  // - TAR 277988
  ps.ClearSMMItemList();
...
/////////////////////////////////////////////
// SR821
// PSButton7 - Key in item code
/////////////////////////////////////////////
SMStateBase  *SMSmmEditAddBase::PSButton7(void)
{
  trace(L6,_T("+SMSmmEditAddBase::PSButton7()"));
  ps.ClearSMMItemList();
  RETURNSTATE(SmmKeyInItemCode)
}
...
SMStateBase *SMSmmEditAddBase::DMScannerEncrypted(void)
{
    g_SavedAnchorState = getAnchorState();
    nStateBeforeInvalidBarcode =  BES_SMMEDITADD;
