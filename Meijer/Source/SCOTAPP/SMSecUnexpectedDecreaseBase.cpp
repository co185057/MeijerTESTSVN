...
SMStateBase  *SMSecUnexpectedDecreaseBase::Initialize(void)
{
	...
	//SSCOB-931 Don't report any wt decrease when transaction is suspended
	if(fInSuspendTrans)
	{
...
SMStateBase *SMSecUnexpectedDecreaseBase::OnBackToLGW(void)
{
	...
	{
		if (m_PrevState == BES_WAITFORAPPROVAL && !g_bgVoidTrx )
		{
...
SMStateBase *SMSecUnexpectedDecreaseBase::OnWtIncreaseNotAllowed()	 
{
	if (   ((m_PrevState == m_NextState) && (m_PrevState == BES_ASSISTEDTENDER))
        ...
        || ((m_PrevState == m_NextState) && (m_PrevState == BES_WAITFORAPPROVAL)) )
	{
		return VoidingWtIncrease(m_PrevState, m_NextState);
	}
	if ( bPrevStateBagAndEAS )   //Fix TAR435980
	{	
...
// Notes:		
//************************************************************************ 
SMStateBase *SMSecUnexpectedDecreaseBase::OnTBNeedData(void)
{
