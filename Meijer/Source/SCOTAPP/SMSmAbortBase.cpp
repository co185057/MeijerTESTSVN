...
// End NewUI
//TAR440495+
SMStateBase *SMSmAbortBase::OnWtIncreaseNotAllowed()
{
    ...
    return STATE(StoreModeState)::OnWtIncreaseNotAllowed();
}
...
SMStateBase *SMSmAbortBase::OnWtDecrease()
{
	if(!fInMultiSelectPickList)
	{
		TellApplicationStateToSecurity(KEY_AC_SET_APP_STATE_END,_T("SmAuthorization") ); 
	...
	trace(L6, _T("ra.OnNoRAPDataNeeded()"));
	ra.OnNoRAPDataNeeded();	
    return STATE(StoreModeState)::OnWtDecrease();
}
//TAR440495-
#pragma warning( pop )
